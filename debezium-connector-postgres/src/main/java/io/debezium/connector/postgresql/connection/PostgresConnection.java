...
public class PostgresConnection extends JdbcConnection {
    ...
    protected static final ConnectionFactory FACTORY = JdbcConnection.patternBasedFactory(URL_PATTERN,
            org.postgresql.Driver.class.getName(), PostgresConnection.class.getClassLoader());
    ...
    /**
     * Prints out information about the REPLICA IDENTITY status of a table. This in turn determines how much information
     * is available for UPDATE and DELETE operations for logical replication.
     *
     * @param tableId
     ...
     * 
     * @throws SQLException
     *             if there is a problem obtaining the replica identity information for the given table
     */
    public ServerInfo.ReplicaIdentity readReplicaIdentityInfo(TableId tableId) throws SQLException {
        String statement = "SELECT relreplident FROM pg_catalog.pg_class c "
                + "LEFT JOIN pg_catalog.pg_namespace n ON c.relnamespace=n.oid " + "WHERE n.nspname=? and c.relname=?";
     ...
     * @param pluginName
     *            the name of the plugin used for the desired slot
     * 
     ...
     *            the name of the plugin used for the desired slot
     * 
     * @return the {@link ServerInfo.ReplicationSlot} object or a {@link ServerInfo.ReplicationSlot#INVALID} if the slot
     *         is not valid
     * 
     * @throws SQLException
     *             is thrown by the underlying JDBC
     */
    ...
    /**
     * Fetches a replication slot, repeating the query until either the slot is created or until the max number of
     ...
     * 
     * @throws SQLException
     *             is thrown by the underyling jdbc driver
     * @throws InterruptedException
     *             is thrown if we don't return an answer within the set number of retries
     */
    @VisibleForTesting
    ...
    private Long tryParseLsn(String slotName, String pluginName, String database, ResultSet rs, String column)
        ...
        try {
            ...
            throw new ConnectException("Value " + column + " in the pg_replication_slots table for slot = '" + slotName
                    + "', plugin = '" + pluginName + "', database = '" + database
     ...
     *
     * @param slotName
     *            the name of the replication slot, may not be null
     * 
     ...
     *            the publication name, may not be null
     * 
    ...
    public long currentXLogLocation() throws SQLException {
        ...
        query(majorVersion >= 10 ? "select * from pg_current_wal_lsn()" : "select * from pg_current_xlog_location()",
                rs -> {
                    if (!rs.next()) {
    ...
    protected static void defaultSettings(Configuration.Builder builder) {
        // we require Postgres 9.4 as the minimum server version since that's where logical replication was first
        // introduced
    ...
    protected Optional<ColumnEditor> readTableColumn(ResultSet columnMetadata, TableId tableId,
            Tables.ColumnNameFilter columnFilter) throws SQLException {
