...
/**
 * Implementation of a {@link ReplicationConnection} for Postgresql. Note that replication connections in PG cannot
 * execute regular statements but only a limited number of replication-related commands.
 *
...
public class PostgresReplicationConnection extends JdbcConnection implements ReplicationConnection {
     ...
     *
     * @param config
     *            the JDBC configuration for the connection; may not be null
     * @param slotName
     *            the name of the DB slot for logical replication; may not be null
     * @param publicationName
     *            the name of the DB publication for logical replication; may not be null
     * @param plugin
     *            decoder matching the server side plug-in used for streaming changes; may not be null
     * @param dropSlotOnClose
     *            whether the replication slot should be dropped once the connection is closed
     * @param statusUpdateInterval
     *            the interval at which the replication connection should periodically send status
     * @param exportSnapshot
     *            whether the replication should export a snapshot when created
     * @param typeRegistry
     *            registry with PostgreSQL types
     * @param streamParams
     *            additional parameters to pass to the replication stream
     * @param schema
     *            the schema; must not be null
     *
     *            updates to the server
     */
    private PostgresReplicationConnection(Configuration config, String slotName, String publicationName,
            PostgresConnectorConfig.LogicalDecoder plugin, boolean dropSlotOnClose, boolean exportSnapshot,
            Duration statusUpdateInterval, TypeRegistry typeRegistry, Properties streamParams, PostgresSchema schema) {
    ...
    protected void initPublication() {
        if (PostgresConnectorConfig.LogicalDecoder.PGOUTPUT.equals(plugin)) { // 插件为PGOUTPUT才创建订阅
            LOGGER.info("Initializing PgOutput logical decoder publication");
            try {
                String selectPublication = String.format("SELECT COUNT(1) FROM pg_publication WHERE pubname = '%s'",
                        publicationName);
        ...
        try {
            ...
            execute(statement -> {
                ...
                try (ResultSet rs = statement.executeQuery(identifySystemStatement)) {
                    ...
                    LOGGER.info("执行IDENTIFY_SYSTEM之后：" + xlogpos);
            ...
            if (slotCreationInfo != null) {
                ...
                // this is a new slot or we weren't able to read a valid flush LSN pos, so we always start from the xlog
                // pos that was reported
    ...
    /**
     * creating a replication connection and starting to stream involves a few steps: 1. we create the connection and
     * ensure that a. the slot exists b. the slot isn't currently being used 2. we query to get our potential start
     * position in the slot (lsn) 3. we try and start streaming, depending on our options (such as in wal2json) this may
     * fail, which can result in the connection being killed and we need to start the process over if we are using a
     * temporary slot 4. actually start the streamer
     *
     * This method takes care of all of these and this method queries for a default starting position If you know where
     * you are starting from you should call {@link #startStreaming(Long)}, this method delegates to that method
     * 
     * @return
     * 
     * @throws SQLException
    ...
    public void initConnection() throws SQLException, InterruptedException {
        ...
        initPublication();
        if (!hasInitedSlot) {
            LOGGER.info("初始化replicationslot");
            initReplicationSlot(); // 核心方法
        }
    }
    ...
    public Optional<SlotCreationResult> createReplicationSlot() throws SQLException {
        ...
        try (Statement stmt = pgConnection().createStatement()) {
            String createCommand = String.format("CREATE_REPLICATION_SLOT %s %s LOGICAL %s", slotName, tempPart,
                    plugin.getPostgresPluginName()); // 创建逻辑槽
    ...
    private ReplicationStream createReplicationStream(final LogSequenceNumber startLsn, boolean skipFirstFlushRecord)
        ...
        return new ReplicationStream() {
            ...
            public boolean readPending(ReplicationMessageProcessor processor)
                ...
                if (read == null || messageDecoder.shouldMessageBeSkipped(read, lastReceiveLsn, startingLsn,
                        skipFirstFlushRecord)) {
                    return false;
                }
            ...
            public void startKeepAlive(ExecutorService service) {
                LOGGER.info("into startKeepAlive方法，keepAliveRuning状态为" + keepAliveRunning);
                if (keepAliveExecutor == null) {
                    ...
                    keepAliveExecutor.submit(() -> {
                        while (keepAliveRunning.get()) {
                            try {
                                LOGGER.info("探测逻辑复制活跃的线程：" + Thread.currentThread().getId());
            ...
            private void processWarnings(final boolean forced) throws SQLException {
                if (--warningCheckCounter == 0 || forced) {
                    ...
                    for (SQLWarning w = connection().getWarnings(); w != null; w = w.getNextWarning()) {
                        LOGGER.debug("Server-side message: '{}', state = {}, code = {}", w.getMessage(),
                                w.getSQLState(), w.getErrorCode());
                    }
                }
            }
    ...
    private PGReplicationStream startPgReplicationStream(final LogSequenceNumber lsn,
            Function<ChainedLogicalStreamBuilder, ChainedLogicalStreamBuilder> configurator) throws SQLException {
        assert lsn != null;
        ChainedLogicalStreamBuilder streamBuilder = pgConnection().getReplicationAPI().replicationStream().logical()
                .withSlotName(slotName).withStartPosition(lsn).withSlotOptions(streamParams);
    ...
    protected static void defaultSettings(Configuration.Builder builder) {
        ...
        builder.with("replication", "database").with("preferQueryMode", "simple"); // replication protocol only supports
                                                                                   // simple query mode
    }
    ...
    protected static class ReplicationConnectionBuilder implements Builder {
        ...
        public ReplicationConnectionBuilder streamParams(final String slotStreamParams) {
            LOGGER.info("逻辑复制参数：" + slotStreamParams);
        ...
        public ReplicationConnection build() {
            ...
            return new PostgresReplicationConnection(config, slotName, publicationName, plugin, dropSlotOnClose,
                    exportSnapshot, statusUpdateIntervalVal, typeRegistry, slotStreamParams, schema);
        }
