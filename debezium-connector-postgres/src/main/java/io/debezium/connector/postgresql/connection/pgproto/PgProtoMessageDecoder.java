...
/**
 * ProtoBuf deserialization of message sent by <a href="https://github.com/debezium/postgres-decoderbufs">Postgres
 * Decoderbufs</a>. Only one message is delivered for processing.
 *
...
public class PgProtoMessageDecoder extends AbstractMessageDecoder {
    ...
    private static final Set<Op> SUPPORTED_OPS = Collect.unmodifiableSet(Op.INSERT, Op.UPDATE, Op.DELETE, Op.BEGIN,
            Op.COMMIT);

    private boolean warnedOnUnkownOp = false;

    @Override
    public void processNotEmptyMessage(final ByteBuffer buffer, ReplicationMessageProcessor processor,
        ...
        try {
            ...
            if (!message.getNewTypeinfoList().isEmpty()
                ...
                throw new ConnectException(
                        String.format("Message from transaction {} has {} data columns but only {} of type info",
                                Integer.toUnsignedLong(message.getTransactionId()), message.getNewTupleCount(),
