...
public class PostgresStreamingChangeEventSource implements StreamingChangeEventSource {
    ...
    /**
     * The minimum of (number of event received since the last event sent to Kafka, number of event received since last
     * WAL growing warning issued).
     */
    private long numberOfEventsSinceLastEventSentOrWalGrowingWarning = 0; // 统计未发送到kafka的消息
    ...
    public PostgresStreamingChangeEventSource(PostgresConnectorConfig connectorConfig, Snapshotter snapshotter,
            PostgresOffsetContext offsetContext, PostgresConnection connection, EventDispatcher<TableId> dispatcher,
            ErrorHandler errorHandler, Clock clock, PostgresSchema schema, PostgresTaskContext taskContext,
        ...
        this.clock = clock;
        this.schema = schema;
        this.offsetContext = (offsetContext != null) ? offsetContext // 如果前置offset为空，则初始化
    ...
    public void execute(ChangeEventSourceContext context) throws InterruptedException {
        ...
        try {
            if (hasStartLsnStoredInContext) { // 上次lsn位置
                // start streaming from the last recorded position in the offset
                final Long lsn = offsetContext.lastCompletelyProcessedLsn() != null
                        ? offsetContext.lastCompletelyProcessedLsn() : offsetContext.lsn();
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("retrieved latest position from stored offset '{}'",
                            ReplicationConnection.format(lsn));
                }
                LOGGER.info(
                        "find previous LSN found in Kafka, streaming from the latest xlogpos or flushed LSN... " + lsn);
            ...
            stream.startKeepAlive(Executors.newSingleThreadExecutor()); // 加入线程池，开启连接
                ...
                boolean receivedMessage = stream.readPending(message -> {
                    ...
                    if (message.isTransactionalMessage()) {
                        ...
                        offsetContext.updateWalPosition(lsn, lastCompletelyProcessedLsn, message.getCommitTime(),
                                message.getTransactionId(), null, taskContext.getSlotXmin(connection));
                        if (message.getOperation() == Operation.BEGIN) {
                            LOGGER.info("BEGIN 事件");
                            dispatcher.dispatchTransactionStartedEvent(Long.toString(message.getTransactionId()),
                                    offsetContext);
                        } else if (message.getOperation() == Operation.COMMIT) {
                            LOGGER.info("COMMIT 事件");
                            dispatcher.dispatchTransactionCommittedEvent(offsetContext);
                        }
                        maybeWarnAboutGrowingWalBacklog(true); // 触发告警
                        return;
                    }
                    // DML event
                    else {
                        ...
                        if (message.getOperation() != Operation.NOOP) {
                            ...
                            Objects.requireNonNull(tableId); // 非空判断
                        }

                        offsetContext.updateWalPosition(lsn, lastCompletelyProcessedLsn, message.getCommitTime(),
                                message.getTransactionId(), tableId, taskContext.getSlotXmin(connection));

                        boolean dispatched = (message.getOperation() == Operation.NOOP) ? false
                                : dispatcher.dispatchDataChangeEvent(tableId, new PostgresChangeRecordEmitter(
                                        offsetContext, clock, connectorConfig, schema, connection, message));
            ...
            if (replicationConnection != null) {
                ...
                // TODO author=Horia Chiorean date=08/11/2016 description=Ideally we'd close the stream, but it's not
                // reliable atm (see javadoc)
    ...
    private void skipMessage(final Long lsn) throws SQLException, InterruptedException {
        lastCompletelyProcessedLsn = lsn;
        offsetContext.updateWalPosition(lsn, lastCompletelyProcessedLsn, null, null, null,
     ...
     * This in turn means no LSN is ever acknowledged with the replication slot, causing any ever growing WAL backlog.
     * <p>
     * This situation typically occurs if there are changes on the database server, (e.g. in a blacklisted database),
     * but none of them is in a whitelisted table. To prevent this, heartbeats can be used, as they will allow us to
     * commit offsets also when not propagating any "real" change event.
     * <p>
     * The purpose of this method is to detect this situation and log a warning every
     * {@link #GROWING_WAL_WARNING_LOG_INTERVAL} filtered events.
     *
     * @param dispatched
