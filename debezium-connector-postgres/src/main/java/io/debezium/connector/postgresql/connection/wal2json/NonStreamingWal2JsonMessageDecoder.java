 ...
 * <a href="https://github.com/eulerto/wal2json">wal2json</a> logical decoding plugin. The plugin sends all changes in
 * one transaction as a single batch in a big JSON file and they are passed to processor one-by-one.
 *
...
public class NonStreamingWal2JsonMessageDecoder extends AbstractMessageDecoder {
        ...
        try {
            ...
            if (changes.isEmpty()) {
                ...
                while (it.hasNext()) {
                    ...
                    processor.process(new Wal2JsonReplicationMessage(txId, commitTime, value.asDocument(),
                            containsMetadata, !it.hasNext(), typeRegistry));
                }
                processor.process(new TransactionMessage(Operation.COMMIT, txId, commitTime));
            }
    ...
    public ChainedLogicalStreamBuilder optionsWithoutMetadata(ChainedLogicalStreamBuilder builder) {
        return builder.withSlotOption("pretty-print", 1).withSlotOption("write-in-chunks", 0)
