 ...
 * https://www.postgresql.org/docs/10/protocol-logicalrep-message-formats.html for the protocol specification.
 *
...
public class PgOutputMessageDecoder extends AbstractMessageDecoder {
    ...
    public enum MessageType {
        RELATION, BEGIN, COMMIT, INSERT, UPDATE, DELETE, TYPE, ORIGIN, TRUNCATE;
    ...
    @Override
    public void processNotEmptyMessage(ByteBuffer buffer, ReplicationMessageProcessor processor,
    ...
    public ChainedLogicalStreamBuilder optionsWithMetadata(ChainedLogicalStreamBuilder builder) {
        return builder.withSlotOption("proto_version", 1).withSlotOption("publication_names",
     ...
     *
     * @param buffer
     *            The replication stream buffer
     * @param processor
     *            The replication message processor
     */
    ...
    private void handleRelationMessage(ByteBuffer buffer, TypeRegistry typeRegistry) throws SQLException {
        ...
        LOGGER.trace("Event: {}, RelationId: {}, Replica Identity: {}, Columns: {}", MessageType.RELATION, relationId,
                replicaIdentityId, columnCount);
        ...
        try (final PostgresConnection connection = new PostgresConnection(config.getConfiguration())) {
            ...
            if (primaryKeyColumns == null || primaryKeyColumns.isEmpty()) {
                primaryKeyColumns = new HashSet<>(
                        connection.readTableUniqueIndices(databaseMetadata, new TableId(null, schemaName, tableName)));
            }
        }
        ...
        Table table = resolveRelationFromMetadata(
                new PgOutputRelationMetaData(relationId, schemaName, tableName, columns));
        config.getSchema().applySchemaChangesForTable(relationId, table);
    }

    private Map<String, Boolean> getTableColumnOptionalityFromDatabase(DatabaseMetaData databaseMetadata,
        ...
        try {
            try (ResultSet resultSet = databaseMetadata.getColumns(null, schemaName, tableName, null)) {
                while (resultSet.next()) {
                    columnOptionality.put(resultSet.getString("COLUMN_NAME"),
    ...
    private Set<String> getTablePrimaryKeyColumnNamesFromDatabase(DatabaseMetaData databaseMetadata, String schemaName,
            String tableName) {
     ...
     *            The replication message processor
     */
    private void decodeInsert(ByteBuffer buffer, TypeRegistry typeRegistry, ReplicationMessageProcessor processor)
            throws SQLException, InterruptedException {
        int relationId = buffer.getInt();
        ...
        if (!resolvedTable.isPresent()) {
            ...
            processor.process(new PgOutputReplicationMessage(Operation.INSERT, table.id().toDoubleQuotedString(),
                    commitTimestamp, transactionId, null, columns));
        }
    }
     ...
     *            The replication message processor
     */
    private void decodeUpdate(ByteBuffer buffer, TypeRegistry typeRegistry, ReplicationMessageProcessor processor)
            throws SQLException, InterruptedException {
        int relationId = buffer.getInt();
        ...
        if (!resolvedTable.isPresent()) {
            ...
            // 'K' (Optional) - States the following tuple-data is the old tuple, only for replica identity full
            // configs.
            //
            ...
            List<Column> columns = resolveColumnsFromStreamTupleData(buffer, typeRegistry, table);
            processor.process(new PgOutputReplicationMessage(Operation.UPDATE, table.id().toDoubleQuotedString(),
                    commitTimestamp, transactionId, oldColumns, columns));
        }
    }
     ...
     *            The replication message processor
     */
    private void decodeDelete(ByteBuffer buffer, TypeRegistry typeRegistry, ReplicationMessageProcessor processor)
            throws SQLException, InterruptedException {
        int relationId = buffer.getInt();
        ...
        if (!resolvedTable.isPresent()) {
            ...
            Table table = resolvedTable.get();
            List<Column> columns = resolveColumnsFromStreamTupleData(buffer, typeRegistry, table);
            processor.process(new PgOutputReplicationMessage(Operation.DELETE, table.id().toDoubleQuotedString(),
                    commitTimestamp, transactionId, columns, null));
        }
    }
     ...
     *
     * @param relationId
     *            The replication message stream's relation identifier
     * 
     * @return table resolved from a prior relation message or direct lookup from the schema or empty when the table is
     *         filtered
     */
     ...
     *
     * @param metadata
     *            The relation metadata collected from previous 'R' replication stream messages
     * 
    ...
    private Table resolveRelationFromMetadata(PgOutputRelationMetaData metadata) {
        ...
        for (ColumnMetaData columnMetadata : metadata.getColumns()) {
            ColumnEditor editor = io.debezium.relational.Column.editor().name(columnMetadata.getColumnName())
                    ...
                    .length(columnMetadata.getLength()).scale(columnMetadata.getScale());
        ...
        Table table = Table.editor().addColumns(columns).setPrimaryKeyNames(pkColumnNames)
                .tableId(metadata.getTableId()).create();
     ...
     *            The replication stream buffer
     * @param typeRegistry
     *            The database type registry
     * @param table
     *            The database table
     * 
     * @return list of replication message columns
     */
    private static List<Column> resolveColumnsFromStreamTupleData(ByteBuffer buffer, TypeRegistry typeRegistry,
            Table table) {
        ...
        for (short i = 0; i < numberOfColumns; ++i) {
            ...
            if (type == 't') {
                ...
                columns.add(
                        new AbstractReplicationMessageColumn(columnName, columnType, typeExpression, optional, true) {
                            ...
                            public Object getValue(PgConnectionSupplier connection, boolean includeUnknownDatatypes) {
                                return PgOutputReplicationMessage.getValue(columnName, columnType, typeExpression,
                                        valueStr, connection, includeUnknownDatatypes, typeRegistry);
                            }
                ...
                columns.add(new UnchangedToastedReplicationMessageColumn(columnName, columnType, typeExpression,
                        optional, true) {
                    @Override
                    public String toString() {
