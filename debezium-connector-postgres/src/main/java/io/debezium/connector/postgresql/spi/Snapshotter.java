 ...
 *
 * Namely: - Should a snapshot occur at all - Should streaming occur - What queries should be used to snapshot
 *
 * While many default snapshot modes are provided with debezium (see documentation for details) a custom implementation
 * of this interface can be provided by the implementor which can provide more advanced functionality, such as partial
 * snapshots
 *
 * Implementor's must return true for either {@link #shouldSnapshot()} or {@link #shouldStream()} or true for both.
 */
@Incubating
public interface Snapshotter {
    ...
    /**
     * @return true if when creating a slot, a snapshot should be exported, which can be used as an alternative to
     *         taking a lock
     */
    ...
    /**
     * Generate a valid postgres query string for the specified table, or an empty {@link Optional} to skip snapshotting
     * this table (but that table will still be streamed from)
     *
     * @param tableId
     ...
     *
     * @param newSlotInfo
     *            if a new slow was created for snapshotting, this contains information from the
    ...
    default Optional<String> snapshotTableLockingStatement(Duration lockTimeout, Set<TableId> tableIds) {
        ...
        tableIds.forEach(tableId -> statements.append("LOCK TABLE ").append(tableId.toDoubleQuotedString())
