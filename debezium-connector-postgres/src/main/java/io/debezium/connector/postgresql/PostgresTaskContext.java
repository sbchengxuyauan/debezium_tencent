...
/**
 * The context of a {@link PostgresConnectorTask}. This deals with most of the brunt of reading various configuration
...
public class PostgresTaskContext extends CdcSourceTaskContext {
    ...
    protected ReplicationConnection createReplicationConnection(boolean exportSnapshot) throws SQLException {
        ...
        return ReplicationConnection.builder(config.jdbcConfig()).withSlot(config.slotName())
                .withPublication(config.publicationName()).withPlugin(config.plugin()).dropSlotOnClose(dropSlotOnStop)
                .streamParams(config.streamParams()).statusUpdateInterval(config.statusUpdateInterval())
                .withTypeRegistry(schema.getTypeRegistry()).exportSnapshotOnCreate(exportSnapshot).withSchema(schema)
                .build();
    }

    protected PostgresConnection createConnection() {
        LOGGER.info("逻辑槽连接");
        return new PostgresConnection(config.jdbcConfig()); // 创建逻辑槽，需要新的连接
    }
