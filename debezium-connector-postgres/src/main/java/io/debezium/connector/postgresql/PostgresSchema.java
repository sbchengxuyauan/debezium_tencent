...
/**
 * Component that records the schema information for the {@link PostgresConnector}. The schema information contains the
 * {@link Tables table definitions} and the Kafka Connect {@link #schemaFor(TableId) Schema}s for each table, where the
...
public class PostgresSchema extends RelationalDatabaseSchema {
    ...
    protected PostgresSchema(PostgresConnectorConfig config, TypeRegistry typeRegistry, Charset databaseCharset,
        ...
        super(config, topicSelector, new Filters(config).tableFilter(), new Filters(config).columnFilter(),
                getTableSchemaBuilder(config, typeRegistry, databaseCharset), false, config.getKeyMapper());
    ...
    private static TableSchemaBuilder getTableSchemaBuilder(PostgresConnectorConfig config, TypeRegistry typeRegistry,
        ...
        PostgresValueConverter valueConverter = new PostgresValueConverter(databaseCharset, config.getDecimalMode(),
                config.getTemporalPrecisionMode(), ZoneOffset.UTC, null, config.includeUnknownDatatypes(), typeRegistry,
                config.hStoreHandlingMode(), config.intervalHandlingMode(), config.toastedValuePlaceholder());

        return new TableSchemaBuilder(valueConverter, SchemaNameAdjuster.create(LOGGER),
     ...
     *
     * @param connection
     *            a {@link JdbcConnection} instance, never {@code null}
     * @param printReplicaIdentityInfo
     *            whether or not to look and print out replica identity information about the tables
     * 
     * @return this object so methods can be chained together; never null
     * 
     * @throws SQLException
     *             if there is a problem obtaining the schema from the database server
     */
    protected PostgresSchema refresh(PostgresConnection connection, boolean printReplicaIdentityInfo)
            throws SQLException {
     ...
     * @param refreshToastableColumns
     *            refreshes the cache of toastable columns for `tableId`, if {@code true}
     * 
     * @throws SQLException
     *             if there is a problem refreshing the schema from the database server
     */
    protected void refresh(PostgresConnection connection, TableId tableId, boolean refreshToastableColumns)
            throws SQLException {
     ...
     *
     * @param table
     *            constructed externally - typically from decoder metadata
     */
    ...
    private void refreshToastableColumnsMap(PostgresConnection connection, TableId tableId) {
        ...
        // last resort.
        //
        // Also, we cannot account for the possibility that future versions of PostgreSQL introduce new storage
        // strategies
        ...
        //
        // NOTE (Ian Axelrod):
        // I Would prefer to use data provided by PgDatabaseMetaData, but the PG JDBC driver does not expose storage
        // type
        ...
        String statement = "select att.attname" + " from pg_attribute att "
                + " join pg_class tbl on tbl.oid = att.attrelid" + " join pg_namespace ns on tbl.relnamespace = ns.oid"
                + " where tbl.relname = ?" + " and ns.nspname = ?" + " and att.attnum > 0"
                + " and att.attstorage in ('x', 'e', 'm')" + " and not att.attisdropped;";
     ...
     *
     * @param relationId
     *            the postgres relation unique identifier for the table
     * @param table
     *            externally constructed table, typically from the decoder; must not be null
     */
    public void applySchemaChangesForTable(int relationId, Table table) {
        assert table != null;

        if (isFilteredOut(table.id())) {
            LOGGER.trace("Skipping schema refresh for table '{}' with relation '{}' as table is filtered", table.id(),
                    relationId);
            return;
        }
     ...
     * <p>
     * This implementation relies on a prior call to {@link #applySchemaChangesForTable(int, Table)} to have applied
     * schema changes from a replication stream with the {@code relationId} for the relationship to exist and be capable
     * of lookup.
     *
     * @param relationId
     *            the unique table relation identifier
     * 
