...
public class ServerInfo {
    ...
    public String toString() {
        ...
        String roles = permissionsByRoleName.entrySet().stream()
                .map(entry -> "\trole '" + entry.getKey() + "' [" + entry.getValue() + "]")
                .collect(Collectors.joining(lineSeparator));

        return "user '" + username + "' connected to database '" + database + "' on " + server + " with roles:"
                + lineSeparator + roles;
    }
    ...
    protected static class ReplicationSlot {
         ...
         *
         * This value represents the latest LSN that the logical replication consumer has reported back to postgres.
         * 
         ...
         *
         * The restart_lsn will be the LSN the slot restarts from in the event of the disconnect. This can be distinct
         * from the `confirmed_flushed_lsn` as the two pointers are moved independently
         * 
