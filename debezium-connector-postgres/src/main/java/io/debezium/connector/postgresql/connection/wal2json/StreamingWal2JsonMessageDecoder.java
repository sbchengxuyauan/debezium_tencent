...
/**
 * <p>
 * JSON deserialization of a message sent by <a href="https://github.com/eulerto/wal2json">wal2json</a> logical decoding
 * plugin. The plugin sends all changes in one transaction as a single batch and they are passed to processor
 * one-by-one. The JSON file arrives in chunks of a big JSON file where the chunks are not valid JSON itself.
 * </p>
 *
 * <p>
 * There are four different chunks that can arrive from the decoder. <b>Beginning of message</b></br>
 * 
 * <pre>
 * {
 *   "xid": 563,
 ...
 *
 * <p>
 * For parsing purposes it is necessary to add or remove a fragment of JSON to make a well-formatted JSON out of it. The
 * last message is just dropped.
 * </p>
 * 
...
public class StreamingWal2JsonMessageDecoder extends AbstractMessageDecoder {
    ...
    /**
     * To identify if the last current chunk is the last one we can send the current one for processing only after we
     * read the next one or the end of message fragment.
     */
            ...
            if (!messageInProgress) {
                ...
                if (firstChar != LEFT_BRACE) {
                    ...
                    if (getLastNonWhiteChar(content) != RIGHT_BRACE) {
                        // Chunks are enabled and we have an unfinished message, it is necessary to add a sequence of
                        // closing chars
    ...
    /**
     * This method is called when a database server or Debezium crashes. With wal2json streaming mode it can happen that
     * the message preamble is no replayed but the message is streamed form the middle. This issue is very hard to
     * reproduce so a precaution is taken and metadata are filled with synthetic values.
     * <p>
    ...
    private void doProcessMessage(ReplicationMessageProcessor processor, TypeRegistry typeRegistry, byte[] content,
            boolean lastMessage) throws IOException, SQLException, InterruptedException {
        if (content != null) {
            ...
            processor.process(new Wal2JsonReplicationMessage(txId, commitTime, change, containsMetadata, lastMessage,
                    typeRegistry));
        } else {
    ...
    public ChainedLogicalStreamBuilder optionsWithMetadata(ChainedLogicalStreamBuilder builder) {
        return optionsWithoutMetadata(builder).withSlotOption("include-not-null", "true");
    }

    @Override
    public ChainedLogicalStreamBuilder optionsWithoutMetadata(ChainedLogicalStreamBuilder builder) {
        return builder.withSlotOption("pretty-print", 1).withSlotOption("write-in-chunks", 1)
                .withSlotOption("include-xids", 1).withSlotOption("include-timestamp", 1);
    }
