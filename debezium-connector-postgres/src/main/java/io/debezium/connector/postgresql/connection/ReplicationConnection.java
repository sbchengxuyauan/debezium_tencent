...
/**
 * A Postgres logical streaming replication connection. Replication connections are established for a slot and a given
 * plugin and WAL offset tracking is specific to the [database name, plugin name, slot name] triple.
 *
...
public interface ReplicationConnection extends AutoCloseable {
    ...
    /**
     * Opens a stream for reading logical replication changes from the last known position of the slot for which the
     * connection was opened. The last known position is based on the server's {@code confirmed_flush_lsn} value from
     * the {@code pg_replication_slots} table.
     * <p>
     * If there is no "last known value" (i.e. the connection is for a newly created slot) then the Postgres server will
     * start streaming changes from that last position reported during this connection's creation.
     * </p>
     *
     * @return a {@link PGReplicationStream} from which data is read; never null
     * 
     * @throws SQLException
     *             if there is a problem obtaining the replication stream
     */
     ...
     * <p>
     * Note that it is possible for a server to have recycled old WAL segments (see the {@code wal_keep_segments}
     * setting). If that is the case, then even though a LSN number may be valid, the server will not stream back any
     * changes because they are not available.
     * </p>
     * 
     * @param offset
     *            a value representing the WAL sequence number where replication should start from; if the value is
     *            {@code null} or negative, this behaves exactly like {@link #startStreaming()}.
     * 
     * @return a {@link PGReplicationStream} from which data is read; never null
     * 
    ...
    /**
     * Creates a new replication slot with the given option and returns the result of the command, which may contain
     * results (depending on postgres versions)
     *
     * @throws SQLException
     *             if anything fails
     */
    ...
    /**
     * Forces the connection to be created, is called by startStreaming, but can be called manually in cases where we
     * want the connection but not to to start streaming yet.
     *
     ...
     *
     * @param jdbcConfig
     *            a {@link Configuration} instance that contains JDBC settings; may not be null
     * 
    ...
    interface Builder {
         ...
         *
         * @param slotName
         *            the name of the slot, may not be null.
         * 
         ...
         *            the name of the publication, may not be null.
         * 
         * @return this instance
         * 
         ...
         *            true if the slot should be dropped once the connection is closed, false otherwise
         * 
         * @return this instance
         * 
         ...
         *            a duration; null or non-positive value causes Postgres' default to be applied
         * 
         * @return this instance
         */
         ...
         *
         * @param streamParams
         *            String of key and value pairs declared with "=". Pairs are separated by ";". Example:
         ...
         *            true if a snapshot should be exported, false if otherwise
         * 
         * @return this instance
         * 
