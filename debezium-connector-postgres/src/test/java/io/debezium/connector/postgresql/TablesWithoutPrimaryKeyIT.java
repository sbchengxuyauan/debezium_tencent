...
public class TablesWithoutPrimaryKeyIT extends AbstractRecordsProducerTest {
    ...
    private static final String STATEMENTS = "CREATE SCHEMA nopk;" + "CREATE TABLE nopk.t1 (pk INT UNIQUE, val INT);"
            + "CREATE TABLE nopk.t2 (pk INT UNIQUE, val INT UNIQUE);" + "CREATE TABLE nopk.t3 (pk INT, val INT);"
            + "INSERT INTO nopk.t1 VALUES (1,10);" + "INSERT INTO nopk.t2 VALUES (2,20);"
    ...
    public void shouldProcessFromSnapshot() throws Exception {
        ...
        start(PostgresConnector.class,
                TestHelper.defaultConfig().with(PostgresConnectorConfig.SNAPSHOT_MODE, SnapshotMode.INITIAL_ONLY)
                        .with(PostgresConnectorConfig.SCHEMA_WHITELIST, "nopk").build());
