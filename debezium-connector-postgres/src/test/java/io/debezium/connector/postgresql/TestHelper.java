...
public final class TestHelper {
     ...
     * @param dropOnClose
     *            true if the slot should be dropped upon close
     * 
     * @return the PostgresConnection instance; never null
     * 
     * @throws SQLException
     *             if there is a problem obtaining a replication connection
     */
    public static ReplicationConnection createForReplication(String slotName, boolean dropOnClose) throws SQLException {
        ...
        return ReplicationConnection.builder(defaultJdbcConfig()).withPlugin(plugin).withSlot(slotName)
                .withTypeRegistry(getTypeRegistry()).dropSlotOnClose(dropOnClose)
                .statusUpdateInterval(Duration.ofSeconds(10)).withSchema(getSchema(config)).build();
    }
    ...
    public static PostgresConnectorConfig.LogicalDecoder decoderPlugin() {
        ...
        return (s == null || s.length() == 0) ? PostgresConnectorConfig.LogicalDecoder.DECODERBUFS
                : PostgresConnectorConfig.LogicalDecoder.parse(s);
    }
     ...
     *
     * @param appName
     *            the name of the application used for PostgreSQL diagnostics
     *
     * @return the PostgresConnection instance; never null
     */
     ...
     * @param statement
     *            A SQL statement
     * @param furtherStatements
     *            Further SQL statement(s)
     */
    ...
    public static void dropAllSchemas() throws SQLException {
        ...
        String dropStmts = schemaNames.stream().map(schema -> "\"" + schema.replaceAll("\"", "\"\"") + "\"")
    ...
    public static PostgresSchema getSchema(PostgresConnectorConfig config, TypeRegistry typeRegistry) {
        return new PostgresSchema(config, typeRegistry, Charset.forName("UTF-8"), PostgresTopicSelector.create(config));
    }
    ...
    public static JdbcConfiguration defaultJdbcConfig() {
        return JdbcConfiguration.copy(Configuration.fromSystemProperties("database."))
                ...
                .withDefault(JdbcConfiguration.HOSTNAME, "localhost").withDefault(JdbcConfiguration.PORT, 5432)
                .withDefault(JdbcConfiguration.USER, "postgres").withDefault(JdbcConfiguration.PASSWORD, "postgres")
                .build();
    }
    ...
    protected static Configuration.Builder defaultConfig() {
        ...
        jdbcConfiguration
                .forEach((field, value) -> builder.with(PostgresConnectorConfig.DATABASE_CONFIG_PREFIX + field, value));
    ...
    protected static SourceInfo sourceInfo() {
        return new SourceInfo(new PostgresConnectorConfig(
                Configuration.create().with(PostgresConnectorConfig.SERVER_NAME, TEST_SERVER)
                        .with(PostgresConnectorConfig.DATABASE_NAME, TEST_DATABASE).build()));
    }
    ...
    protected static boolean publicationExists(String publicationName) {
        if (decoderPlugin().equals(PostgresConnectorConfig.LogicalDecoder.PGOUTPUT)) {
            try (PostgresConnection connection = create()) {
                String query = String.format("SELECT pubname FROM pg_catalog.pg_publication WHERE pubname = '%s'",
                        publicationName);
                try {
    ...
    protected static void waitForDefaultReplicationSlotBeActive() {
        try (PostgresConnection connection = create()) {
            Awaitility.await().atMost(5, TimeUnit.SECONDS).until(() -> connection.prepareQueryAndMap(
                    "select * from pg_replication_slots where slot_name = ? and database = ? and plugin = ? and active = true",
                    statement -> {
