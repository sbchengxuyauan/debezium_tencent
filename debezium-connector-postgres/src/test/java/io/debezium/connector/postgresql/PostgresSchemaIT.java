...
public class PostgresSchemaIT {
    ...
    private static final String[] TEST_TABLES = new String[] { "public.numeric_table", "public.numeric_decimal_table",
            "public.string_table", "public.cash_table", "public.bitbin_table", "public.network_address_table",
            "public.cidr_network_address_table", "public.macaddr_table", "public.time_table", "public.text_table",
            "public.geom_table", "public.range_table", "public.array_table",
            "\"Quoted_\"\" . Schema\".\"Quoted_\"\" . Table\"", "public.custom_table" };
    ...
    public void shouldLoadSchemaForBuiltinPostgresTypes() throws Exception {
        ...
        try (PostgresConnection connection = TestHelper.create()) {
            ...
            assertTableSchema("public.numeric_table", "si, i, bi, r, db, ss, bs, b", Schema.OPTIONAL_INT16_SCHEMA,
                    Schema.OPTIONAL_INT32_SCHEMA, Schema.OPTIONAL_INT64_SCHEMA, Schema.OPTIONAL_FLOAT32_SCHEMA,
                    Schema.OPTIONAL_FLOAT64_SCHEMA, Schema.INT16_SCHEMA, Schema.INT64_SCHEMA,
            ...
            assertTableSchema("public.numeric_decimal_table", "d, dzs, dvs, n, nzs, nvs",
                    ...
                    Decimal.builder(0).parameter(TestHelper.PRECISION_PARAMETER_KEY, "4").optional().build(),
                    VariableScaleDecimal.builder().optional().build());
            assertTableSchema("public.string_table", "vc, vcv, ch, c, t, ct", Schema.OPTIONAL_STRING_SCHEMA,
            ...
            assertTableSchema("public.bitbin_table", "ba, bol, bs, bv", Schema.OPTIONAL_BYTES_SCHEMA,
                    Schema.OPTIONAL_BOOLEAN_SCHEMA, Bits.builder(2).optional().build(),
            ...
            assertTableSchema("public.time_table", "ts, tz, date, ti, ttz, it",
                    ...
                    Date.builder().optional().build(), MicroTime.builder().optional().build(),
                    ZonedTime.builder().optional().build(), MicroDuration.builder().optional().build());
            assertTableSchema("public.text_table", "j, jb, x, u", Json.builder().optional().build(),
                    Json.builder().optional().build(), Xml.builder().optional().build(),
    ...
    public void shouldLoadSchemaForPostgisTypes() throws Exception {
        ...
        try (PostgresConnection connection = TestHelper.create()) {
            ...
            assertTableSchema("public.postgis_table", "p, ml", Geometry.builder().optional().build(),
                    Geography.builder().optional().build());
        }
    }

    @Test
    public void shouldApplyFilters() throws Exception {
        String statements = "CREATE SCHEMA s1; " + "CREATE SCHEMA s2; " + "DROP TABLE IF EXISTS s1.A;"
                + "DROP TABLE IF EXISTS s1.B;" + "DROP TABLE IF EXISTS s2.A;" + "DROP TABLE IF EXISTS s2.B;"
        ...
        PostgresConnectorConfig config = new PostgresConnectorConfig(
                TestHelper.defaultConfig().with(SCHEMA_BLACKLIST, "s1").build());
        ...
        config = new PostgresConnectorConfig(
                TestHelper.defaultConfig().with(PostgresConnectorConfig.TABLE_BLACKLIST, "s1.A,s2.A").build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (PostgresConnection connection = TestHelper.create()) {
            schema.refresh(connection, false);
        ...
        config = new PostgresConnectorConfig(TestHelper.defaultConfig().with(SCHEMA_BLACKLIST, "s2")
                .with(PostgresConnectorConfig.TABLE_BLACKLIST, "s1.A").build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (PostgresConnection connection = TestHelper.create()) {
            schema.refresh(connection, false);
        ...
        config = new PostgresConnectorConfig(
                TestHelper.defaultConfig().with(PostgresConnectorConfig.COLUMN_BLACKLIST, ".*aa").build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (PostgresConnection connection = TestHelper.create()) {
            schema.refresh(connection, false);
    ...
    public void shouldDetectNewChangesAfterRefreshing() throws Exception {
        String statements = "CREATE SCHEMA IF NOT EXISTS public;" + "DROP TABLE IF EXISTS table1;"
        ...
        statements = "DROP TABLE IF EXISTS table1;" + "DROP TABLE IF EXISTS table2;"
        ...
        statements = "ALTER TABLE table2 ADD COLUMN vc VARCHAR(2);" + "ALTER TABLE table2 ADD COLUMN si SMALLINT;"
    ...
    public void shouldPopulateToastableColumnsCache() throws Exception {
        ...
        try (PostgresConnection connection = TestHelper.create()) {
            ...
            // After refreshing w/ toastable column refresh enabled, we should have only the 'toasted' column in the
            // cache
