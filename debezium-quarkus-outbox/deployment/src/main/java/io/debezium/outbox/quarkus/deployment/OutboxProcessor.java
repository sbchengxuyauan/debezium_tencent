                        ...
                        if (parameterizedTypesDetected) {
                            if (!pTypeAggregateType.equals(aggregateIdType)) {
                                throw new IllegalStateException(String.format(
                                        ...
                                        classInfo.name(), aggregateIdType.name(), pTypeAggregateType.name()));
                            }
                            if (!pTypePayloadType.equals(payloadType)) {
                                throw new IllegalStateException(String.format(
                                        ...
                                        classInfo.name(), payloadType.name(), pTypePayloadType.name()));
                            }
                        } else {
    ...
    private void generateHbmMapping(OutboxEventEntityBuildItem outboxBuildItem,
        ...
        try {
            try (ByteArrayOutputStream os = new ByteArrayOutputStream()) {
                final JaxbHbmHibernateMapping jaxbMapping = OutboxEventHbmWriter.write(debeziumOutboxConfig,
                        outboxBuildItem);
                ...
                generatedResourcesProducer
                        .produce(new GeneratedResourceBuildItem(OUTBOX_ENTITY_HBMXML, os.toByteArray()));
            }
