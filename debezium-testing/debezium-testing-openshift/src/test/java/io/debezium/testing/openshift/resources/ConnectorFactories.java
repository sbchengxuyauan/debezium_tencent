...
public class ConnectorFactories {
    ...
    public ConnectorConfigBuilder mysql() {
        ...
        String dbHost = DATABASE_MYSQL_HOST
                .orElse("mysql." + ConfigProperties.OCP_PROJECT_MYSQL + ".svc.cluster.local");
        return cb.put("connector.class", "io.debezium.connector.mysql.MySqlConnector").put("task.max", 1)
                .put("database.hostname", dbHost).put("database.port", ConfigProperties.DATABASE_MYSQL_PORT)
                ...
                .put("database.history.kafka.bootstrap.servers",
                        "debezium-kafka-cluster-kafka-bootstrap." + ConfigProperties.OCP_PROJECT_DBZ
    ...
    public ConnectorConfigBuilder postgresql() {
        ConnectorConfigBuilder cb = new ConnectorConfigBuilder();
        String dbHost = DATABASE_POSTGRESQL_HOST
                .orElse("postgresql." + ConfigProperties.OCP_PROJECT_POSTGRESQL + ".svc.cluster.local");
        return cb.put("connector.class", "io.debezium.connector.postgresql.PostgresConnector").put("task.max", 1)
                .put("database.hostname", dbHost).put("database.port", ConfigProperties.DATABASE_POSTGRESQL_PORT)
