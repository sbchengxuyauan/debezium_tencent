...
public class PostgreSqlConnectorIT extends ConnectorTestBase {
    ...
    public static void setupDatabase() throws IOException, InterruptedException {
        if (!ConfigProperties.DATABASE_MYSQL_HOST.isPresent()) {
            dbDeployer = new PostgreSqlDeployer(ocp).withProject(ConfigProperties.OCP_PROJECT_POSTGRESQL)
                    .withDeployment(DB_DEPLOYMENT_PATH).withServices(DB_SERVICE_PATH_LB, DB_SERVICE_PATH);
            dbDeployer.deploy();
        }
        ...
        ConnectorConfigBuilder connectorConfig = connectorFactories.postgresql()
                .put("database.server.name", connectorName).put("slot.name", "debezium_" + id)
    ...
    public void shouldCreateKafkaTopics() {
        assertTopicsExist(connectorName + ".inventory.customers", connectorName + ".inventory.orders",
                connectorName + ".inventory.products", connectorName + ".inventory.products_on_hand");
    }
