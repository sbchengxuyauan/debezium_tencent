...
public class ConnectorConfigBuilder {
    ...
    public KafkaConnector getCustomResource() {
        ...
        return connectorBuilder.withNewSpec().withClassName((String) crConfig.remove("connector.class"))
                .withTasksMax((Integer) crConfig.remove("task.max")).withConfig(crConfig).endSpec().build();
    }
}
