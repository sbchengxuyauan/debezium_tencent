...
public class KafkaConnectController {
    ...
    public KafkaConnectController(KafkaConnect kafkaConnect, OpenShiftClient ocp, OkHttpClient http,
    ...
    public NetworkPolicy allowServiceAccess() {
        LOGGER.info("Creating NetworkPolicy allowing public access to " + kafkaConnect.getMetadata().getName()
                + "'s services");
        ...
        List<NetworkPolicyPort> ports = Stream.of(8083, 8404).map(IntOrString::new)
                .map(p -> new NetworkPolicyPortBuilder().withProtocol("TCP").withPort(p).build())
                .collect(Collectors.toList());

        NetworkPolicy policy = ocpUtils.createNetworkPolicy(project, kafkaConnect.getMetadata().getName() + "-allowed",
                labels, ports);
        return policy;
    }
    ...
    public Route exposeMetrics() {
        ...
        metricsRoute = ocpUtils.createRoute(project, name, nameSvc, "prometheus", service.getMetadata().getLabels());
    ...
    private void deployConnectorJson(String name, ConnectorConfigBuilder config) throws IOException {
        ...
        Request r = new Request.Builder().url(url)
                .put(RequestBody.create(config.getJsonString(), MediaType.parse("application/json"))).build();
     ...
     *             when deployment doesn't use custom resources
     */
     ...
     *            name of the connect
     * @param metricName
     *            name of the metric used to determine the state
     * @throws IOException
     *             on metric request error
     */
    public void waitForSnapshot(String connectorName, String metricName) throws IOException {
        ...
        await().atMost(5, TimeUnit.MINUTES).pollInterval(10, TimeUnit.SECONDS)
    ...
    public HttpUrl getApiURL() {
        return new HttpUrl.Builder().scheme("http").host(apiRoute.getSpec().getHost()).build();
    }
    ...
    public HttpUrl getMetricsURL() {
        return new HttpUrl.Builder().scheme("http").host(metricsRoute.getSpec().getHost()).build();
    }
