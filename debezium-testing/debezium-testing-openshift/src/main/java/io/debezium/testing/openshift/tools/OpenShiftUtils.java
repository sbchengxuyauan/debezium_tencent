...
public class OpenShiftUtils {
     ...
     * 
     * @param project
     *            project where this route will be created
     * @param name
     *            name of the route
     * @param service
     *            target service
     ...
     *            target port
     * @param labels
     *            labels set to set on this route
    ...
    public Route createRoute(String project, String name, String service, String port, Map<String, String> labels) {
        Route route = client.routes().inNamespace(project).createOrReplaceWithNew().withNewMetadata().withName(name)
                .withLabels(labels).endMetadata().withNewSpec().withNewTo().withKind("Service").withName(service)
                .endTo().withNewPort().withNewTargetPort(port).endPort().endSpec().done();
        return route;
    }
     ...
     * 
     * @param project
     *            project where this network policy will be created
     * @param name
     *            name of the policy
     * @param podSelectorLabels
     *            labels used as pod selectors
     * @param ports
     *            ports for which access will be allowed
     * @return {@link NetworkPolicy} object for created policy
     */
    public NetworkPolicy createNetworkPolicy(String project, String name, Map<String, String> podSelectorLabels,
            List<NetworkPolicyPort> ports) {
        NetworkPolicy policy = client.network().networkPolicies().inNamespace(project).createOrReplaceWithNew()
                .withNewMetadata().withName(name).endMetadata().withNewSpec().withNewPodSelector()
                .withMatchLabels(podSelectorLabels).endPodSelector().addNewIngress()
                .addToPorts(ports.toArray(new NetworkPolicyPort[ports.size()])).endIngress().withPolicyTypes("Ingress")
                .endSpec().done();
     ...
     * 
     * @param project
     *            project where this operation happens
     * @param sa
    ...
    public ServiceAccount linkPullSecret(String project, String sa, String secret) {
        ...
        if (!linked) {
            return client.serviceAccounts().inNamespace(project).withName(sa).edit().addNewImagePullSecret()
                    .withName(secret).endImagePullSecret().addNewSecret().withName(secret).endSecret().done();
        }
     ...
     * 
     * @param project
     *            project where to look for pods
     * @param labels
        ...
        for (Pod p : pods.getItems()) {
            try {
                ...
                throw new IllegalStateException(
                        "Error when waiting for pod " + p.getMetadata().getName() + " to get ready", e);
            }
        }
    }
}
