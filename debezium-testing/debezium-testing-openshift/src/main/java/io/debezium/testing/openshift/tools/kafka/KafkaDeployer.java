...
public class KafkaDeployer {
     ...
     * 
     * @param yamlPath
     *            path to CR descriptor (must be available on class path)
     ...
     * @param yamlPath
     *            path to CR descriptor (must be available on class path)
     * @param useConnectorResources
     *            true if connector deployment should be managed by operator
     * @return {@link KafkaController} instance for deployed cluster
     */
    public KafkaConnectController deployKafkaConnectCluster(String yamlPath, String loggingYamlPath,
            boolean useConnectorResources) throws InterruptedException {
        ...
        if (useConnectorResources) {
            kafkaConnect = new KafkaConnectBuilder(kafkaConnect).editMetadata()
                    .addToAnnotations("strimzi.io/use-connector-resources", "true").endMetadata().build();
        }
    ...
    private <T extends Status> boolean waitForReadyStatus(HasStatus<T> kc) {
        return kc.getStatus() != null && kc.getStatus().getConditions().stream()
