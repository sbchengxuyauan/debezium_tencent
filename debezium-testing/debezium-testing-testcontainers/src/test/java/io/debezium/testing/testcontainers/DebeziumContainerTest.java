...
public class DebeziumContainerTest {
    ...
    public static DebeziumContainer debeziumContainer = new DebeziumContainer("1.1.0.Beta1").withNetwork(network)
            .withKafka(kafkaContainer).withLogConsumer(new Slf4jLogConsumer(LOGGER)).dependsOn(kafkaContainer);
    ...
    public void shouldRegisterPostgreSQLConnector() throws Exception {
        try (Connection connection = getConnection(postgresContainer);
            ...
            assertThat(JsonPath.<String> read(changeEvents.get(0).value(), "$.before.title"))
    ...
    private ConnectorConfiguration getConfiguration(int id) {
        ...
        return ConnectorConfiguration.forJdbcContainer(postgresContainer).with("database.server.name", "dbserver" + id)
