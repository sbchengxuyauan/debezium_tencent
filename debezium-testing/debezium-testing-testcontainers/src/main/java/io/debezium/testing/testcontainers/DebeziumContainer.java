...
public class DebeziumContainer extends GenericContainer<DebeziumContainer> {
    ...
    public void registerConnector(String name, ConnectorConfiguration configuration) throws IOException {
        ...
        // This is just a guard, probably in most of use cases you won't need that as preparation time of the test might
        // be enough to configure connector.
        Awaitility.await().atMost(10, TimeUnit.SECONDS).until(() -> isConnectorConfigured(connector.getName()));
    }
    ...
    private boolean isConnectorConfigured(String connectorName) throws IOException {
        final Request request = new Request.Builder().url(getConnector(connectorName)).build();
