...
public final class EmbeddedEngine implements DebeziumEngine<SourceRecord> {
    ...
    public static final Field ENGINE_NAME = Field.create("name")
            .withDescription("Unique name for this connector instance.").withValidation(Field::isRequired);
    ...
    public static final Field CONNECTOR_CLASS = Field.create("connector.class")
            .withDescription("The Java class for the connector").withValidation(Field::isRequired);
    ...
    public static final Field OFFSET_STORAGE_FILE_FILENAME = Field
            ...
            .withDescription("The file where offsets are to be stored. Required when "
                    + "'offset.storage' is set to the " + FileOffsetBackingStore.class.getName() + " class.")
            .withDefault("");
    ...
    public static final Field OFFSET_STORAGE_KAFKA_PARTITIONS = Field
            .create(DistributedConfig.OFFSET_STORAGE_PARTITIONS_CONFIG).withType(ConfigDef.Type.INT)
    ...
    public static final Field OFFSET_STORAGE_KAFKA_REPLICATION_FACTOR = Field
            .create(DistributedConfig.OFFSET_STORAGE_REPLICATION_FACTOR_CONFIG).withType(ConfigDef.Type.SHORT)
    ...
    public static final Field OFFSET_FLUSH_INTERVAL_MS = Field.create("offset.flush.interval.ms")
            ...
            .withDefault(60000L).withValidation(Field::isNonNegativeInteger);
    ...
    public static final Field OFFSET_COMMIT_TIMEOUT_MS = Field.create("offset.flush.timeout.ms")
            ...
            .withDefault(5000L).withValidation(Field::isPositiveInteger);
    ...
    protected static final Field.Set ALL_FIELDS = CONNECTOR_FIELDS.with(OFFSET_STORAGE, OFFSET_STORAGE_FILE_FILENAME,
            OFFSET_FLUSH_INTERVAL_MS, OFFSET_COMMIT_TIMEOUT_MS, INTERNAL_KEY_CONVERTER_CLASS,
    ...
    public static final class BuilderImpl implements Builder {
        ...
        public EmbeddedEngine build() {
            ...
            return new EmbeddedEngine(config, classLoader, clock, handler, completionCallback, connectorCallback,
                    offsetCommitPolicy);
        }
    ...
    private EmbeddedEngine(Configuration config, ClassLoader classLoader, Clock clock,
            DebeziumEngine.ChangeConsumer<SourceRecord> handler, DebeziumEngine.CompletionCallback completionCallback,
            DebeziumEngine.ConnectorCallback connectorCallback, OffsetCommitPolicy offsetCommitPolicy) {
        this.config = config;
        this.handler = handler;
        this.classLoader = classLoader;
        this.clock = clock;
        ...
        if (runningThread.compareAndSet(null, Thread.currentThread())) {
            ...
            try {
                ...
                if (offsetCommitPolicy == null) {
                    offsetCommitPolicy = config.getInstance(EmbeddedEngine.OFFSET_COMMIT_POLICY,
                            OffsetCommitPolicy.class, config);
                }
                ...
                OffsetStorageWriter offsetWriter = new OffsetStorageWriter(offsetStore, engineName, keyConverter,
                        valueConverter);
                OffsetStorageReader offsetReader = new OffsetStorageReaderImpl(offsetStore, engineName, keyConverter,
                        valueConverter);
     ...
     * 
     * @param offsetWriter
     *            the offsetWriter current in use
     * @param task
     *            the sourcetask
     * @param commitTimeout
     *            the time in ms until a commit times out
     ...
     *
     * @param offsetWriter
     *            the offset storage writer; may not be null
     * @param policy
     *            the offset commit policy; may not be null
     * @param commitTimeout
     *            the timeout to wait for commit results
     * @param task
     *            the task which produced the records for which the offsets have been committed
     */
    ...
    public boolean stop() {
        ...
        if (thread != null) {
            try {
                latch.await(
                        Long.valueOf(System.getProperty(WAIT_FOR_COMPLETION_BEFORE_INTERRUPT_PROP,
     ...
     *
     * @param timeout
     *            the maximum amount of time to wait before returning
     * @param unit
     *            the unit of time; may not be null
