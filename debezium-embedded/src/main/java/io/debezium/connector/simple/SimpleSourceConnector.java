...
/**
 * A very simple {@link SourceConnector} for testing that reliably produces the same records in the same order, and
 * useful for testing the infrastructure to run {@link SourceConnector}s.
 * <p>
    ...
    public static class SimpleConnectorTask extends SourceTask {
        ...
        public void start(Map<String, String> props) {
            if (running.compareAndSet(false, true)) {
                ...
                Schema valueSchema = SchemaBuilder.struct().name("simple.value").field("batch", Schema.INT32_SCHEMA)
                        .field("record", Schema.INT32_SCHEMA).field("timestamp", Schema.OPTIONAL_INT64_SCHEMA).build();
                ...
                for (int batch = 0; batch != batchCount; ++batch) {
                    for (int recordNum = 0; recordNum != recordsPerBatch; ++recordNum) {
                        ...
                        SourceRecord record = new SourceRecord(partition, offset, topic, 1, keySchema, key, valueSchema,
                                value);
                        records.add(record);
                    }
                }
