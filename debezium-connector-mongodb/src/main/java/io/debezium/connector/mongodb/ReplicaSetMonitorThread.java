...
/**
 * A thread that can be used to when new replica sets are added or existing replica sets are removed. The logic does not
 * evaluate membership changes of individual replica sets, since that is handled independently by each task.
 *
...
public final class ReplicaSetMonitorThread implements Runnable {
    ...
    /**
     * @param monitor
     *            the component used to periodically obtain the replica set specifications; may not be null
     * @param period
     *            the time period between polling checks; must be non-negative
     * @param clock
     *            the clock to use; may be null if the system clock should be used
     * @param onStartup
     *            the function to call when the thread is started; may be null if not needed
     * @param onChange
     *            the function to call when the set of replica set specifications has changed; may be null if not needed
     */
            ...
            try {
                ...
                if (replicaSets.haveChangedSince(previousReplicaSets)) {
                    ...
                    try {
                        ...
                        logger.error("Interrupted while calling the function with the new replica set specifications",
                                t);
     ...
     *
     * @param timeout
     *            the time to block until the replica sets are first obtained from MongoDB; may not be negative
     * @param unit
     *            the time unit for the {@code timeout}; may not be null
