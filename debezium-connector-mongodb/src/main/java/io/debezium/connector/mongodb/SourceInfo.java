...
/**
 * Information about the source of information, which includes the partitions and offsets within those partitions. The
 * MongoDB connector considers each MongoDB database a separate "partitions" (in Kafka Connect terminology), and each
 * partition has an "offset" (if Kafka Connect terminology) that defines the position within that partition/database
 * where the connector finds a particular record. As the connector uses the Kafka Connect framework to process each
 * record in a partition, Kafka Connect keeps track of the most recent offset for that partition.
 * <p>
 * The {@link #partition(String) source partition} information identifies the particular MongoDB replica set and the
 * connector's logical name of the MongoDB server. A JSON-like representation of the source partition for a database
 * named "customers" hosted in a MongoDB replica set named "myMongoServer" is as follows:
 *
 * <pre>
 * {
 ...
 *
 * <p>
 * The {@link #lastOffset(String) source offset} information describes the position within a particular partition of
 * each record. Since each event in MongoDB's oplog is identified by a {@link BSONTimestamp} that tracks the time and
 * the order of the event for that particular time (e.g., multiple events that occur at the same time will have unique
 * orders), the offset includes the BSONTimetamp representation. (The event's {@code h} field is the unique ID for the
 * operation, so this is also included in the offset.) And, if an initial sync is in progress, the offset will include
 * the {@code initsync} field.
 * <p>
 * Here's a JSON-like representation of an example timestamp:
 *
 * <pre>
 * { "sec" = 1422998530, "ord" = 0, "h" = 398278732173914406, "initsync" = true }
 * </pre>
 *
...
public final class SourceInfo extends BaseSourceInfo {
    ...
    /**
     * Id of collection the current event applies to. May be {@code null} after noop events, after which the recorded
     * offset may be retrieved but not the source struct.
     */
     ...
     * @param partition
     *            the partition map
     * @return the replica set name (when the partition is valid), or {@code null} if the partition is null or has no
     *         replica set name entry
     */
    ...
    /**
     * Get the Kafka Connect detail about the source "partition" for the given database in the replica set. If the
     * database is not known, this method records the new partition.
     *
     * @param replicaSetName
     *            the name of the replica set name for which the partition is to be obtained; may not be null
     ...
     *
     * @param replicaSetName
     *            the name of the replica set name for which the new offset is to be obtained; may not be null
    ...
    /**
     * Get the Kafka Connect detail about the source "offset" for the named database, which describes the given position
     * in the database where we have last read. If the database has not yet been seen, this records the starting
     * position for that database. However, if there is a position for the database, the offset representation is
     * returned.
     *
     * @param replicaSetName
     *            the name of the replica set name for which the new offset is to be obtained; may not be null
    ...
    public Map<String, ?> lastOffset(String replicaSetName) {
        ...
        if (isInitialSyncOngoing(replicaSetName)) {
            return Collect.hashMapOf(TIMESTAMP, Integer.valueOf(existing.getTime()), ORDER,
                    Integer.valueOf(existing.getInc()), OPERATION_ID, existing.getOperationId(), INITIAL_SYNC, true);
        }
        Map<String, Object> offset = Collect.hashMapOf(TIMESTAMP, Integer.valueOf(existing.getTime()), ORDER,
                Integer.valueOf(existing.getInc()), OPERATION_ID, existing.getOperationId());
     ...
     * {@link #lastOffset(String) offset} information where we have last read. The Struct complies with the
     * {@link #schema} for the MongoDB connector.
     *
     * @param replicaSetName
     *            the name of the replica set name for which the new offset is to be obtained; may not be null
     * @param collectionId
     *            the event's collection identifier; may not be null
    ...
    /**
     * Get a {@link Struct} representation of the source {@link #partition(String) partition} and
     * {@link #lastOffset(String) offset} information. The Struct complies with the {@link #schema} for the MongoDB
     * connector.
     *
     * @param replicaSetName
     *            the name of the replica set name for which the new offset is to be obtained; may not be null
     * @param oplogEvent
     *            the replica set oplog event that was last read; may be null if the position is the start of the oplog
     * @param masterEvent
     *            the replica set oplog event that contains event metadata; same as oplogEvent for non-transactional
     *            changes
     * @param orderInTx
     *            order in transaction batch, 0 for non-transactional events
     * @see #schema()
     */
     ...
     *
     * @param replicaSetName
     *            the name of the replica set name; may not be null
     * @return {@code true} if an offset has been recorded for the replica set, or {@code false} if the replica set has
     *         not yet been seen
     */
    ...
    /**
     * Set the source offset, as read from Kafka Connect, for the given replica set. This method does nothing if the
     * supplied map is null.
     *
     * @param replicaSetName
     *            the name of the replica set name for which the new offset is to be obtained; may not be null
     * @param sourceOffset
     *            the previously-recorded Kafka Connect source offset; may be null
     ...
     *
     * @param partition
     *            the partition information; may not be null
     * @param sourceOffset
     *            the previously-recorded Kafka Connect source offset; may be null
     * @return {@code true} if the offset was recorded, or {@code false} if the source offset is null
     * @throws ConnectException
     ...
     *
     * @param replicaSetName
     *            the name of the replica set; never null
     */
     ...
     * @param replicaSetName
     *            the name of the replica set; never null
     * @return {@code true} if the initial sync for this replica is still ongoing or was not completed before
     *         restarting, or {@code false} if there is currently no initial sync operation for this replica set
     */
    ...
    private static int intOffsetValue(Map<String, ?> values, String key) {
        ...
        try {
            ...
            throw new ConnectException(
                    "Source offset '" + key + "' parameter value " + obj + " could not be converted to an integer");
        }
    }
