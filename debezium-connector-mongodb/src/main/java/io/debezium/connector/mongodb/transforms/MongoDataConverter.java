...
/**
 * MongoDataConverter handles translating MongoDB strings to Kafka Connect schemas and row data to Kafka Connect
 * records.
 *
        ...
        case ARRAY:
            if (keyValuesforSchema.getValue().asArray().isEmpty()) {
                ...
                case ARRAY:
                    ...
                    for (BsonValue element : keyValuesforSchema.getValue().asArray()) {
                        if (element.getBsonType() != valueType) {
                            throw new ConnectException(
                                    "Field " + key + " of schema " + builder.name() + " is not a homogenous array.\n"
                    ...
                    case DOCUMENT:
                        final SchemaBuilder documentSchemaBuilder = SchemaBuilder.struct()
                                .name(builder.name() + "." + key).optional();
                        ...
                        for (BsonValue element : keyValuesforSchema.getValue().asArray()) {
                            ...
                            for (Entry<String, BsonValue> arrayDoc : arrayDocs.entrySet()) {
                                final BsonType prevType = union.putIfAbsent(arrayDoc.getKey(),
                                        arrayDoc.getValue().getBsonType());
                                if (prevType == null) {
                                    addFieldSchema(arrayDoc, documentSchemaBuilder);
                                } else if (prevType != arrayDoc.getValue().getBsonType()) {
                                    throw new ConnectException(
                                            "Field " + arrayDoc.getKey() + " of schema " + documentSchemaBuilder.name()
                ...
                case DOCUMENT:
                    ...
                    final SchemaBuilder arrayStructBuilder = SchemaBuilder.struct().name(builder.name() + "." + key)
                            .optional();
                    final Map<String, BsonValue> convertedArray = new HashMap<>();
                    for (int i = 0; i < array.size(); i++) {
                        convertedArray.put(arrayElementStructName(i), array.get(i));
