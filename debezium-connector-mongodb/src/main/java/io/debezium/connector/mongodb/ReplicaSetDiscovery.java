...
/**
 * A component that monitors a single replica set or the set of replica sets that make up the shards in a sharded
 * cluster.
 *
...
public class ReplicaSetDiscovery {
     ...
     *
     * @param context
     *            the replication context; may not be null
     */
    ...
    /**
     * Connect to the shard cluster or replica set defined by the seed addresses, and obtain the specifications for each
     * of the replica sets.
     *
    ...
    public ReplicaSets getReplicaSets() {
        ...
        try {
            ...
            logger.error("Interrupted while reading the '{}' collection in the '{}' database: {}", shardsCollection,
        ...
        if (replicaSetSpecs.isEmpty()) {
            ...
            if (rsStatus != null) {
                ...
                // that we're not connected to a config server replica set, so any replica set name from the seed
                // addresses
