 ...
 * <p>
 * Generally, the {@link MongoDbConnector} assigns each replica set to a separate task, although multiple replica sets
 * will be assigned to each task when the maximum number of tasks is limited. Regardless, every task will use a separate
 * thread to replicate the contents of each replica set, and each replication thread may use multiple threads to perform
 * an initial sync of the replica set.
 *
...
public final class MongoDbConnectorTask extends BaseSourceTask {
    ...
    public ChangeEventSourceCoordinator start(Configuration config) {
        ...
        try {
            ...
            final EventDispatcher<CollectionId> dispatcher = new EventDispatcher<>(connectorConfig,
                    taskContext.topicSelector(), schema, queue, taskContext.filters().collectionFilter()::test,
                    DataChangeEvent::new, metadataProvider);

            ChangeEventSourceCoordinator coordinator = new ChangeEventSourceCoordinator(previousOffsets, errorHandler,
                    MongoDbConnector.class, connectorConfig, new MongoDbChangeEventSourceFactory(connectorConfig,
                            errorHandler, dispatcher, clock, replicaSets, taskContext),
                    dispatcher, schema);
    ...
    private MongoDbOffsetContext getPreviousOffsets(MongoDbConnectorConfig connectorConfig, ReplicaSets replicaSets) {
        ...
        if (offsets != null
                && !offsets.values().stream().filter(Objects::nonNull).collect(Collectors.toList()).isEmpty()) {
