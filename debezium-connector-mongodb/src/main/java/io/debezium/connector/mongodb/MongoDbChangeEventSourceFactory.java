...
public class MongoDbChangeEventSourceFactory implements ChangeEventSourceFactory {
    ...
    public MongoDbChangeEventSourceFactory(MongoDbConnectorConfig configuration, ErrorHandler errorHandler,
            EventDispatcher<CollectionId> dispatcher, Clock clock, ReplicaSets replicaSets,
    ...
    public SnapshotChangeEventSource getSnapshotChangeEventSource(OffsetContext offsetContext,
        ...
        return new MongoDbSnapshotChangeEventSource(configuration, taskContext, replicaSets,
                (MongoDbOffsetContext) offsetContext, dispatcher, clock, snapshotProgressListener, errorHandler);
    }

    @Override
    public StreamingChangeEventSource getStreamingChangeEventSource(OffsetContext offsetContext) {
        return new MongoDbStreamingChangeEventSource(configuration, taskContext, replicaSets,
                (MongoDbOffsetContext) offsetContext, dispatcher, errorHandler, clock);
    }
}
