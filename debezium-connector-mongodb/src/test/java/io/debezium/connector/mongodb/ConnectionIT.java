...
public class ConnectionIT extends AbstractMongoIT {
    ...
    public void shouldCreateMovieDatabase() {
        useConfiguration(config.edit().with(MongoDbConnectorConfig.DATABASE_WHITELIST, "dbA,dbB")
                .with(MongoDbConnectorConfig.COLLECTION_BLACKLIST, "dbB.moviesB").build());
        ...
        primary.execute("read oplog from beginning", mongo -> {
            ...
            FindIterable<Document> results = mongo.getDatabase("local").getCollection("oplog.rs").find(filter)
                    .sort(new Document("$natural", 1)).oplogReplay(true) // tells Mongo to not rely on indexes
