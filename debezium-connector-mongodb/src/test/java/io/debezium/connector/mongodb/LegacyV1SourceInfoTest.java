...
public class LegacyV1SourceInfoTest {
    ...
    public void beforeEach() {
        source = new SourceInfo(
                new MongoDbConnectorConfig(Configuration.create().with(MongoDbConnectorConfig.LOGICAL_NAME, "serverX")
                        .with(MongoDbConnectorConfig.SOURCE_STRUCT_MAKER_VERSION, Version.V1).build()));
    }
    ...
    public void shouldHaveSchemaForSource() {
        ...
        assertThat(schema.field(SourceInfo.INITIAL_SYNC).schema())
                .isEqualTo(SchemaBuilder.bool().optional().defaultValue(false).build());
    }
    ...
    public void shouldSetAndReturnRecordedOffset() {
        Document event = new Document().append("ts", new BsonTimestamp(100, 2)).append("h", Long.valueOf(1987654321))
    ...
    public void versionIsPresent() {
        final Document event = new Document().append("ts", new BsonTimestamp(100, 2))
                .append("h", Long.valueOf(1987654321)).append("ns", "dbA.collectA");
    ...
    public void schemaIsCorrect() {
        final Schema schema = SchemaBuilder.struct().name("io.debezium.connector.mongo.Source").version(1)
                .field("version", Schema.OPTIONAL_STRING_SCHEMA).field("connector", Schema.OPTIONAL_STRING_SCHEMA)
                .field("name", Schema.STRING_SCHEMA).field("rs", Schema.STRING_SCHEMA).field("ns", Schema.STRING_SCHEMA)
                .field("sec", Schema.INT32_SCHEMA).field("ord", Schema.INT32_SCHEMA)
                .field("h", Schema.OPTIONAL_INT64_SCHEMA)
                .field("initsync", SchemaBuilder.bool().optional().defaultValue(false).build()).build();
