...
public abstract class AbstractMongoIT implements Testing {
    ...
    /**
     * A method that will initialize the state after the configuration is changed, reusing the same partition offsets
     ...
     *            {@code true} if the context should have no prior partition offsets, or {@code false} if the partition
     *            offsets that exist at this time should be reused
     */
    private void initialize(boolean restartFromBeginning) {
        ...
        if (!restartFromBeginning && context != null && replicaSet != null
                && context.source().hasOffset(replicaSet.replicaSetName())) {
