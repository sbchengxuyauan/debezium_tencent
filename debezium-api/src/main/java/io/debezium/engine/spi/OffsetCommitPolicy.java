...
public interface OffsetCommitPolicy {
    ...
    public static class PeriodicCommitOffsetPolicy implements OffsetCommitPolicy {
        ...
        public PeriodicCommitOffsetPolicy(Properties config) {
            minimumTime = Duration
                    .ofMillis(Long.valueOf(config.getProperty(DebeziumEngine.OFFSET_FLUSH_INTERVAL_MS_PROP)));
        }
     ...
     *
     * @param numberOfMessagesSinceLastCommit
     *            the number of messages that have been received from the connector since last the offsets were last
     *            committed; never negative
     * @param timeSinceLastCommit
     *            the time that has elapsed since the offsets were last committed; never negative
     ...
     *
     * @param other
     *            the other commit policy; if null, then this policy instance is returned as is
