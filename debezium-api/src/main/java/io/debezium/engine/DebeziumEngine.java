...
/**
 * A mechanism for running a single Kafka Connect {@link SourceConnector} within an application's process. The engine is
 * entirely standalone and only talks with the source system; no Kafka, Kafka Connect, or Zookeeper processes are
 * needed. Applications using the engine simply set one up and supply a {@link Consumer consumer function} to which the
...
public interface DebeziumEngine<R> extends Runnable, Closeable {
    ...
    public interface CompletionCallback {
         ...
         *
         * @param success
         *            {@code true} if the connector completed normally, or {@code false} if the connector produced an
         *            error that prevented startup or premature termination.
         * @param message
    ...
    public interface ConnectorCallback {
        ...
        /**
         * Called after a connector has been successfully started by the engine; i.e. {@link SourceConnector#start(Map)}
        ...
        /**
         * Called after a connector task has been successfully started by the engine; i.e. {@link SourceTask#start(Map)}
         * has completed successfully
         */
    ...
    public static interface ChangeConsumer<R> {
        ...
        /**
         * Handles a batch of records, calling the {@link RecordCommitter#markProcessed(SourceRecord)} for each record
         * and {@link RecordCommitter#markBatchFinished()} when this batch is finished.
         * 
         * @param records
    ...
    public static interface Builder<R> {
        ...
        /**
         * Pass a custom ChangeConsumer override the default implementation, this allows for more complex handling of
         * records for batch and async handling
         *
         * @param handler
         *            the consumer function
         * @return this builder object so methods can be chained together; never null
        ...
        /**
         * Use the specified class loader to find all necessary classes. Passing <code>null</code> or not calling this
         * method results in the connector using this class's class loader.
         *
         * @param classLoader
         *            the class loader
         * @return this builder object so methods can be chained together; never null
         ...
         *            the callback function; may be null if errors should be written to the log
         * @return this builder object so methods can be chained together; never null
         */
        ...
        /**
         * During the engine's {@link DebeziumEngine#run()} method, decide when the offsets should be committed into the
         * {@link OffsetBackingStore}.
         * 
         * @param policy
         * @return this builder object so methods can be chained together; never null
         */
         ...
         *             if a {@link #using(Properties) configuration} or {@link #notifying(Consumer) consumer function}
    ...
    public static <T> Builder<T> create(Class<? extends ChangeEventFormat<T>> eventFormat) {
        ...
        if (iterator.hasNext()) {
            LoggerFactory.getLogger(Builder.class).warn(
                    "More than one Debezium engine builder implementation was found, using {}", builder.getClass());
        }
        return builder;
    }
}
