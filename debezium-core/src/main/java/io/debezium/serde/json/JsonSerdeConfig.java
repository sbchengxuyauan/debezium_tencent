...
public class JsonSerdeConfig extends AbstractConfig {
    ...
    public static final Field FROM_FIELD = Field.create("from.field")
            .withDisplayName("What Envelope field should be deserialized (before/after)").withType(Type.STRING)
            .withWidth(Width.SHORT).withImportance(Importance.MEDIUM)
            .withDescription(
                    "Enables user to choose which of Envelope provided fields should be deserialized as the payload."
    ...
    public static final Field UNKNOWN_PROPERTIES_IGNORED = Field.create("unknown.properties.ignored")
            .withDisplayName("Unknown properties ignored").withType(Type.BOOLEAN).withWidth(Width.SHORT)
            .withImportance(Importance.LOW)
            .withDescription(
                    "Controls whether unknown properties will be ignored or cause a JsonMappingException when encountered.")
            .withDefault(false);
