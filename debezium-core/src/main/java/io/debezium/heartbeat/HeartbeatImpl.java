...
class HeartbeatImpl implements Heartbeat {
    ...
    /**
     * Default length of interval in which connector generates periodically heartbeat messages. A size of 0 disables
     * heartbeat.
     */
    ...
    private static Schema KEY_SCHEMA = SchemaBuilder.struct()
            ...
            .field(SERVER_NAME_KEY, Schema.STRING_SCHEMA).build();
    ...
    private SourceRecord heartbeatRecord(Map<String, ?> sourcePartition, Map<String, ?> sourceOffset) {
        ...
        return new SourceRecord(sourcePartition, sourceOffset, topicName, partition, KEY_SCHEMA, serverNameKey(key),
                VALUE_SCHEMA, messageValue());
    }
