...
public interface Heartbeat {
    ...
    /**
     * Returns the offset to be used when emitting a heartbeat event. This supplier interface allows for a lazy creation
     * of the offset only when a heartbeat actually is sent, in cases where it's determination is costly.
     */
    @FunctionalInterface
    ...
    public static final Field HEARTBEAT_INTERVAL = Field.create(HEARTBEAT_INTERVAL_PROPERTY_NAME)
            .withDisplayName("Connector heartbeat interval (milli-seconds)").withType(Type.INT).withWidth(Width.MEDIUM)
            .withImportance(Importance.MEDIUM)
            .withDescription(
                    "Length of an interval in milli-seconds in in which the connector periodically sends heartbeat messages "
                            + "to a heartbeat topic. " + "Use 0 to disable heartbeat messages. "
                            + "Disabled by default.")
            .withDefault(HeartbeatImpl.DEFAULT_HEARTBEAT_INTERVAL).withValidation(Field::isNonNegativeInteger);

    public static final Field HEARTBEAT_TOPICS_PREFIX = Field.create("heartbeat.topics.prefix")
            .withDisplayName("A prefix used for naming of heartbeat topics").withType(Type.STRING)
            .withWidth(Width.MEDIUM).withImportance(Importance.LOW)
            .withDescription("The prefix that is used to name heartbeat topics." + "Defaults to "
                    + HeartbeatImpl.DEFAULT_HEARTBEAT_TOPICS_PREFIX + ".")
    ...
    public static final Heartbeat NULL = new Heartbeat() {
        ...
        @Override
        public void heartbeat(Map<String, ?> partition, Map<String, ?> offset, BlockingConsumer<SourceRecord> consumer)
     ...
     *
     * @param partition
     *            partition for the heartbeat record
     * @param offset
     *            offset for the heartbeat record
     * @param consumer
     *            - a code to place record among others to be sent into Connect
     */
    // TODO would be nice to pass OffsetContext here; not doing it for now, though, until MySQL is using OffsetContext,
    // too
    void heartbeat(Map<String, ?> partition, Map<String, ?> offset, BlockingConsumer<SourceRecord> consumer)
            throws InterruptedException;
     ...
     * @param partition
     *            partition for the heartbeat record
     * @param offsetProducer
     *            lazily calculated offset for the heartbeat record
     * @param consumer
     *            - a code to place record among others to be sent into Connect
     */
    void heartbeat(Map<String, ?> partition, OffsetProducer offsetProducer, BlockingConsumer<SourceRecord> consumer)
            throws InterruptedException;
    ...
    // TODO would be nice to pass OffsetContext here; not doing it for now, though, until MySQL is using OffsetContext,
    // too
    void forcedBeat(Map<String, ?> partition, Map<String, ?> offset, BlockingConsumer<SourceRecord> consumer)
            throws InterruptedException;
     ...
     * @param topicName
     *            topic to which the heartbeat messages will be sent
     * @param key
    ...
    public static Heartbeat create(Configuration configuration, String topicName, String key) {
        return configuration.getDuration(HeartbeatImpl.HEARTBEAT_INTERVAL, ChronoUnit.MILLIS).isZero() ? NULL
                : new HeartbeatImpl(configuration, topicName, key);
    }
