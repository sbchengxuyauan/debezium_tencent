...
public class Threads {
     ...
     * specified amount. The supplied {@link TimeSince} object will be {@link TimeSince#reset() reset} when the new
     * thread is started, and should also be {@link TimeSince#reset() reset} any time the elapsed time should be reset
     * to 0.
     *
     * @param threadName
     *            the name of the new thread; may not be null
     * @param timeout
     *            the maximum amount of time that can elapse before the thread is interrupted; must be positive
     * @param timeoutUnit
     *            the unit for {@code timeout}; may not be null
     * @param elapsedTimer
     *            the component used to measure the elapsed time; may not be null
     * 
     * @return the new thread that has not yet been {@link Thread#start() started}; never null
     */
    public static Thread interruptAfterTimeout(String threadName, long timeout, TimeUnit timeoutUnit,
     ...
     *            the thread that should be interrupted upon timeout; may not be null
     * 
     * @return the new thread that has not yet been {@link Thread#start() started}; never null
     */
    public static Thread interruptAfterTimeout(String threadName, long timeout, TimeUnit timeoutUnit,
            TimeSince elapsedTimer, Thread threadToInterrupt) {
        return timeout(threadName, timeout, timeoutUnit, 100, TimeUnit.MILLISECONDS, elapsedTimer::elapsedTime,
                elapsedTimer::reset, () -> threadToInterrupt.interrupt());
    }
     ...
     *            the component used to measure the elapsed time; may not be null
     * @param uponTimeout
     *            the function to be called when the maximum amount of time has elapsed; may not be null
     * 
     * @return the new thread that has not yet been {@link Thread#start() started}; never null
     */
    public static Thread timeout(String threadName, long timeout, TimeUnit timeoutUnit, TimeSince elapsedTimer,
            Runnable uponTimeout) {
        return timeout(threadName, timeout, timeoutUnit, 100, TimeUnit.MILLISECONDS, elapsedTimer::elapsedTime,
                elapsedTimer::reset, uponTimeout);
    }
     ...
     *            the unit for {@code timeout}; may not be null
     * @param sleepInterval
     *            the amount of time for the new thread to sleep after checking the elapsed time; must be positive
     * @param sleepUnit
     *            the unit for {@code sleepInterval}; may not be null
     * @param elapsedTimer
     *            the component used to measure the elapsed time; may not be null
     * @param uponTimeout
     *            the function to be called when the maximum amount of time has elapsed; may not be null
     ...
     * @return the new thread that has not yet been {@link Thread#start() started}; never null
     */
    public static Thread timeout(String threadName, long timeout, TimeUnit timeoutUnit, long sleepInterval,
            TimeUnit sleepUnit, TimeSince elapsedTimer, Runnable uponTimeout) {
        return timeout(threadName, timeout, timeoutUnit, sleepInterval, sleepUnit, elapsedTimer::elapsedTime,
                elapsedTimer::reset, uponTimeout);
    }
     ...
     *            the unit for {@code sleepInterval}; may not be null
     * @param elapsedTime
     *            the function that returns the total elapsed time; may not be null
     * @param uponStart
     *            the function that will be called when the returned thread is {@link Thread#start() started}; may be
     *            null
     * @param uponTimeout
     *            the function to be called when the maximum amount of time has elapsed; may not be null
     * 
     ...
     */
    public static Thread timeout(String threadName, long timeout, TimeUnit timeoutUnit, long sleepInterval,
            TimeUnit sleepUnit, LongSupplier elapsedTime, Runnable uponStart, Runnable uponTimeout) {
     ...
     * {@code debezium-<connector class>-<connector-id>-<thread-name>}.
     *
     * @param connector
     *            - the source connector class
     * @param connectorId
     *            - the identifier to differentiate between connector instances
     * @param name
     *            - the name of the thread
     * @param indexed
     *            - true if the thread name should be appended with an index
     * 
     * @return the thread factory setting the correct name
     */
    public static ThreadFactory threadFactory(Class<? extends SourceConnector> connector, String connectorId,
            String name, boolean indexed) {
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("Requested thread factory for connector {}, id = {} named = {}", connector.getSimpleName(),
                    connectorId, name);
        }
        ...
        return new ThreadFactory() {
            ...
            public Thread newThread(Runnable r) {
                StringBuilder threadName = new StringBuilder(DEBEZIUM_THREAD_NAME_PREFIX)
                        .append(connector.getSimpleName().toLowerCase()).append('-').append(connectorId).append('-')
                        .append(name);
                if (indexed) {
    ...
    public static ExecutorService newSingleThreadExecutor(Class<? extends SourceConnector> connector,
            String connectorId, String name) {
        return Executors.newSingleThreadExecutor(threadFactory(connector, connectorId, name, false));
    }

    public static ExecutorService newFixedThreadPool(Class<? extends SourceConnector> connector, String connectorId,
