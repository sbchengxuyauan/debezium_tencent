...
/**
 * A class that can be used to perform an action at a regular interval. To use, set up a {@code Metronome} instance and
 * perform a repeated event (perhaps using a loop), calling {@link #pause()} before each event.
 *
...
public interface Metronome {
     ...
     * <p>
     * Generally speaking, this is a simple but inaccurate approach for periods anywhere close to the precision of the
     * supplied Clock (which for the {@link Clock#system() system clock} is typically around 10-15 milliseconds for
     * modern Linux and OS X systems, and potentially worse on Windows and older Linux/Unix systems. And because this
     * metronome uses Thread#sleep(long), thread context switches are likely and will negatively affect the precision of
     * the metronome's period.
     * <p>
     * Although the method seemingly supports taking {@link TimeUnit#MICROSECONDS} and {@link TimeUnit#NANOSECONDS}, it
     * is likely that the JVM and operating system do not support such fine-grained precision. And as mentioned above,
     * care should be used when specifying a {@code period} of 20 milliseconds or smaller.
     *
     * @param period
     *            the period of time that the metronome ticks and for which {@link #pause()} waits
     * @param timeSystem
     *            the time system that will provide the current time; may not be null
     * 
     ...
     * Although the method seemingly supports taking {@link TimeUnit#MICROSECONDS} and {@link TimeUnit#NANOSECONDS}, it
     * is likely that the JVM and operating system do not support such fine-grained precision. And as mentioned above,
     * care should be used when specifying a {@code period} of 10-15 milliseconds or smaller.
     *
     * @param period
     *            the period of time that the metronome ticks and for which {@link #pause()} waits
     * @param timeSystem
