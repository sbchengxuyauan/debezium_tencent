 ...
 * iterator/enumeration. They do <em>not</em> throw {@link java.util.ConcurrentModificationException}. However,
 ...
 * <p/>
 * <p>
 * This class is copied from Hibernate (which took it from Infinispan), and was originally written by Doug Lea with
 * assistance from members of JCP JSR-166 Expert Group and released to the public domain, as explained at
 * http://creativecommons.org/licenses/publicdomain
 * </p>
 * <p/>
 * <p/>
...
public class BoundedConcurrentHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
    ...
    public interface EvictionPolicy<K, V> {
        ...
        /**
         * Invoked to notify EvictionPolicy implementation that there has been an attempt to access an entry in Segment,
         * however that entry was not present in Segment.
         *
         * @param e
        ...
        /**
         * Invoked to notify EvictionPolicy implementation that an entry in Segment has been accessed. Returns true if
         * batching threshold has been reached, false otherwise.
         * <p/>
    ...
    private static final class LIRSHashEntry<K, V> extends HashEntry<K, V> {
        ...
        /**
         * Records a cache miss. This is how new entries join the LIRS stack and queue. This is called both when a new
         * entry is first created, and when a non-resident entry is re-computed.
         */
        ...
        /**
         * Temporarily removes this entry from the stack, fixing up neighbor links. This entry's links remain unchanged,
         * meaning that {@link #inStack()} will continue to return true. This should only be called if this node's links
        ...
        /**
         * Temporarily removes this entry from the queue, fixing up neighbor links. This entry's links remain unchanged.
         * This should only be called if this node's links will be subsequently changed.
         */
        ...
        /**
         * Moves this entry from the stack to the queue, marking it cold (as hot entries must remain in the stack). This
         * should only be called on resident entries, as non-resident entries should not be made resident. The bottom
         * entry on the queue is always hot due to stack pruning.
         */
        ...
        /**
         * Moves this entry from the queue to the stack, marking it hot (as cold resident entries must remain in the
         * queue).
         */
        ...
        /**
         * Evicts this entry, removing it from the queue and setting its status to cold non-resident. If the entry is
         * already absent from the stack, it is removed from the backing map; otherwise it remains in order for its
        ...
        /**
         * Removes this entry from the cache. This operation is not specified in the paper, which does not account for
         * forced eviction.
         */
        private V remove() {
    ...
    static final class LIRS<K, V> implements EvictionPolicy<K, V> {
        ...
        /**
         * The accessQueue for reducing lock contention See "BP-Wrapper: a system framework making any replacement
         * algorithms (almost) lock contention free"
         * <p/>
         ...
         * <p/>
         * <ul>
         * <li>The LIRS stack, S, which is maintains recency information. All hot entries are on the stack. All cold and
         * non-resident entries which are more recent than the least recent hot entry are also stored in the stack (the
         * stack is always pruned such that the last entry is hot, and all entries accessed more recently than the last
         * hot entry are present in the stack). The stack is ordered by recency, with its most recently accessed entry
         * at the top, and its least recently accessed entry at the bottom.</li>
         * <p/>
         * <li>The LIRS queue, Q, which enqueues all cold entries for eviction. Cold entries (by definition in the
         * queue) may be absent from the stack (due to pruning of the stack). Cold entries are added to the end of the
         * queue and entries are evicted from the front of the queue.</li>
         * </ul>
         */
        ...
        /**
         * Prunes HIR blocks in the bottom of the stack until an HOT block sits in the stack bottom. If pruned blocks
         * were resident, then they remain in the queue; otherwise they are no longer referenced, and are thus removed
                ...
                if (e != null) {
                    ...
                    if (value == null || value.equals(v)) {
                        ...
                        for (HashEntry<K, V> p = first; p != e; p = p.next) {
                            // TODO A remove operation makes the map behave like all the other keys in the bucket were
                            // just added???
                            // allow p to be GC-ed
    ...
    /**
     * Creates a new, empty map with the specified maximum capacity, load factor, concurrency level and LRU eviction
     * policy.
     *
     * @param capacity
     *            is the upper bound capacity for the number of elements in this map
    ...
    /**
     * Creates a new, empty map with the specified maximum capacity, load factor, concurrency level and eviction
     * strategy.
     *
     * @param capacity
     *            is the upper bound capacity for the number of elements in this map
