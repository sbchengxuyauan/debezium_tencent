...
/**
 * The class has externalized its state in {@link TransactionContext} context class so it can be stored in and recovered
 * from offsets. The class receives all processed events and keeps the transaction tracking depending on transaction id.
 * Upon transaction change the metadata events are delivered to a dedicated topic informing about {@code START/END} of
 * the transaction, including transaction id and in case of {@code END} event the amount of events generated by the
 * transaction.
 * <p>
...
public class TransactionMonitor {
    ...
    public static final Schema TRANSACTION_BLOCK_SCHEMA = SchemaBuilder.struct().optional()
            ...
            .field(DEBEZIUM_TRANSACTION_DATA_COLLECTION_ORDER_KEY, Schema.INT64_SCHEMA).build();
    ...
    private static final Schema TRANSACTION_KEY_SCHEMA = SchemaBuilder.struct()
            ...
            .field(DEBEZIUM_TRANSACTION_ID_KEY, Schema.STRING_SCHEMA).build();
    ...
    private static final Schema EVENT_COUNT_PER_DATA_COLLECTION_SCHEMA = SchemaBuilder.struct()
            ...
            .field(DEBEZIUM_TRANSACTION_EVENT_COUNT_KEY, Schema.INT64_SCHEMA).build();
    ...
    private static final Schema TRANSACTION_VALUE_SCHEMA = SchemaBuilder.struct()
            ...
            .field(DEBEZIUM_TRANSACTION_DATA_COLLECTIONS_KEY,
                    SchemaBuilder.array(EVENT_COUNT_PER_DATA_COLLECTION_SCHEMA).optional().build())
            .build();
    ...
    public void dataEvent(DataCollectionId source, OffsetContext offset, Object key, Struct value)
        ...
        if (txId == null) {
            if (LOGGER.isTraceEnabled()) {
                LOGGER.trace("Event '{}' has no transaction id",
                        eventMetadataProvider.toSummaryString(source, offset, key, value));
            }
            return;
        }
    ...
    private void beginTransaction(OffsetContext offsetContext) throws InterruptedException {
        ...
        sender.accept(new SourceRecord(offsetContext.getPartition(), offsetContext.getOffset(), topicName, null,
    ...
    private void endTransaction(OffsetContext offsetContext) throws InterruptedException {
        ...
        final Set<Entry<String, Long>> perTableEventCount = offsetContext.getTransactionContext()
                .getPerTableEventCount().entrySet();
