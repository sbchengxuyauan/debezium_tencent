...
/**
 * An abstract implementation of {@link SnapshotChangeEventSource} that all implementations should extend to inherit
...
public abstract class AbstractSnapshotChangeEventSource implements SnapshotChangeEventSource {
    ...
    public AbstractSnapshotChangeEventSource(CommonConnectorConfig connectorConfig, OffsetContext previousOffset,
    ...
    public SnapshotResult execute(ChangeEventSourceContext context) throws InterruptedException {
        ...
        delaySnapshotIfNeeded(context); // 快照前进行一定的延迟
        ...
        try {
            return doExecute(context, ctx, snapshottingTask); // 真正执行
    ...
    protected void delaySnapshotIfNeeded(ChangeEventSourceContext context) throws InterruptedException {
        Duration snapshotDelay = connectorConfig.getSnapshotDelay(); // 开始快照前的延迟秒数
     ...
     *            mutable context information populated throughout the snapshot process
     * @param snapshottingTask
     *            immutable information about what tasks should be performed during snapshot
     * 
     * @return an indicator to the position at which the snapshot was taken
     */
    protected abstract SnapshotResult doExecute(ChangeEventSourceContext context, SnapshotContext snapshotContext,
    ...
    /**
     * A configuration describing the task to be performed during snapshotting. 执行快照的相关配置,根据该类判断是否执行快照
     *
     *
     */
