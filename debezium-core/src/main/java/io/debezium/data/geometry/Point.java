 ...
 *
 * This historically used to be a useful class, but is now mostly obsolete due to the generic {@link Geometry} type.
 * TODO for 1.0: Decide whether to remove it or keep it.
 *
...
public class Point extends Geometry {
    ...
    /**
     * Returns a {@link SchemaBuilder} for a Point field. A Geometry with extra X & Y fields
     *
     * @return the schema builder
     */
    public static SchemaBuilder builder() {
        return SchemaBuilder.struct().name(LOGICAL_NAME).version(1).doc("Geometry (POINT)")
                .field(X_FIELD, Schema.FLOAT64_SCHEMA).field(Y_FIELD, Schema.FLOAT64_SCHEMA)
                .field(WKB_FIELD, Schema.OPTIONAL_BYTES_SCHEMA).field(SRID_FIELD, Schema.OPTIONAL_INT32_SCHEMA);
    }
    ...
    /**
     * Parses a 2D WKB Point into a {x,y} coordinate array. Returns null for any non-point or points with Z/M/etc
     * modifiers.
     * 
     * @param wkb
     *            OGC WKB geometry
    ...
    public static double[] parseWKBPoint(byte[] wkb) throws IllegalArgumentException {
        if (wkb.length != WKB_POINT_SIZE) {
            throw new IllegalArgumentException(
                    String.format("Invalid WKB for Point (length %d < %d)", wkb.length, WKB_POINT_SIZE));
        }
     ...
     *
     * @param pointSchema
     *            a {@link Schema} instance which represents a point; may not be null
     * @param x
     *            the X coordinate of the point; may not be null
     * @param y
     *            the Y coordinate of the point; may not be null
     * 
     ...
     *            a {@link Schema} instance which represents a point; may not be null
     * @param wkb
     *            the original Well-Known binary representation of the coordinate; may not be null
     * @param srid
     *            the coordinate reference system identifier; null if unset/unknown
     * 
     * @return a {@link Struct} which represents a Connect value for this schema; never null
     */
