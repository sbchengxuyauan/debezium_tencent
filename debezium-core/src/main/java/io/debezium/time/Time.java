 ...
 * <em>milliseconds</em> since midnight, and for defining a Kafka Connect {@link Schema} for time values with no date or
...
public class Time {
    ...
    /**
     * Returns a {@link SchemaBuilder} for a {@link Time}. The resulting schema will describe a field with the
     * {@value #SCHEMA_NAME} as the {@link Schema#name() name} and {@link SchemaBuilder#int32() INT32} for the literal
    ...
    /**
     * Returns a Schema for a {@link Time} but with all other default Schema settings. The schema describes a field with
     ...
     *
     * @param value
     *            the duration value; may not be null
     * @param acceptLargeValues
     *            whether to accept values less than 00:00:00 and larger than 24:00:00 or not
     * 
        ...
        if (value instanceof Duration) {
            ...
            if (!acceptLargeValues && (duration.isNegative() || duration.compareTo(ONE_DAY) > 0)) {
                throw new IllegalArgumentException(
                        "Time values must be between 00:00:00 and 24:00:00 (inclusive): " + duration);
            }
