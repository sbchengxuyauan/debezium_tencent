...
/**
 * A provider of {@link ValueConverter}s and {@link SchemaBuilder}s for various column types. This implementation is
 * aware of the most common JDBC types and values. Specializations for specific DBMSes can be addressed in subclasses.
 * <p>
 * Although it is more likely that values will correspond pretty closely to the expected JDBC types, this class assumes
 * it is possible for some variation to occur when values originate in libraries that are not JDBC drivers.
 * Specifically, the conversion logic for JDBC temporal types with timezones (e.g.,
 * {@link Types#TIMESTAMP_WITH_TIMEZONE}) do support converting values that don't have timezones (e.g.,
 * {@link java.sql.Timestamp}) by assuming a default time zone offset for values that don't have (but are expected to
 * have) timezones. Again, when the values are highly-correlated with the expected SQL/JDBC types, this default timezone
 * offset will not be needed.
 *
...
public class JdbcValueConverters implements ValueConverterProvider {
     ...
     * information to values that require timezones, and uses adapts time and timestamp values based upon the precision
     ...
     *
     * @param decimalMode
     *            how {@code DECIMAL} and {@code NUMERIC} values should be treated; may be null if
     *            {@link DecimalMode#PRECISE} is to be used
     * @param temporalPrecisionMode
     *            temporal precision mode based on {@link io.debezium.jdbc.TemporalPrecisionMode}
     * @param defaultOffset
     *            the zone offset that is to be used when converting non-timezone related values to values that do have
     *            timezones; may be null if UTC is to be used
     * @param adjuster
     *            the optional component that adjusts the local date value before obtaining the epoch day; may be null
     *            if no adjustment is necessary
     * @param bigIntUnsignedMode
     *            how {@code BIGINT UNSIGNED} values should be treated; may be null if
     *            {@link BigIntUnsignedMode#PRECISE} is to be used
     */
    public JdbcValueConverters(DecimalMode decimalMode, TemporalPrecisionMode temporalPrecisionMode,
            ZoneOffset defaultOffset, TemporalAdjuster adjuster, BigIntUnsignedMode bigIntUnsignedMode) {
        ...
        this.adaptiveTimeMicrosecondsPrecisionMode = temporalPrecisionMode
                .equals(TemporalPrecisionMode.ADAPTIVE_TIME_MICROSECONDS);
        ...
        this.fallbackTimestampWithTimeZone = ZonedTimestamp.toIsoString(
                OffsetDateTime.of(LocalDate.ofEpochDay(0), LocalTime.MIDNIGHT, defaultOffset), defaultOffset, adjuster);
        this.fallbackTimeWithTimeZone = ZonedTime.toIsoString(OffsetTime.of(LocalTime.MIDNIGHT, defaultOffset),
    ...
    /**
     * Converts a value object for an expected JDBC type of {@link Types#TIMESTAMP_WITH_TIMEZONE}. The
     * <a href="http://www.oracle.com/technetwork/articles/java/jf14-date-time-2125367.html">standard ANSI to Java 8
     * type mappings</a> specify that the preferred mapping (when using JDBC's {@link java.sql.ResultSet#getObject(int)
     * getObject(...)} methods) in Java 8 is to return {@link OffsetDateTime} for these values.
     * <p>
     ...
     *
     * @param column
     *            the column definition describing the {@code data} value; never null
     * @param fieldDefn
     *            the field definition; never null
     * @param data
     *            the data object to be converted into a {@link Date Kafka Connect date} type; never null
     * 
    ...
    /**
     * Converts a value object for an expected JDBC type of {@link Types#TIME_WITH_TIMEZONE}. The
     * <a href="http://www.oracle.com/technetwork/articles/java/jf14-date-time-2125367.html">standard ANSI to Java 8
     * type mappings</a> specify that the preferred mapping (when using JDBC's {@link java.sql.ResultSet#getObject(int)
     * getObject(...)} methods) in Java 8 is to return {@link OffsetTime} for these values.
     * <p>
     * This method handles several types of objects, including {@link OffsetTime}, {@link java.sql.Time},
     * {@link java.util.Date}, {@link java.time.LocalTime}, and {@link java.time.LocalDateTime}. If any of the types
    ...
    /**
     * Converts a value object for an expected JDBC type of {@link Types#TIMESTAMP} to {@link Timestamp} values, or
     * milliseconds past epoch.
     * <p>
     * Per the JDBC specification, databases should return {@link java.sql.Timestamp} instances, which have date and
     * time info but no time zone info. This method handles {@link java.sql.Date} objects plus any other standard
     * date-related objects such as {@link java.util.Date}, {@link java.time.LocalTime}, and
    ...
    /**
     * Converts a value object for an expected JDBC type of {@link Types#TIMESTAMP} to {@link java.util.Date} values
     * representing milliseconds past epoch.
     * <p>
     * Per the JDBC specification, databases should return {@link java.sql.Timestamp} instances, which have date and
     * time info but no time zone info. This method handles {@link java.sql.Date} objects plus any other standard
     * date-related objects such as {@link java.util.Date}, {@link java.time.LocalTime}, and
    ...
    /**
     * Converts a value object for an expected JDBC type of {@link Types#TIME} to {@link Time} values, or milliseconds
     * past midnight.
     * <p>
     * Per the JDBC specification, databases should return {@link java.sql.Time} instances that have no notion of date
     * or time zones. This method handles {@link java.sql.Date} objects plus any other standard date-related objects
     * such as {@link java.util.Date}, {@link java.time.LocalTime}, and {@link java.time.LocalDateTime}. If any of the
     * types might have date components, those date components are ignored.
     *
     * @param column
     *            the column definition describing the {@code data} value; never null
     * @param fieldDefn
    ...
    /**
     * Converts a value object for an expected JDBC type of {@link Types#TIME} to {@link MicroTime} values, or
    ...
    /**
     * Converts a value object for an expected JDBC type of {@link Types#TIME} to {@link NanoTime} values, or
    ...
    /**
     * Converts a value object for an expected JDBC type of {@link Types#TIME} to {@link java.util.Date} values
     * representing the milliseconds past midnight on the epoch day.
     * <p>
     * Per the JDBC specification, databases should return {@link java.sql.Time} instances that have no notion of date
     * or time zones. This method handles {@link java.sql.Date} objects plus any other standard date-related objects
     * such as {@link java.util.Date}, {@link java.time.LocalTime}, and {@link java.time.LocalDateTime}. If any of the
     ...
     * <p>
     * Per the JDBC specification, databases should return {@link java.sql.Date} instances that have no notion of time
     * or time zones. This method handles {@link java.sql.Date} objects plus any other standard date-related objects
     * such as {@link java.util.Date}, {@link java.time.LocalDate}, and {@link java.time.LocalDateTime}. If any of the
     * types might have time components, those time components are ignored.
     *
     * @param column
     *            the column definition describing the {@code data} value; never null
     * @param fieldDefn
    ...
    protected Object convertDateToEpochDays(Column column, Field fieldDefn, Object data) {
        ...
        return convertValue(column, fieldDefn, data, 0, (r) -> {
            try {
                ...
                logger.warn("Unexpected JDBC DATE value for field {} with schema {}: class={}, value={}",
                        fieldDefn.name(), fieldDefn.schema(), data.getClass(), data);
            }
        });
    }

    /**
     * Converts a value object for an expected JDBC type of {@link Types#DATE} to the number of days past epoch, but
     * represented as a {@link java.util.Date} value at midnight on the date.
     * <p>
     * Per the JDBC specification, databases should return {@link java.sql.Date} instances that have no notion of time
     * or time zones. This method handles {@link java.sql.Date} objects plus any other standard date-related objects
     * such as {@link java.util.Date}, {@link java.time.LocalDate}, and {@link java.time.LocalDateTime}. If any of the
    ...
    /**
     * Converts the given byte array value into a byte buffer as preferred by Kafka Connect. Specific connectors can
     * perform value adjustments based on the column definition, e.g. right-pad with 0x00 bytes in case of fixed length
     * BINARY in MySQL.
     */
     ...
     *
     * @param value
     *            the binary value for which no conversion was found; never null
     * @param fieldDefn
     *            the field definition in the Kafka Connect schema; never null
     * 
     * @return the converted value, or null if the conversion could not be made and the column allows nulls
     * 
     * @throws IllegalArgumentException
    ...
    protected Object convertDecimal(Column column, Field fieldDefn, Object data) {
        ...
        if (decimal instanceof BigDecimal) {
            return SpecialValueDecimal.fromLogical(new SpecialValueDecimal((BigDecimal) decimal), decimalMode,
                    column.name());
        }
        return decimal;
    }
     ...
     * {@link Types#LONGVARCHAR}, {@link Types#CLOB}, {@link Types#NCHAR}, {@link Types#NVARCHAR},
     * {@link Types#LONGNVARCHAR}, {@link Types#NCLOB}, {@link Types#DATALINK}, and {@link Types#SQLXML}.
     *
     * @param column
     *            the column definition describing the {@code data} value; never null
     * @param fieldDefn
        ...
        return convertValue(column, fieldDefn, data, "", (r) -> {
            if (data instanceof SQLXML) {
                try {
                    ...
                    throw new RuntimeException("Error processing data from " + column.jdbcType() + " and column "
                            + column + ": class=" + data.getClass(), e);
                }
            } else {
     ...
     *            the data object to be converted into a {@link Date Kafka Connect date} type; never null
     * @param numBytes
     *            the number of bytes that should be included in the resulting byte[]
     * 
     * @return the converted value, or null if the conversion could not be made and the column allows nulls
     * 
     * @throws IllegalArgumentException
    ...
    /**
     * Determine whether the {@code byte[]} values for columns of type {@code BIT(n)} are {@link ByteOrder#BIG_ENDIAN
     * big-endian} or {@link ByteOrder#LITTLE_ENDIAN little-endian}. All values for {@code BIT(n)} columns are to be
     * returned in {@link ByteOrder#LITTLE_ENDIAN little-endian}.
     * <p>
        ...
        if (column.isOptional() || fieldDefn.schema().isOptional()) {
            ...
            if (logger.isWarnEnabled()) {
                logger.warn("Unexpected value for JDBC type {} and column {}: class={}", column.jdbcType(), column,
                        dataClass.isArray() ? dataClass.getSimpleName() : dataClass.getName()); // don't include value
                                                                                                // in case its
                                                                                                // sensitive
            }
            return null;
        }
        throw new IllegalArgumentException("Unexpected value for JDBC type " + column.jdbcType() + " and column "
                + column + ": class=" + data.getClass()); // don't include value in case its sensitive
    }
     ...
     */
    protected Object convertValue(Column column, Field fieldDefn, Object data, Object fallback,
