...
public class EventRouter<R extends ConnectRecord<R>> implements Transformation<R> {
    ...
    public R apply(R r) {
        ...
        final Schema structValueSchema = onlyHeadersInOutputMessage ? null
                : (fieldSchemaVersion == null) ? getValueSchema(eventValueSchema)
        ...
        additionalFields.forEach((additionalField -> {
            ...
            case ENVELOPE:
                ...
                break;
            case HEADER:
                headers.add(additionalField.getAlias(), eventStruct.get(additionalField.getField()),
        ...
        R newRecord = r.newRecord(eventStruct.getString(routeByField), null, Schema.STRING_SCHEMA,
                defineRecordKey(eventStruct, payloadId), updatedSchema, updatedValue, timestamp, headers);

        return regexRouter.apply(newRecord);
    }

    /**
     * Returns the Kafka record timestamp for the outgoing record. Either obtained from the configured field or the
    ...
    private Long getEventTimestampMs(Struct debeziumEventValue, Struct eventStruct) {
        ...
        if (timestampField == null) {
            throw new ConnectException(
                    String.format("Unable to find timestamp field %s in event", fieldEventTimestamp));
        }
        ...
        if (schemaName == null) {
            throw new ConnectException(
                    String.format("Unsupported field type %s (without logical schema name) for event timestamp",
                            timestampField.schema().type()));
        }
    ...
    public void configure(Map<String, ?> configMap) {
        ...
        io.debezium.config.Field.Set allFields = io.debezium.config.Field
                .setOf(EventRouterConfigDefinition.CONFIG_FIELDS);
        if (!config.validateAndRecord(allFields, LOGGER::error)) {
            throw new ConnectException("Unable to validate config.");
        }

        invalidOperationBehavior = EventRouterConfigDefinition.InvalidOperationBehavior
                .parse(config.getString(EventRouterConfigDefinition.OPERATION_INVALID_BEHAVIOR));
        ...
        onlyHeadersInOutputMessage = !additionalFields.stream().anyMatch(
                field -> field.getPlacement() == EventRouterConfigDefinition.AdditionalFieldPlacement.ENVELOPE);
    }
    ...
    private Schema getValueSchema(Schema debeziumEventSchema, Integer version) {
        if (!versionedValueSchema.containsKey(version)) {
            final Schema schema = getSchemaBuilder(debeziumEventSchema).version(version).build();
    ...
    private SchemaBuilder getSchemaBuilder(Schema debeziumEventSchema) {
        ...
        // Add payload field
        schemaBuilder.field(ENVELOPE_PAYLOAD, debeziumEventSchema.field(fieldPayload).schema());
        ...
        additionalFields.forEach((additionalField -> {
            if (additionalField.getPlacement() == EventRouterConfigDefinition.AdditionalFieldPlacement.ENVELOPE) {
                schemaBuilder.field(additionalField.getAlias(),
