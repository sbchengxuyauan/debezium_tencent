...
public abstract class BaseSourceTask extends SourceTask {
    ...
    /**
     * The change event source coordinator for those connectors adhering to the new framework structure, {@code null}
     * for legacy-style connectors.
     */
    private ChangeEventSourceCoordinator coordinator;

    /**
     * The latest offset that has been acknowledged by the Kafka producer. Will be acknowledged with the source database
     * in {@link BaseSourceTask#commit()} (which may be a no-op depending on the connector).
     */
    ...
    public final void start(Map<String, String> props) {
        ...
        try {
            ...
            if (!config.validateAndRecord(getAllConfigurationFields(), LOGGER::error)) {
                throw new ConnectException("Error configuring an instance of " + getClass().getSimpleName()
    ...
    protected OffsetContext getPreviousOffset(OffsetContext.Loader loader) {
        ...
        if (lastOffset != null) {
            LOGGER.info("lastoffset不为null");
            OffsetContext offsetContext = loader.load(lastOffset);
            LOGGER.info("Found previous offset after restart {}", offsetContext);
            return offsetContext;
        }

        Map<String, Object> previousOffset = context.offsetStorageReader().offsets(Collections.singleton(partition)) // 该方法获取offset
                .get(partition);
