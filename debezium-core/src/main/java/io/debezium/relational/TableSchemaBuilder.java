 ...
 * <p>
 * This builder is responsible for mapping {@link Column table columns} to {@link Field fields} in Kafka Connect
 * {@link Schema}s, and this is necessarily dependent upon the database's supported types. Although mappings are defined
 * for standard types, this class may need to be subclassed for each DBMS to add support for DBMS-specific types by
 * overriding any of the "{@code add*Field}" methods.
 * <p>
 * See the <a href="http://docs.oracle.com/javase/6/docs/technotes/guides/jdbc/getstart/mapping.html#table1">Java SE
 * Mapping SQL and Java Types</a> for details about how JDBC {@link Types types} map to Java value types.
 *
...
public class TableSchemaBuilder {
     ...
     *            the provider for obtaining {@link ValueConverter}s and {@link SchemaBuilder}s; may not be null
    ...
    public TableSchemaBuilder(ValueConverterProvider valueConverterProvider, SchemaNameAdjuster schemaNameAdjuster,
            CustomConverterRegistry customConverterRegistry, Schema sourceInfoSchema, boolean sanitizeFieldNames) {
     ...
     * and a {@link TableSchema#valueSchema() value schema} that contains only those columns that are not in the table's
     * primary key.
     * <p>
     * This is equivalent to calling {@code create(table,false)}.
     *
     * @param schemaPrefix
     *            the prefix added to the table identifier to construct the schema names; may be null if there is no
     *            prefix
     * @param envelopSchemaName
     *            the name of the schema of the built table's envelope
     * @param table
     *            the table definition; may not be null
     * @param filter
     *            the filter that specifies whether columns in the table should be included; may be null if all columns
     *            are to be included
     * @param mappers
     *            the mapping functions for columns; may be null if none of the columns are to be mapped to different
     *            values
     * 
    ...
    public TableSchema create(String schemaPrefix, String envelopSchemaName, Table table, ColumnNameFilter filter,
        ...
        table.columns().stream()
                .filter(column -> filter == null
                        || filter.matches(tableId.catalog(), tableId.schema(), tableId.table(), column.name()))
                .forEach(column -> {
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Mapped primary key for table '{}' to schema: {}", tableId,
                    SchemaUtil.asDetailedString(keySchema));
            LOGGER.debug("Mapped columns for table '{}' to schema: {}", tableId,
                    SchemaUtil.asDetailedString(valSchema));
        }

        Envelope envelope = Envelope.defineSchema().withName(schemaNameAdjuster.adjust(envelopSchemaName))
                .withRecord(valSchema).withSource(sourceInfoSchema).build();

        // Create the generators ...
        Function<Object[], Object> keyGenerator = createKeyGenerator(keySchema, tableId, tableKey.keyColumns());
        Function<Object[], Struct> valueGenerator = createValueGenerator(valSchema, tableId, table.columns(), filter,
                mappers);
     ...
     *
     * @param schema
     *            the Kafka Connect schema for the key; may be null if there is no known schema, in which case the
     *            generator will be null
     * @param columnSetName
     *            the name for the set of columns, used in error messages; may not be null
     * @param columns
     *            the column definitions for the table that defines the row; may not be null
     * 
     * @return the key-generating function, or null if there is no key schema
     */
    protected Function<Object[], Object> createKeyGenerator(Schema schema, TableId columnSetName,
            List<Column> columns) {
        if (schema != null) {
    ...
    private void validateIncomingRowToInternalMetadata(int[] recordIndexes, Field[] fields, ValueConverter[] converters,
        ...
        if (position >= converters.length) {
            LOGGER.error("Error requesting a converter, converters: {}, requested index: {}", converters.length,
                    position);
        ...
        if (position >= fields.length) {
            ...
            throw new ConnectException(
                    "Too few schema fields, internal schema representation is probably out of sync with real database schema");
        }
     ...
     *
     * @param schema
     *            the Kafka Connect schema for the value; may be null if there is no known schema, in which case the
     *            generator will be null
     * @param tableId
    ...
    /**
     * Obtain the array of converters for each column in a row. A converter might be null if the column is not be
    ...
    private ValueConverter wrapInMappingConverterIfNeeded(ColumnMappers mappers, TableId tableId, Column column,
     ...
     * @param table
     *            the table definition; never null
     * @param column
     *            the column definition
     * @param mapper
     *            the mapping function for the column; may be null if the columns is not to be mapped to different
     *            values
     */
    protected void addField(SchemaBuilder builder, Table table, Column column, ColumnMapper mapper) {
        ...
        if (fieldBuilder != null) {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("- field '{}' ({}{}) from column {}", column.name(),
                        builder.isOptional() ? "OPTIONAL " : "", fieldBuilder.type(), column);
            }
        } else {
    ...
    /**
     * Create a {@link ValueConverter} that can be used to convert row values for the given column into the Kafka
     * Connect value object described by the {@link Field field definition}. This uses the supplied
     * {@link ValueConverterProvider} object.
     *
     * @param tableId
     *            the id of the table containing the column; never null
     * @param column
     *            the column describing the input values; never null
     * @param fieldDefn
     *            the definition for the field in a Kafka Connect {@link Schema} describing the output of the function;
     *            never null
     * 
    ...
    protected ValueConverter createValueConverterFor(TableId tableId, Column column, Field fieldDefn) {
        return customConverterRegistry.getValueConverter(tableId, column)
                .orElse(valueConverterProvider.converter(column, fieldDefn));
    }
}
