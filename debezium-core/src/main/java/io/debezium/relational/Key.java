...
/**
 * An immutable definition of a table's key. By default, the key will be comprised of the primary key column(s). A key
 * can be customized with a {@code KeyMapper}.
 *
...
public class Key {
    ...
    public static class CustomKeyMapper {
        ...
        public static KeyMapper getInstance(String fullyQualifiedColumnNames) {
            ...
            // will become =>
            // [inventory.customers.pk1,inventory.customers.pk2,(.*).purchaseorders.pk3,(.*).purchaseorders.pk4]
            // then joining those values
            String regexes = Arrays.asList(fullyQualifiedColumnNames.split(";")).stream().map(s -> s.split(":"))
            ...
            return (table) -> {
                List<Column> candidates = table.columns().stream().filter(c -> {
