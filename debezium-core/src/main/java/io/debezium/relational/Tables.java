...
public final class Tables {
    ...
    public static interface ColumnNameFilter {
        ...
        /**
         * Build the {@link ColumnNameFilter} that determines whether a column identified by a given {@link ColumnId} is
         * to be included, using the given comma-separated regular expression patterns defining which columns (if any)
         * should be <i>excluded</i>.
         * <p>
         * Note that this predicate is completely independent of the table selection predicate, so it is expected that
         * this predicate be used only <i>after</i> the table selection predicate determined the table containing the
         * column(s) is to be used.
         *
         * @param fullyQualifiedColumnNames
         *            the comma-separated list of fully-qualified column names to exclude; may be null or
         * 
        ...
        public static ColumnNameFilter getInstance(String fullyQualifiedColumnNames) {
            ...
            return (catalogName, schemaName, tableName, columnName) -> delegate
                    .test(new ColumnId(new TableId(catalogName, schemaName, tableName), columnName));
        }
    }
     ...
     *            - true if lookup is case insensitive (typical for MySQL on Windows)
     */
     ...
     * @param columnDefs
     *            the list of column definitions; may not be null or empty
     * @param primaryKeyColumnNames
     *            the list of the column names that make up the primary key; may be null or empty
     ...
     *
     * @param table
     *            the definition for the table; may not be null
     * 
     * @return the previous table definition, or null if there was no prior table definition
     */
     ...
     *
     * @param existingTableId
     *            the identifier of the existing table to be renamed; may not be null
     * @param newTableId
     *            the new identifier for the table; may not be null
     * 
     * @return the previous table definition, or null if there was no prior table definition
     */
     ...
     *            the identifier of the table
     * @param changer
     *            the function that accepts the current {@link Table} and returns either the same or an updated
     ...
     *
     * @param catalogName
     *            the name of the database catalog that contains the table; may be null if the JDBC driver does not show
     *            a schema for this table
     * @param schemaName
     *            the name of the database schema that contains the table; may be null if the JDBC driver does not show
     *            a schema for this table
     * @param tableName
     *            the name of the table
     * 
     ...
     * with caution. The resulting editor can be used to modify the table definition, but when completed the new
     * {@link Table} needs to be added back to this object via {@link #overwriteTable(Table)}.
     *
     * @param tableId
     *            the identifier of the table
     * 
    ...
    /**
     * Obtain an editor for the table with the given ID. This method does not lock or modify the set of table
     * definitions, so use with caution. The resulting editor can be used to modify the table definition, but when
     * completed the new {@link Table} needs to be added back to this object via {@link #overwriteTable(Table)}.
     *
     * @param tableId
     *            the identifier of the table
     * 
    ...
    private static class TablesById {
        ...
        public void putAll(TablesById tablesByTableId) {
            if (tableIdCaseInsensitive) {
                tablesByTableId.values.entrySet().forEach(e -> put(e.getKey().toLowercase(), e.getValue()));
            } else {
