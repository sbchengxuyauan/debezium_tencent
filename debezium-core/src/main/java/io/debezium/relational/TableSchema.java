...
/**
 * Defines the Kafka Connect {@link Schema} functionality associated with a given {@link Table table definition}, and
 * which can be used to send rows of data that match the table definition to Kafka Connect.
 * <p>
 ...
 * <p>
 * When the table structure changes, simply obtain a new or updated {@link Table} definition (e.g., via an
 * {@link Table#edit() editor}), rebuild the {@link TableSchema} for that {@link Table}, and use the new
 * {@link TableSchema} instance for subsequent records.
 *
...
public class TableSchema implements DataCollectionSchema {
    ...
    /**
     * Create an instance with the specified {@link Schema}s for the keys and values, and the functions that generate
     * the key and value for a given row of data.
     *
     * @param id
     *            the id of the table corresponding to this schema
     * @param keySchema
     *            the schema for the primary key; may be null
     * @param keyGenerator
     *            the function that converts a row into a single key object for Kafka Connect; may not be null but may
     *            return nulls
     * @param valueSchema
     *            the schema for the values; may be null
     * @param valueGenerator
     *            the function that converts a row into a single value object for Kafka Connect; may not be null but may
     *            return nulls
     */
    public TableSchema(TableId id, Schema keySchema, Function<Object[], Object> keyGenerator, Envelope envelopeSchema,
    ...
    /**
     * Get the {@link Schema} that represents the table's columns, excluding those that make up the
     * {@link #keySchema()}.
     *
    ...
    /**
     * Get the {@link Schema} that represents the entire value of messages for the table, i.e. including before/after
     * state and source info.
     *
    ...
    /**
     * Convert the specified row of values into a Kafka Connect key. The row is expected to conform to the structured
    ...
    /**
     * Convert the specified row of values into a Kafka Connect value. The row is expected to conform to the structured
     * defined by the table.
     *
     * @param columnData
     *            the column values for the table
