...
public interface DatabaseHistoryMXBean {
    ...
    /**
     * The database history starts in {@code STOPPED} state. Upon start it transitions to {@code RECOVERING} state. When
     * all changes from stored history were applied then it switches to {@code RUNNING} state.
     * <p>
     * Maps to {@link DatabaseHistoryMetrics.DatabaseHistoryStatus} enum.
     *
    ...
    /**
     * @return number of changes that were applied during recovery phase increased by number of changes applied during
     *         runtime
     */
