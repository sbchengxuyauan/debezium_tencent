...
public class TableChanges implements Iterable<TableChange> {
     ...
     *            true if the parsed string contains only 2 items and the first should be used as the catalog and the
     *            second as the table name, or false if the first should be used as the schema and the second as the
     *            table name
     */
    ...
    public Array toArray() {
        List<Value> values = changes.stream().map(TableChange::toDocument).map(Value::create)
    ...
    public static class TableChange {
        ...
        private static Table fromDocument(TableId id, Document document) {
            TableEditor editor = Table.editor().tableId(id)
            ...
            document.getArray("columns").streamValues().map(Value::asDocument).map(v -> {
                ColumnEditor columnEditor = Column.editor().name(v.getString("name"))
                ...
                columnEditor.position(v.getInteger("position")).optional(v.getBoolean("optional"))
                        .autoIncremented(v.getBoolean("autoIncremented")).generated(v.getBoolean("generated"));

                return columnEditor.create();
            }).forEach(editor::addColumn);

            editor.setPrimaryKeyNames(document.getArray("primaryKeyColumnNames").streamValues().map(Value::asString)
                    .collect(Collectors.toList()));
