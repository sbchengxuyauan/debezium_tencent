...
/**
 * A {@link DatabaseHistory} implementation that records schema changes as normal {@link SourceRecord}s on the specified
...
public class KafkaDatabaseHistory extends AbstractDatabaseHistory {
    ...
    /**
     * The default replication factor for the history topic which is used in case the value couldn't be retrieved from
     * the broker.
     */
    ...
    public static final Field TOPIC = Field.create(CONFIGURATION_FIELD_PREFIX_STRING + "kafka.topic")
            ...
            .withImportance(Importance.HIGH).withDescription("The name of the topic for the database schema history")
            .withValidation(Field::isRequired);

    public static final Field BOOTSTRAP_SERVERS = Field
            .create(CONFIGURATION_FIELD_PREFIX_STRING + "kafka.bootstrap.servers")
            .withDisplayName("Kafka broker addresses").withType(Type.STRING).withWidth(Width.LONG)
    ...
    public static final Field RECOVERY_POLL_INTERVAL_MS = Field
            .create(CONFIGURATION_FIELD_PREFIX_STRING + "kafka.recovery.poll.interval.ms")
            .withDisplayName("Poll interval during database history recovery (ms)").withType(Type.INT)
            ...
            .withDefault(100).withValidation(Field::isNonNegativeInteger);

    public static final Field RECOVERY_POLL_ATTEMPTS = Field
            .create(CONFIGURATION_FIELD_PREFIX_STRING + "kafka.recovery.attempts")
            .withDisplayName("Max attempts to recovery database history").withType(Type.INT).withWidth(Width.SHORT)
            ...
            .withDefault(100).withValidation(Field::isInteger);
    ...
    public void configure(Configuration config, HistoryRecordComparator comparator, DatabaseHistoryListener listener,
        ...
        this.consumerConfig = config.subset(CONSUMER_PREFIX, true).edit()
                ...
                .withDefault(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class).build();
        this.producerConfig = config.subset(PRODUCER_PREFIX, true).edit()
                ...
                .withDefault(ProducerConfig.CLIENT_ID_CONFIG, dbHistoryName).withDefault(ProducerConfig.ACKS_CONFIG, 1)
    ...
    protected void storeRecord(HistoryRecord record) throws DatabaseHistoryException {
        ...
        try {
            ProducerRecord<String, String> produced = new ProducerRecord<>(topicName, PARTITION, null,
                    record.toString());
            ...
            if (metadata != null) {
                logger.debug("Stored record in topic '{}' partition {} at offset {} ", metadata.topic(),
        ...
        try (KafkaConsumer<String, String> historyConsumer = new KafkaConsumer<>(consumerConfig.asProperties())) {
            ...
            do {
                if (recoveryAttempts > maxRecoveryAttempts) {
                    throw new IllegalStateException(
                            "The database history couldn't be recovered. Consider to increase the value for "
    ...
    private Long getEndOffsetOfDbHistoryTopic(Long previousEndOffset, KafkaConsumer<String, String> historyConsumer) {
        Map<TopicPartition, Long> offsets = historyConsumer
                .endOffsets(Collections.singleton(new TopicPartition(topicName, PARTITION)));
    ...
    public String toString() {
        if (topicName != null) {
            return "Kafka topic " + topicName + ":" + PARTITION + " using brokers at "
    ...
    public void initializeStorage() {
        ...
        try (AdminClient admin = AdminClient.create(this.producerConfig.asProperties())) {
            ...
            throw new ConnectException("Creation of database history topic failed, please create the topic manually",
                    e);
        }
    }
    ...
    private short getDefaultTopicReplicationFactor(AdminClient admin) throws Exception {
        ...
        logger.warn(
                ...
                producerConfig.getString(BOOTSTRAP_SERVERS), DEFAULT_TOPIC_REPLICATION_FACTOR);

        return DEFAULT_TOPIC_REPLICATION_FACTOR;
    }

    private Config getKafkaBrokerConfig(AdminClient admin) throws Exception {
        final Collection<Node> nodes = admin.describeCluster().nodes().get(KAFKA_QUERY_TIMEOUT.toMillis(),
        ...
        final Map<ConfigResource, Config> configs = admin.describeConfigs(resources).all()
