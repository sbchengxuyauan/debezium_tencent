...
public class DdlChanges implements DdlParserListener {
    ...
    /**
     * Consume the events in the same order they were {@link #handle(io.debezium.relational.ddl.DdlParserListener.Event)
     * recorded}, but grouped by database name. Multiple sequential statements that were applied to the same database
    ...
    public boolean anyMatch(Predicate<String> databaseFilter, Predicate<TableId> tableFilter) {
        return events.stream().anyMatch(
                event -> (event instanceof DatabaseEvent) && databaseFilter.test(((DatabaseEvent) event).databaseName())
