...
/**
 * A parser of SQL data types. Callers set up a parser and register patterns that describe the possible lists of data
 * type tokens, and then repeatedly {@link #parse(TokenStream, Consumer) parse} {@link TokenStream streams of tokens}
 * looking for matches.
 * <p>
 * This is typically used within a {@link LegacyDdlParser} implementation to parse and identify data types appearing
 * within a stream of DDL content.
 *
...
public class DataTypeParser {
     ...
     * <p>
     * The order that grammars are registered is the same order that they are evaluated by the resulting parser.
     * However, when multiple grammars match the same input tokens, the grammar that successfully consumes the most
     * input tokens will be selected.
     *
     * @param jdbcType
     *            the best JDBC type for the data type
     * @param grammar
     *            the grammar the defines the data type format; may not be null
     * 
    ...
    /**
     * Examine the stream starting at its current position for a matching data type. If this method finds a matching
     * data type, it will consume the stream of all tokens that make up the data type. However, if no data type is
     * found, the stream is left unchanged.
     * <p>
     * This method looks for data types that match those registered patterns. This method also looks for
     * multi-dimensional arrays, where any registered data type pattern is followed by one or more array dimensions of
     * the form {@code [n]}, where {@code n} is the integer dimension.
     * <p>
     * Sometimes, a data type matches one of the registered patterns but it contains a malformed length/precision,
     * scale, and/or array dimensions. These parsing exceptions can be reported back to the caller via the supplied
     * {@code errorHandler}, although these errors are only reported when no data type is found. This is often useful
     * when the caller expects to find a data type, but no such data type can be found due to one or more parsing
     * exceptions. When this happens, the method calls {@code errorHandler} with all of the {@link ParsingException}s
     * and then returns <code>null</code>.
     *
     * @param stream
     *            the stream of tokens; may not be null
     * @param errorHandler
     *            a function that should be called when no data type was found because at least one
     *            {@link ParsingException} occurred with the length/precision, scale, and/or array dimensions; may be
     *            null
     * 
