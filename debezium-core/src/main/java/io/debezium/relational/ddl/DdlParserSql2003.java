 ...
 * <p>
 * See the <a href="http://savage.net.au/SQL/sql-2003-2.bnf.html">BNF Grammar for ISO/IEC 9075-2:2003</a> for the
...
public class DdlParserSql2003 extends LegacyDdlParser {
    ...
    protected void parseCreate(Marker marker) {
        ...
        if (tokens.matches("TABLE") || tokens.matches("GLOBAL", "TEMPORARY", "TABLE")
                || tokens.matches("LOCAL", "TEMPORARY", "TABLE")) {
    ...
    protected void parseColumnDefinition(Marker start, String columnName, TokenStream tokens, TableEditor table,
            ColumnEditor column, AtomicBoolean isPrimaryKey) {
    ...
    protected void parseDefaultClause(Marker start, ColumnEditor column) {
        ...
        if (tokens.canConsume("CURRENT", "DATE")) {
            ...
            // do nothing, since we don't really care too much about the default value as a function
        } else if (tokens.canConsume("USER") || tokens.canConsume("CURRENT", "USER")
                || tokens.canConsume("CURRENT", "ROLE") || tokens.canConsume("SESSION", "USER")
                || tokens.canConsume("SYSTEM", "USER") || tokens.canConsume("CURRENT", "PATH")) {
            // do nothing, since we don't really care too much about the default value as a function
