...
public abstract class AbstractDdlParser implements DdlParser {
     ...
     *
     * @return the current schema name, or null if the current schema name has not been {@link #setCurrentSchema(String)
     *         set}
     */
    ...
    /**
     * Create a {@link TableId} from the supplied schema and table names. By default, this method uses the supplied
     * schema name as the TableId's catalog, which often matches the catalog name in JDBC database metadata.
     *
     * @param schemaName
     *            the name of the schema; may be null if not specified
     * @param tableName
     *            the name of the table; should not be null
     * 
    ...
    /**
     * Determine whether parsing should exclude comments from the token stream. By default, this method returns
     * {@code true}.
     *
    ...
    protected void signalSetVariable(String variableName, String variableValue, String statement) {
        signalChangeEvent(
                new DdlParserListener.SetVariableEvent(variableName, variableValue, currentSchema, statement));
    }
     ...
     *
     * @param input
     *            input with possible line feeds
     * 
     ...
     *
     * @param possiblyQuoted
     *            string with possible quotes
     * 
     ...
     * @param list
     *            the list of previous parsing exceptions; may be null
     * 
     * @return the list of previous and current parsing exceptions; if {@code e} is null then always {@code list}, but
     *         otherwise non-null list
     */
    public static Collection<ParsingException> accumulateParsingFailure(ParsingException e,
     ...
     *
     * @param e
     *            the multiple parsing exceptions
     * @param list
     *            the list of previous parsing exceptions; may be null
     * 
     * @return the list of previous and current parsing exceptions; if {@code e} is null then always {@code list}, but
     *         otherwise non-null list
     */
    protected Collection<ParsingException> accumulateParsingFailure(MultipleParsingExceptions e,
            Collection<ParsingException> list) {
        if (e == null) {
            return list;
        }
