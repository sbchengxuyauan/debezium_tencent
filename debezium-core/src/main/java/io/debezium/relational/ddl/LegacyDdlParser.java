...
public class LegacyDdlParser extends AbstractDdlParser implements DdlParser {
     ...
     *
     * @param terminator
     *            the terminator character sequence; may be null if the default terminator ({@code ;}) should be used
     */
     ...
     *            the terminator character sequence; may be null if the default terminator ({@code ;}) should be used
     * @param includeViews
     *            {@code true} if view definitions should be included, or {@code false} if they should be skipped
     */
    ...
    /**
     * Parse the next tokens for a possibly qualified name. This method builds up a string containing the schema name
     * (or if none is found the the {@link #currentSchema() current schema}), a '.' delimiter, and the object name. If
     * no schema name is found, just the object name is returned.
     *
     * @param start
     ...
     * token stream, or if none is found the {@link #currentSchema()}, and then calls
     * {@link #resolveTableId(String, String)} with the values.
     *
     * @param start
     *            the start of the statement
     * 
    ...
    /**
     * Parse the next tokens for one or more comma-separated qualified table names. This method uses the schema name
     * that appears in the token stream, or if none is found the {@link #currentSchema()}, and then calls
     * {@link #resolveTableId(String, String)} with the values.
     *
     * @param start
     *            the start of the statement
    ...
    /**
     * Examine the supplied string containing DDL statements, and apply those statements to the specified database table
     * definitions.
     *
     * @param ddlContent
     *            the stream of tokens containing the DDL statements; may not be null
     * @param databaseTables
     *            the database's table definitions, which should be used by this method to create, change, or remove
    ...
    public final void parse(String ddlContent, Tables databaseTables) {
        TokenStream stream = new TokenStream(ddlContent, new DdlTokenizer(!skipComments(), this::determineTokenType),
                false);
        stream.start();
    ...
    /**
     * Examine the stream starting at its current position for DDL statements, and apply those statements to the
     * specified database table definitions.
     *
     * @param ddlContent
     *            the stream of tokens containing the DDL statements; may not be null
     * @param databaseTables
     ...
     *             if the supplied token stream is in an invalid state
     */
    public final void parse(TokenStream ddlContent, Tables databaseTables)
        ...
        try {
            ...
            throw new ParsingException(e.getPosition(),
                    "Failed to parse statement '" + ddlContent.getInputString() + "'", e);
        } catch (Throwable t) {
            parsingFailed(ddlContent.hasNext() ? ddlContent.nextPosition() : null,
                    "Unexpected exception while parsing statement " + ddlContent.getInputString(), t);
        }
    }
     ...
     *
     * @param marker
     *            the start of the statement; never null
     * 
     * @throws ParsingException
     *             if there is an error parsing the statement
     */
    ...
    /**
     * Parse the a DDL line comment. This is generally called by {@link #parseNextStatement} for line comments that
     * appear between other DDL statements, and is not typically called for comments that appear <i>within</i> DDL
     * statements.
     *
     * @param marker
     *            the start of the statement; never null
    ...
    /**
     * Parse a DDL statement that is not known by the {@link #parseNextStatement}. This method can be overridden by
     * subclasses, although it will be more common for subclasses to override {@link #parseNextStatement}.
     * <p>
     * By default this method simply consumes the complete statement.
     *
     * @param marker
     ...
     * @param databaseName
     *            the database name; may not be null
     * @param previousDatabaseName
     *            the previous name of the database if it was renamed, or null if it was not renamed
     * @param statementStart
     *            the start of the statement; may not be null
     */
     ...
     *            the table identifier; may not be null
     * @param previousId
     *            the previous name of the view if it was renamed, or null if it was not renamed
     * @param statementStart
     *            the start of the statement; may not be null
     */
     ...
     * @param id
     *            the table identifier; may be null if the index does not apply to a single table
     * @param statementStart
     *            the start of the statement; may not be null
     */
    ...
    /**
     * Consume all tokens from the current position that is a {@link #initializeStatementStarts(TokenSet)
     * starting-statement token} until either the {@link #terminator() end-of-statement terminator token} or before the
     * next {@link #initializeStatementStarts(TokenSet) starting-statement token}.
     *
     * @throws ParsingException
     *             if the next token is not a {@link #initializeStatementStarts(TokenSet) starting-statement token}
     */
     ...
     * terminator token} or one of the {@link #initializeStatementStarts(TokenSet) tokens that is registered} as the
     * start of a statement.
     *
     * @param start
     *            the marker at which the statement was begun
     */
     ...
     * 
     * @throws ParsingException
     *             if there is no single-quoted string at the current position
     */
     ...
     * 
     * @throws ParsingException
     *             if there is no double-quoted string at the current position
     */
     ...
     * 
     * @throws ParsingException
     *             if there is no single- or double-quoted string at the current position
     */
     ...
     *
     * @param position
     *            the position at which the error occurred; may not be null
     * @param msg
     *            the leading portion of the message; may not be null
     */
     ...
     *            the leading portion of the message; may not be null
     * @param t
     *            the exception that occurred; may be null
     */
    ...
    /**
     * Generate a {@link ParsingException} or {@link MultipleParsingExceptions} with the supplied error or errors and
     * the supplied message, which is appended by this method with additional information about the position's line and
     * column.
     *
     * @param position
     *            the position at which the error occurred; may not be null
     * @param errors
     *            the multiple parsing exception errors; may not be null
     * @param msg
     *            the leading portion of the message; may not be null
     */
    protected void parsingFailed(Position position, Collection<ParsingException> errors, String msg) {
        ...
        throw new MultipleParsingExceptions(msg + " at line " + position.line() + ", column " + position.column(),
                errors);
    }
     ...
     *            the start of the statement
     * 
     * @return the map of resolved Columns keyed by the column alias (or name) used in the SELECT statement; never null
     *         but possibly empty if we couldn't parse the SELECT clause correctly
     */
    protected Map<String, Column> parseColumnsInSelectClause(Marker start) {
        ...
        Table singleTable = fromTablesByAlias.size() == 1 ? fromTablesByAlias.values().stream().findFirst().get()
                : null;
     ...
     * @param start
     *            the start of the statement
     * @param tableAliasByColumnAliases
     *            the map to which is added the column's alias (or name) keyed by the alias of the table in which the
     *            column should appear; may not be null
     * @param columnNameByAliases
     *            the map to which is added the column's name keyed by the its alias (or itself if there is no alias);
     *            may not be null
     */
    protected void parseColumnName(Marker start, Map<String, String> tableAliasByColumnAliases,
     ...
     *            the start of the statement
     * 
     * @return the map of resolved tables keyed by the alias (or table name) used in the SELECT statement; never null
     *         but possibly empty if we couldn't parse the from clause correctly
     */
    ...
    protected boolean canConsumeJoin(Marker start) {
        return tokens.canConsume("JOIN") || tokens.canConsume("INNER", "JOIN") || tokens.canConsume("OUTER", "JOIN")
                || tokens.canConsume("CROSS", "JOIN") || tokens.canConsume("RIGHT", "OUTER", "JOIN")
                || tokens.canConsume("LEFT", "OUTER", "JOIN") || tokens.canConsume("FULL", "OUTER", "JOIN");
    }
     ...
     *            the start of the statement
     * @param tablesByAlias
     *            the map to which this method should add the table keyed by its alias (or name if there is no alias);
     *            may not be null
     */
    private void parseAliasedTableInFrom(Marker start, Map<String, Table> tablesByAlias) {
        ...
        // Aliases in JOIN clauses don't have to be preceded by AS, but can simply be the alias followed by the 'ON'
        // clause
