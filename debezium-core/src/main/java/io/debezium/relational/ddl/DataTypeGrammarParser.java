...
/**
 * A parser for data type grammars that produces one {@link DataTypePattern} for a grammar, where the
 * {@link DataTypePattern} can be used when {@link DataTypeParser parsing} content containing data types adhering to the
 * grammar(s).
 * <p>
 * The grammar using the following rules:
 * <ul>
 * <li>Uppercase {@link DdlTokenizer#KEYWORD keyword} or {@link DdlTokenizer#WORD word} tokens are considered literals
 * that much be matched case-insensitively but completely.</li>
 * <li>Lowercase {@link DdlTokenizer#KEYWORD keyword} or {@link DdlTokenizer#WORD word} tokens are considered logical
 * names of a variable and will match and single token.</li>
 ...
 * {@code S}, with '(L,S)' or '(M,D)'. In these cases, the length will match any positive long, and scale will match any
 * integer value. When the length or scale are represented with integers, they will match only those values.</li>
 * <li>The type may have a length of {@code ...}, in which case the parser will allow (but not capture) a list of
 * values.</li>
...
public class DataTypeGrammarParser {
    ...
    public static class DataTypePattern {
         ...
         *
         * @param stream
         *            the stream of tokens containing the data type definition
         * 
         ...
         *            the stream of tokens containing the data type definition
         * @param errors
         *            the function called for each parsing exception; may be null if not needed
         * 
         * @return the data type, or null if no data type could be found
         */
