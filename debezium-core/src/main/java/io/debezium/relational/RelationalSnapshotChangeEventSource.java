...
public abstract class RelationalSnapshotChangeEventSource extends AbstractSnapshotChangeEventSource {
    ...
    public RelationalSnapshotChangeEventSource(RelationalDatabaseConnectorConfig connectorConfig,
            OffsetContext previousOffset, JdbcConnection jdbcConnection, EventDispatcher<TableId> dispatcher,
            Clock clock, SnapshotProgressListener snapshotProgressListener) {
        this(connectorConfig, previousOffset, jdbcConnection, null, dispatcher, clock, snapshotProgressListener);
    }

    @Override
    public SnapshotResult doExecute(ChangeEventSourceContext context, SnapshotContext snapshotContext,
        ...
        try {
            LOGGER.info("Snapshot step 1 - Preparing"); // 快照之前的准备
            ...
            connection = jdbcConnection.connection(); // 获取连接，并关闭自动提交
            TimeUnit.SECONDS.sleep(60); // 睡眠60s
            LOGGER.info("获取数据库连接");
            ...
            determineCapturedTables(ctx); // 获取所有表的信息
            snapshotProgressListener.monitoredDataCollectionsDetermined(ctx.capturedTables);

            LOGGER.info("Snapshot step 3 - Locking captured tables"); // 锁定将要捕获的表
            ...
            if (snapshottingTask.snapshotData()) {
                ...
                createDataEvents(context, ctx); // 开始导出数据
            } else {
    ...
    private Set<TableId> sort(Set<TableId> capturedTables) throws Exception {
        ...
        if (value != null) {
            return Strings.listOfRegex(value, Pattern.CASE_INSENSITIVE).stream()
    ...
    private void determineCapturedTables(RelationalSnapshotContext ctx) throws Exception {
        ...
        Set<TableId> capturedTables = new HashSet<>(); // 将要捕获的表

        for (TableId tableId : allTableIds) {
            LOGGER.info("获取的表schema: " + tableId.schema() + ", 表名: " + tableId.table());
    ...
    private void createSchemaChangeEventsForTables(ChangeEventSourceContext sourceContext,
            RelationalSnapshotContext snapshotContext) throws Exception {
     ...
     */
    protected abstract SchemaChangeEvent getCreateTableEvent(RelationalSnapshotContext snapshotContext, Table table)
            throws Exception;

    private void createDataEvents(ChangeEventSourceContext sourceContext, RelationalSnapshotContext snapshotContext) // 开始快照数据
            throws InterruptedException {
        SnapshotReceiver snapshotReceiver = dispatcher.getSnapshotChangeEventReceiver();
        snapshotContext.offset.preSnapshotStart();

        for (Iterator<TableId> tableIdIterator = snapshotContext.capturedTables.iterator(); tableIdIterator
                .hasNext();) {
            ...
            createDataEventsForTable(sourceContext, snapshotContext, snapshotReceiver,
                    snapshotContext.tables.forTable(tableId));
        }

        snapshotContext.offset.preSnapshotCompletion();
        snapshotReceiver.completeSnapshot();
        snapshotContext.offset.postSnapshotCompletion(); // 所有表导出完毕
    }
    ...
    private void createDataEventsForTable(ChangeEventSourceContext sourceContext,
            RelationalSnapshotContext snapshotContext, SnapshotReceiver snapshotReceiver, Table table)
            throws InterruptedException {
        ...
        try (Statement statement = readTableStatement(); ResultSet rs = statement.executeQuery(selectStatement.get()) // 开始拉取数据
        ) {
            ...
            if (rs.next()) {
                while (!snapshotContext.lastRecordInTable) {
                    ...
                    if (logTimer.expired()) { // 每隔10秒，记录一次
                    ...
                    dispatcher.dispatchSnapshotEvent(table.id(),
                            getChangeRecordEmitter(snapshotContext, table.id(), row), snapshotReceiver); // 获取
                }
            } else if (snapshotContext.lastTable) {
                // if the last table does not contain any records we still need to mark the last processed event as the
                // last one
                snapshotContext.offset.markLastSnapshotRecord();
            }

            LOGGER.info("\t Finished exporting {} records for table '{}'; total duration '{}'", rows, table.id(),
     ...
     */
    protected ChangeRecordEmitter getChangeRecordEmitter(SnapshotContext snapshotContext, TableId tableId,
            Object[] row) {
     ...
     *
     * @param tableId
     *            the table to generate a query for
     * 
    ...
    private Optional<String> determineSnapshotSelect(SnapshotContext snapshotContext, TableId tableId) {
        ...
        if (overriddenSelect == null) {
            overriddenSelect = connectorConfig.getSnapshotSelectOverridesByTable()
                    .get(new TableId(null, tableId.schema(), tableId.table()));
        }
        LOGGER.info("overriddenSelect: " + overriddenSelect);

        return overriddenSelect != null ? Optional.of(overriddenSelect) : getSnapshotSelect(snapshotContext, tableId);
    }

    /**
     * Returns the SELECT statement to be used for scanning the given table or empty value if the table will be streamed
     * from but not snapshotted
     */
