...
public abstract class RelationalDatabaseSchema implements DatabaseSchema<TableId> {
    ...
    /**
     * Get the {@link Table} meta-data for the table with the given identifier, if that table exists and is included by
     * the filter configuration
     *
     * @param id
     *            the table identifier; may be null
     * 
     * @return the current table definition, or null if there is no table with the given identifier, if the identifier
     *         is null, or if the table has been excluded by the filters
     */
