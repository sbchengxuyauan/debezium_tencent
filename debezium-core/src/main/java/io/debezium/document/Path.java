...
public interface Path extends Iterable<String> {
    ...
    /**
     * Get an {@link Optional} reference to the root path. The resulting Optional will always
     * {@link Optional#isPresent() be present}.
     *
    ...
    /**
     * Parse a JSON Path expression. Segments are separated by a single forward slash ('{@code /}'); any '{@code ~}' or
     * '{@code /} ' literals must be escaped. Trailing slashes are ignored.
     *
     * @param path
     *            the path as a string; may not be null, but may be an empty string or "/" for a root path
     * 
     ...
     * @param path
     *            the path as a string; may not be null, but may be an empty string or "/" for a root path
     * @param resolveJsonPointerEscapes
     *            {@code true} if '{@code ~}' and '{@code /} ' literals are to be escaped as '{@code ~0}' and
     *            '{@code ~1}', respectively, or {@code false} if they are not to be escaped
     * 
     * @return the path object; never null
     */
     ...
     *
     * @return an optional containing the parent (if this is not the root path), or an empty optional if this is the
     *         root path.
     */
     ...
     *
     * @return an optional containing the last segment of this path (if this is not the root path), or an empty optional
     ...
     *             if the index value is equal to or greater than #size()
     */
     ...
     * 
     * @param relPath
     *            the relative path to be appended to this path; may not be null
     * 
     * @return the new path
     */
     ...
     *
     * @param consumer
     *            the function to call on each path segment
     */
