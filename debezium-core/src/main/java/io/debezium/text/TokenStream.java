 ...
 * A {@link TokenStream} object literally represents the stream of {@link Token} objects that each represent a word,
 * symbol, comment or other lexically-relevant piece of information. This simple framework makes it very easy to create
 * a parser that walks through (or "consumes") the tokens in the order they appear and do something useful with that
 * content (usually creating another representation of the content, such as some domain-specific Abstract Syntax Tree or
 * object model).
 *
 * <h3>The parts</h3>
 * <p>
 ...
 * <p>
 * The {@link Tokenizer} is responsible for consuming the character-level input content and constructing {@link Token}
 * objects for the different words, symbols, or other meaningful elements contained in the content. Each Token object is
 * a simple object that records the character(s) that make up the token's value, but it does this in a very lightweight
 * and efficient way by pointing to the original character stream. Each token can be assigned a parser-specific integral
 * <i>token type</i> that may make it easier to do quickly figure out later in the process what kind of information each
 * token represents. The general idea is to keep the Tokenizer logic very simple, and very often {@link Tokenizer}s will
 * merely look for the different kinds of characters (e.g., symbols, letters, digits, etc.) as well as things like
 * quoted strings and comments. However, {@link Tokenizer}s are never called by the parser, but instead are always given
 * to the TokenStream that then calls the Tokenizer at the appropriate time.
 * <p>
 * The {@link TokenStream} is supplied the input content, a Tokenizer implementation, and a few options. Its job is to
 * prepare the content for processing, call the Tokenizer implementation to create the series of Token objects, and then
 * provide an interface for walking through and consuming the tokens. This interface makes it possible to discover the
 * value and type of the current token, and consume the current token and move to the next token. Plus, the interface
 * has been designed to make the code that works with the tokens to be as readable as possible.
 * <p>
 * The final component in this framework is the <b>Parser</b>. The parser is really any class that takes as input the
 * content to be parsed and that outputs some meaningful information. The parser will do this by defining the Tokenizer,
 * constructing a TokenStream object, and then using the TokenStream to walk through the sequence of Tokens and produce
 * some meaningful representation of the content. Parsers can create instances of some object model, or they can create
 * a domain-specific Abstract Syntax Tree representation.
 * <p>
 * The benefit of breaking the responsibility along these lines is that the TokenStream implementation is able to
 * encapsulate quite a bit of very tedious and very useful functionality, while still allowing a lot of flexibility as
 * to what makes up the different tokens. It also makes the parser very easy to write and read (and thus maintain),
 * without placing very many restrictions on how that logic is to be defined. Plus, because the TokenStream takes
 * responsibility for tracking the positions of every token (including line and column numbers), it can automatically
 ...
 * <ul>
 * <li>The {@link #start()} method must be called before any of the other methods. It performs initialization and
 * tokenization, and prepares the internal state by finding the first token and setting an internal <i>current token</i>
 * reference.</li>
 * <li>The {@link #hasNext()} method can be called repeatedly to determine if there is another token after the
 * <i>current token</i>. This is often useful when an unknown number of tokens is to be processed, and behaves very
 * similarly to the {@link Iterator#hasNext()} method.</li>
 * <li>The {@link #consume()} method returns the {@link Token#value() value} of the <i>current token</i> and moves the
 * <i>current token</i> pointer to the next available token.</li>
 * <li>The {@link #consume(String)} and {@link #consume(char)} methods look at the <i>current token</i> and ensure the
 * token's {@link Token#value() value} matches the value supplied as a method parameter, or they throw a
 * {@link ParsingException} if the values don't match. The {@link #consume(int)} method works similarly, except that it
 * attempts to match the token's {@link Token#type() type}. And, the {@link #consume(String, String...)} is a
 * convenience method that is equivalent to calling {@link #consume(String)} for each of the arguments.</li>
 * <li>The {@link #canConsume(String)} and {@link #canConsume(char)} methods look at the <i>current token</i> and check
 * whether the token's {@link Token#value() value} matches the value supplied as a method parameter. If there is a
 * match, the method advances the <i>current token</i> reference and returns true. Otherwise, the <i>current token</i>
 * does not match and the method returns false without advancing the <i>current token</i> reference or throwing a
 * ParsingException. Similarly, the {@link #canConsume(int)} method checks the token's {@link Token#type() type} rather
 * than the value, consuming the token and returning true if there is a match, or just returning false if there is no
 * match. The {@link #canConsume(String, String...)} method determines whether all of the supplied values can be
 * consumed in the given order.</li>
 * <li>The {@link #matches(String)} and {@link #matches(char)} methods look at the <i>current token</i> and check
 * whether the token's {@link Token#value() value} matches the value supplied as a method parameter. The method then
 * returns whether there was a match, but does <i>not</i> advance the <i>current token</i> pointer. Similarly, the
 * {@link #matches(int)} method checks the token's {@link Token#type() type} rather than the value. The
 * {@link #matches(String, String...)} method is a convenience method that is equivalent to calling
 * {@link #matches(String)} for each of the arguments, and the {@link #matches(int, int...)} method is a convenience
 * method that is equivalent to calling {@link #matches(int)} for each of the arguments.</li>
 * </ul>
 * <li>The {@link #matchesAnyOf(String, String...)} methods look at the <i>current token</i> and check whether the
 * token's {@link Token#value() value} matches at least one of the values supplied as method parameters. The method then
 * returns whether there was a match, but does <i>not</i> advance the <i>current token</i> pointer. Similarly, the
 * {@link #matchesAnyOf(int, int...)} method checks the token's {@link Token#type() type} rather than the value.</li>
 * </ul>
 * </p>
 * <p>
 * With these methods, it's very easy to create a parser that looks at the current token to decide what to do, and then
 * consume that token, and repeat this process.
 * </p>
 * <h3>Example parser</h3>
 * <p>
 * Here is an example of a very simple parser that parses very simple and limited SQL <code>SELECT</code> and
 * <code>DELETE</code> statements, such as <code>SELECT * FROM Customers</code> or
 ...
 * <code>parse(...)</code> method takes the input as a parameter, and returns the domain-specific representation that
 * resulted from the parsing. All other methods are utility methods that simply encapsulate common logic or make the
 * code more readable.
 * <p>
 * In the example, the <code>parse(...)</code> first creates a TokenStream object (using a Tokenizer implementation that
 * is not shown), and then loops as long as there are more tokens to read. As it loops, if the next token is "SELECT",
 * the parser calls the <code>parseSelect(...)</code> method which immediately consumes a "SELECT" token, the names of
 * the columns separated by commas (or a '*' if there all columns are to be selected), a "FROM" token, and the name of
 * the table being queried. The <code>parseSelect(...)</code> method returns a <code>Select</code> object, which then
 * added to the list of statements in the <code>parse(...)</code> method. The parser handles the "DELETE" statements in
 * a similar manner.
 *
 * <h3>Case sensitivity</h3>
 * <p>
 * Very often grammars to not require the case of keywords to match. This can make parsing a challenge, because all
 * combinations of case need to be used. The TokenStream framework provides a very simple solution that requires no more
 * effort than providing a boolean parameter to the constructor.
 * <p>
 * When a <code>false</code> value is provided for the the <code>caseSensitive</code> parameter, the TokenStream
 * performs all matching operations as if each token's value were in uppercase only. This means that the arguments
 * supplied to the <code>match(...)</code>, <code>canConsume(...)</code>, and <code>consume(...)</code> methods should
 * be upper-cased. Note that the <i>actual value</i> of each token remains the <i>actual</i> case as it appears in the
 * input.
 * <p>
 * Of course, when the TokenStream is created with a <code>true</code> value for the <code>caseSensitive</code>
 * parameter, the matching is performed using the <i>actual</i> value as it appears in the input content
 *
 * <h3>Whitespace</h3>
 * <p>
 * Many grammars are independent of lines breaks or whitespace, allowing a lot of flexibility when writing the content.
 * The TokenStream framework makes it very easy to ignore line breaks and whitespace. To do so, the Tokenizer
 * implementation must simply not include the line break character sequences and whitespace in the token ranges. Since
 * none of the tokens contain whitespace, the parser never has to deal with them.
 * <p>
 * Of course, many parsers will require that some whitespace be included. For example, whitespace within a quoted string
 * may be needed by the parser. In this case, the Tokenizer should simply include the whitespace characters in the
 * tokens.
 *
 * <h3>Writing a Tokenizer</h3>
 * <p>
 * Each parser will likely have its own {@link Tokenizer} implementation that contains the parser-specific logic about
 * how to break the content into token objects. Generally, the easiest way to do this is to simply iterate through the
 * character sequence passed into the {@link Tokenizer#tokenize(TokenStream.CharacterStream, TokenStream.Tokens)
 * tokenize(...)} method, and use a switch statement to decide what to do.
 * <p>
 * Here is the code for a very basic Tokenizer implementation that ignores whitespace, line breaks and Java-style
 * (multi-line and end-of-line) comments, while constructing single tokens for each quoted string.
 *
 * <pre>
 ...
 * </pre>
 *
 * {@link Tokenizer}s with exactly this behavior can actually be created using the {@link #basicTokenizer(boolean)}
 * method. So while this very basic implementation is not meant to be used in all situations, it may be useful in some
 * situations.
 * </p>
 *
...
public class TokenStream {
    ...
    /**
     * A constant that can be used with the {@link #matches(int)}, {@link #matches(int, int...)}, {@link #consume(int)},
     * and {@link #canConsume(int)} methods to signal that any token type is allowed to be matched.
     */
     ...
     * "current token" in the "consume(...)" and "canConsume(...)" and "matches(...)" methods, the class caches a
     * "current token" and makes this iterator point to the 2nd token.
     *
     * <pre>
     *     T1     T2    T3    T4    T5
     *         &circ;   &circ;  &circ;
    ...
    /**
     * Method to allow subclasses to pre-process the set of tokens and return the correct tokens to use. The default
     * behavior is to simply return the supplied tokens.
     *
     * @param tokens
     *            the tokens
     * 
    ...
    /**
     * Obtain a marker that records the current position so that the stream can be {@link #rewind(Marker)} back to the
     * mark even after having been advanced beyond the mark.
     *
     * @return the marker; never null
     * 
     * @throws IllegalStateException
     *             if this method was called before the stream was {@link #start() started}
    ...
    /**
     * Reset the stream back to the position described by the supplied marker. This method does nothing if the mark is
     * invalid. For example, it is not possible to advance the token stream beyond the current position.
     *
     * @param marker
     *            the marker
     * 
    ...
    /**
     * Advance the stream back to the position described by the supplied marker. This method does nothing if the mark is
     * invalid. For example, it is not possible to rewind the token stream beyond the current position.
     *
     * @param marker
     *            the marker
     * 
     ...
     *
     * @param count
     *            the number of tokens before the current position (e.g., 1 for the previous position)
     * 
     * @return the previous token's position; never null
     * 
     * @throws IllegalStateException
     ...
     * 
     * @throws ParsingException
     *             if there is no such token to consume, or if the token cannot be converted to an integer
     * @throws IllegalStateException
     *             if this method was called before the stream was {@link #start() started}
     */
    public int consumeInteger() throws ParsingException, IllegalStateException {
        ...
        try {
            ...
            if (ePos != -1) {
                ...
                newTokens.add(new CaseInsensitiveToken(currentToken().startIndex() + ePos,
                        currentToken().startIndex() + ePos + 1, DdlTokenizer.WORD, currentToken().position()));
                // Number is in format xxxEyyy
                if (ePos != value.length() - 1) {
                    newTokens.add(new CaseInsensitiveToken(currentToken().startIndex() + ePos + 1,
                            currentToken().endIndex(), DdlTokenizer.WORD, currentToken().position()));
                }
                value = mantissa;
            }
            ...
            throw new ParsingException(position, "Expecting integer at line " + position.line() + ", column "
                    + position.column() + " but found '" + value + "'");
        }
    }
     ...
     * 
     * @throws ParsingException
     *             if there is no such token to consume, or if the token cannot be converted to a long
     * @throws IllegalStateException
     *             if this method was called before the stream was {@link #start() started}
     */
    public long consumeLong() throws ParsingException, IllegalStateException {
        ...
        try {
            ...
            Position position = currentToken().position();
            throw new ParsingException(position, "Expecting long at line " + position.line() + ", column "
                    + position.column() + " but found '" + value + "'");
        }
    }
    ...
    public boolean consumeBoolean() throws ParsingException, IllegalStateException {
        ...
        try {
            ...
            Position position = currentToken().position();
            throw new ParsingException(position, "Expecting boolean at line " + position.line() + ", column "
                    + position.column() + " but found '" + value + "'");
        }
    }
     ...
     * 
     * @throws ParsingException
     *             if there is no such token to consume
     * @throws IllegalStateException
     *             if this method was called before the stream was {@link #start() started}
     */
    ...
    /**
     * Attempt to consume this current token as long as it matches the expected value, or throw an exception if the
     ...
     *
     * @param expected
     *            the expected value of the current token
     * 
     * @return this token stream instance so callers can chain together methods; never null
     * 
     * @throws ParsingException
     *             if the current token doesn't match the supplied value
     * @throws IllegalStateException
     *             if this method was called before the stream was {@link #start() started}
     */
    public TokenStream consume(String expected) throws ParsingException, IllegalStateException {
        if (completed) {
            throw new ParsingException(tokens.get(tokens.size() - 1).position(),
                    "No more content but was expecting token " + expected);
        }
        // Get the value from the current token ...
        if (expected != ANY_VALUE && !currentToken().matches(expected)) {
            ...
            String msg = "Expecting " + expected + " at line " + pos.line() + ", column " + pos.column()
                    + " but found '" + found + "': " + fragment;
    ...
    /**
     * Attempt to consume this current token as long as it matches the expected character, or throw an exception if the
     * token does not match.
     *
     * @param expected
     *            the expected character of the current token
     * 
     * @return this token stream instance so callers can chain together methods; never null
     * 
     * @throws ParsingException
    ...
    public TokenStream consume(char expected) throws ParsingException, IllegalStateException {
        if (completed) {
            throw new ParsingException(tokens.get(tokens.size() - 1).position(),
                    "No more content but was expecting '" + expected + "'");
        }
        // Get the value from the current token ...
     ...
     *
     * @param expectedType
     *            the expected token type of the current token
     * 
     * @return this token stream instance so callers can chain together methods; never null
     * 
     * @throws ParsingException
    ...
    /**
     * Attempt to consume this current token as the next tokens as long as they match the expected values, or throw an
     ...
     * @param expected
     *            the expected value of the current token
     * @param expectedForNextTokens
     *            the expected values of the following tokens
     * 
     * @return this token stream instance so callers can chain together methods; never null
     * 
     * @throws ParsingException
     ...
     *             if this method was called before the stream was {@link #start() started}
     */
    public TokenStream consume(String expected, String... expectedForNextTokens)
     ...
     *
     * @param nextTokens
     *            the expected values for the next tokens
     * 
     * @return this token stream instance so callers can chain together methods; never null
     * 
     * @throws ParsingException
     ...
     *
     * @param typeOptions
     *            the options for the type of the current token
     * 
    ...
    public String consumeAnyOf(int... typeOptions) throws IllegalStateException {
        ...
        Position pos = currentToken().position();
        String fragment = generateFragment();
        String msg = "Expecting " + Strings.join("|", typeOptions) + " at line " + pos.line() + ", column "
                + pos.column() + " but found '" + found + "': " + fragment;
        throw new ParsingException(pos, msg);
    }
     ...
     *
     * @param options
     *            the additional options for the value of the current token
     * 
     * @return the token that was consumed and that matches one of the supplied options
     * 
     * @throws ParsingException
    ...
    public String consumeAnyOf(String... options) throws IllegalStateException {
        ...
        Position pos = currentToken().position();
        String fragment = generateFragment();
        String msg = "Expecting " + String.join("|", options) + " at line " + pos.line() + ", column " + pos.column()
                + " but found '" + found + "': " + fragment;
        throw new ParsingException(pos, msg);
    }

    /**
     * Attempt to consume all tokens until the specified token is consumed, and then stop. If it is not found, then the
     * token stream is left untouched and a ParsingException is thrown.
     * <p>
     * The {@link #ANY_VALUE ANY_VALUE} constant can be used in the expected values as a wildcard.
     * </p>
     *
     * @param expected
     *            the token that is to be found
     * 
     * @return this token stream instance so callers can chain together methods; never null
     * 
     * @throws ParsingException
     *             if the specified token cannot be found
     * @throws IllegalStateException
     *             if this method was called before the stream was {@link #start() started}
     */
     ...
     * @param expected
     *            the token that is to be found
     * @param skipMatchingTokens
     *            the token that, if found, should result in skipping {@code expected} once for each occurrence of
     *            {@code skipMatchingTokens}; may be null
     * 
     * @return this token stream instance so callers can chain together methods; never null
     * 
     * @throws ParsingException
    ...
    /**
     * Attempt to consume all tokens until the specified token is found, and then stop before consuming that token. If
     ...
     *
     * @param stopTokens
     *            the stop tokens; may not be null
     * 
     * @return this token stream instance so callers can chain together methods; never null
     * 
     * @throws ParsingException
     *             if none of the specified tokens can be found
     * @throws IllegalStateException
     *             if this method was called before the stream was {@link #start() started}
     */
    ...
    /**
     * Attempt to consume this current token if it can be parsed as an integer, and return whether this method was
     * indeed able to consume the token.
     *
     * @param consumer
     *            the function that should be called with the integer value if the current token token could be parsed
     * 
     * @return true if the current token did match and was consumed, or false if the current token did not match and
     *         therefore was not consumed
     * 
     * @throws IllegalStateException
     *             if this method was called before the stream was {@link #start() started}
     */
    ...
    /**
     * Attempt to consume this current token if it can be parsed as a boolean, and return whether this method was indeed
     * able to consume the token.
     *
     * @param consumer
     *            the function that should be called with the boolean value if the current token token could be parsed
     * 
     * @return true if the current token did match and was consumed, or false if the current token did not match and
     *         therefore was not consumed
     * 
    ...
    /**
     * Attempt to consume this current token if it can be parsed as a long, and return whether this method was indeed
     * able to consume the token.
     *
     * @param consumer
     *            the function that should be called with the long value if the current token token could be parsed
     * 
     * @return true if the current token did match and was consumed, or false if the current token did not match and
     *         therefore was not consumed
     * 
    ...
    /**
     * Attempt to consume this current token if it matches the expected value, and return whether this method was indeed
     * able to consume the token.
     * <p>
    ...
    /**
     * Attempt to consume this current token if it is {@link BasicTokenizer#WORD} and it matches the expected value, and
     * return whether this method was indeed able to consume the token.
     * <p>
     * The {@link #ANY_VALUE ANY_VALUE} constant can be used in the expected value as a wildcard.
     * </p>
     *
     ...
     *
     * @param expected
     *            the expected value of the current token token
     * 
     * @return true if the current token did match and was consumed, or false if the current token did not match and
     *         therefore was not consumed
     * 
    ...
    /**
     * Attempt to consume this current token if it matches the expected token type, and return whether this method was
     * indeed able to consume the token.
     * <p>
    ...
    /**
     * Attempt to consume this current token and the next tokens if and only if they match the expected values, and
     * return whether this method was indeed able to consume all of the supplied tokens.
     * <p>
     * This is <i>not</i> the same as calling {@link #canConsume(String)} for each of the supplied arguments, since this
     * method ensures that <i>all</i> of the supplied values can be consumed.
     * </p>
     * <p>
     ...
     * <p>
     * This is <i>not</i> the same as calling {@link #canConsume(type String)} for each of the supplied arguments, since
     * this method ensures that <i>all</i> of the supplied values can be consumed.
     * </p>
     * <p>
     * This method <i>is</i> equivalent to calling the following:
     *
    ...
    /**
     * Attempt to consume this current token and the next tokens if and only if they are of {@link BasicTokenizer#WORD}
     * and match the expected values, and return whether this method was indeed able to consume all of the supplied
     * tokens.
     *
     * @param currentExpected
     *            the expected value of the current token
     ...
     *
     * @param nextTokens
     *            the expected values of the next tokens
     * 
     * @return true if the current token did match and was consumed, or false if the current token did not match and
     *         therefore was not consumed
     * 
     ...
     *
     * @param firstOption
     *            the first option for the value of the current token
     ...
     *
     * @param type
     *            the expected type of the curent token
     * @param expected
     *            the expected value of the current token
     * 
     * @return true if the current token did match, or false if the current token did not match
     ...
     * </p>
     *
     * @param currentExpectedType
     *            the expected type of the current token
     * @param expectedTypeForNextTokens
     *            the expected type for the following tokens
     * 
     * @return true if the tokens did match, or false otherwise
     * 
     * @throws IllegalStateException
     ...
     *            the expected type for each of the next tokens
     * 
     * @return true if the tokens did match, or false otherwise
     * 
     * @throws IllegalStateException
    ...
    /**
     * Gets the content string starting at the specified marker (inclusive) and continuing up to the next position
     * (exclusive).
     *
     * @param starting
     *            the marker describing a point in the stream; may not be null
     * 
    ...
    /**
     * Gets the content string starting at the specified marker (inclusive) and continuing up to the end position
     * (exclusive).
     *
     * @param starting
     *            the marker describing a point in the stream; may not be null
     * @param end
     *            the position located directly after the returned content string; can be null, which means end of
     *            content
     * 
     * @return the content string; never null
     */
    ...
    /**
     * Gets the content string starting at the first position (inclusive) and continuing up to the end position
     * (exclusive).
     *
     * @param starting
     *            the position marking the beginning of the desired content string; may not be null
     * @param end
     *            the position located directly after the returned content string; can be null, which means end of
     *            content
     * 
    ...
    protected String getContentBetween(int startIndex, Position end) {
        ...
        if (startIndex >= endIndex) {
            throw new IllegalArgumentException(
                    "The starting position " + startIndex + " must be before the end position " + end);
        }
     ...
     *
     * @param count
     *            the number of tokens back from the current position that this method should return
     * 
     ...
     *
     * @param content
     *            the content from which the fragment should be taken; may not be null
     * @param indexOfProblem
     *            the index of the problem point that should be highlighted; must be a valid index in the content
     * @param charactersToIncludeBeforeAndAfter
     *            the maximum number of characters before and after the problem point to include in the fragment
     * @param highlightText
     *            the text that should be included in the fragment at the problem point to highlight the location, or an
     *            empty string if there should be no highlighting
     * 
     * @return the highlighted fragment; never null
     */
    static String generateFragment(String content, int indexOfProblem, int charactersToIncludeBeforeAndAfter,
    ...
    /**
     * Interface for a Tokenizer component responsible for processing the characters in a {@link CharacterStream} and
     * constructing the appropriate {@link Token} objects.
     */
    public static interface Tokenizer {
         ...
         *
         * @param input
         *            the character input stream; never null
         * @param tokens
         *            the factory for {@link Token} objects, which records the order in which the tokens are created
         * 
         * @throws ParsingException
    ...
    /**
     * Interface used by a {@link Tokenizer} to iterate through the characters in the content input to the
     * {@link TokenStream}.
     */
    public static interface CharacterStream {
         ...
         * 
         * @throws NoSuchElementException
         *             if there is no {@link #hasNext() next character}
         */
        char next();
         ...
         *            the starting index
         * @param endIndex
         *            the index after the last character to include
         * 
         * @return the content
         */
        ...
        /**
         * Determine if the next character on the stream is a {@link Character#isWhitespace(char) whitespace character}.
         * This method does <i>not</i> advance the stream.
         *
         * @return true if there is a {@link #next() next} character and it is a whitespace character, or false
         *         otherwise
         */
        boolean isNextWhitespace();

        /**
         * Determine if the next character on the stream is a {@link Character#isLetterOrDigit(char) letter or digit}.
         * This method does <i>not</i> advance the stream.
         *
        ...
        /**
         * Determine if the next character on the stream is a {@link XmlCharacters#isValid(int) valid XML character}.
         * This method does <i>not</i> advance the stream.
         *
        ...
        /**
         * Determine if the next character on the sream is a {@link XmlCharacters#isValidName(int) valid XML NCName
         * character}. This method does <i>not</i> advance the stream.
         *
         * @return true if there is a {@link #next() next} character and it is a valid XML Name character, or false
         *         otherwise
         */
        boolean isNextValidXmlNameCharacter();

        /**
         * Determine if the next character on the sream is a {@link XmlCharacters#isValidNcName(int) valid XML NCName
         * character}. This method does <i>not</i> advance the stream.
         *
         * @return true if there is a {@link #next() next} character and it is a valid XML NCName character, or false
         *         otherwise
         */
        boolean isNextValidXmlNcNameCharacter();

        /**
         * Determine if the next character on the sream is the supplied value. This method does <i>not</i> advance the
         * stream.
         *
         * @param c
         *            the character value to compare to the next character on the stream
         * 
         * @return true if there is a {@link #next() next} character and it is the supplied character, or false
         *         otherwise
         */
        boolean isNext(char c);

        /**
         * Determine if the next two characters on the stream match the supplied values. This method does <i>not</i>
         * advance the stream.
         *
         * @param nextChar
         *            the character value to compare to the next character on the stream
         ...
         * 
         * @return true if there are at least two characters left on the stream and the first matches
         *         <code>nextChar</code> and the second matches <code>followingChar</code>
         */
        boolean isNext(char nextChar, char followingChar);

        /**
         * Determine if the next three characters on the sream match the supplied values. This method does <i>not</i>
         * advance the stream.
         *
         * @param nextChar
         *            the character value to compare to the next character on the stream
         * @param nextChar2
         *            the character value to compare to the second character on the stream
         * @param nextChar3
         *            the character value to compare to the second character on the stream
         * 
         * @return true if there are at least two characters left on the stream and the first matches
        ...
        /**
         * Determine if the next character on the stream matches one of the supplied characters. This method does
         * <i>not</i> advance the stream.
         *
         * @param characters
         *            the characters to match
         * 
         * @return true if there is a {@link #next() next} character and it does match one of the supplied characters,
         *         or false otherwise
         */
    ...
    public static interface Tokens {
        /**
         * Create a single-character token at the supplied index in the character stream. The token type is set to 0,
         * meaning this is equivalent to calling <code>addToken(index,index+1)</code> or
         * <code>addToken(index,index+1,0)</code>.
         *
         * @param position
         *            the position (line and column numbers) of this new token; may not be null
         * @param index
        ...
        /**
         * Create a single- or multi-character token with the characters in the range given by the starting and ending
         * index in the character stream. The character at the ending index is <i>not</i> included in the token (as this
         * is standard practice when using 0-based indexes). The token type is set to 0, meaning this is equivalent to
         * calling <code>
         ...
         * @param endIndex
         *            the index just past the last character to appear in the token; must be a valid index in the stream
         */
        ...
        /**
         * Create a single- or multi-character token with the supplied type and with the characters in the range given
         * by the starting and ending index in the character stream. The character at the ending index is <i>not</i>
         * included in the token (as this is standard practice when using 0-based indexes).
         *
         * @param position
         *            the position (line and column numbers) of this new token; may not be null
         * @param startIndex
    ...
    public interface Token {
        ...
        /**
         * Get the position of this token, which includes the line number and column number of the first character in
         * the token.
         *
    ...
    /**
     * Obtain a basic {@link Tokenizer} implementation that ignores whitespace but includes tokens for individual
     * symbols, the period ('.'), single-quoted strings, double-quoted strings, whitespace-delimited words, and
     * optionally comments.
     * <p>
     * Note that the resulting Tokenizer may not be appropriate in many situations, but is provided merely as a
     * convenience for those situations that happen to be able to use it.
     * </p>
     *
    ...
    /**
     * A basic {@link Tokenizer} implementation that ignores whitespace but includes tokens for individual symbols, the
     * period ('.'), single-quoted strings, double-quoted strings, whitespace-delimited words, and optionally comments.
     * <p>
     * Note this Tokenizer may not be appropriate in many situations, but is provided merely as a convenience for those
     * situations that happen to be able to use it.
     * </p>
     */
    public static class BasicTokenizer implements Tokenizer {
        /**
         * The {@link Token#type() token type} for tokens that represent an unquoted string containing a character
         * sequence made up of non-whitespace and non-symbol characters.
         */
        public static final int WORD = 1;
        /**
         * The {@link Token#type() token type} for tokens that consist of an individual "symbol" character. The set of
        ...
        /**
         * The {@link Token#type() token type} for tokens that consist of all the characters within single-quotes.
        ...
        /**
         * The {@link Token#type() token type} for tokens that consist of all the characters within double-quotes.
        ...
        /**
         * The {@link Token#type() token type} for tokens that consist of all the characters between "/*" and "&#42;/"
