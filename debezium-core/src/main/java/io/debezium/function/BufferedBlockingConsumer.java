...
/**
 * A {@link BlockingConsumer} that retains a maximum number of values in a buffer before sending them to a delegate
 * consumer.
 * <p>
...
public interface BufferedBlockingConsumer<T> extends BlockingConsumer<T> {
     ...
     *
     * @param function
     *            the function to apply to the values that are flushed
     * 
     * @throws InterruptedException
     *             if the thread is interrupted while this consumer is blocked
     */
    public void close(Function<T, T> function) throws InterruptedException;

    /**
     * Get a {@link BufferedBlockingConsumer} that buffers just the last value seen by the consumer. When another value
     * is then added to the consumer, this buffered consumer will push the prior value into the delegate and buffer the
     * latest.
     * <p>
     * The resulting consumer is threadsafe.
     *
     * @param delegate
     *            the delegate to which values should be flushed; may not be null
     * 
