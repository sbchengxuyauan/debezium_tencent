...
public abstract class RecordParser {
    ...
    static final Set<String> SOURCE_FIELDS = Collect.unmodifiableSet(AbstractSourceInfo.DEBEZIUM_VERSION_KEY,
            AbstractSourceInfo.DEBEZIUM_CONNECTOR_KEY, AbstractSourceInfo.SERVER_NAME_KEY,
            AbstractSourceInfo.TIMESTAMP_KEY, AbstractSourceInfo.SNAPSHOT_KEY, AbstractSourceInfo.DATABASE_NAME_KEY);
    ...
    public static RecordParser create(Schema schema, Object value) {
        ...
        String connectorType = record.getStruct(Envelope.FieldName.SOURCE)
                .getString(AbstractSourceInfo.DEBEZIUM_CONNECTOR_KEY);
    ...
    protected RecordParser(Schema schema, Struct record, String... dataFields) {
        ...
        this.transaction = record.schema().field(Envelope.FieldName.TRANSACTION) != null
                ? record.getStruct(Envelope.FieldName.TRANSACTION) : null;
    ...
    public static final class MysqlRecordParser extends RecordParser {
        ...
        static final Set<String> MYSQL_SOURCE_FIELDS = Collect.unmodifiableSet(TABLE_NAME_KEY, SERVER_ID_KEY, GTID_KEY,
                BINLOG_FILENAME_OFFSET_KEY, BINLOG_POSITION_OFFSET_KEY, BINLOG_ROW_IN_EVENT_OFFSET_KEY, THREAD_KEY,
                QUERY_KEY);
        ...
        public Object getMetadata(String name) {
            ...
            throw new DataException(
                    "No such field \"" + name + "\" in the \"source\" field of events from MySQL connector");
        }
    }
    ...
    public static final class PostgresRecordParser extends RecordParser {
        ...
        public Object getMetadata(String name) {
            ...
            throw new DataException(
                    "No such field \"" + name + "\" in the \"source\" field of events from PostgreSQL connector");
        }
    }
    ...
    public static final class MongodbRecordParser extends RecordParser {
        ...
        static final Set<String> MONGODB_SOURCE_FIELD = Collect.unmodifiableSet(REPLICA_SET_NAME, ORDER, OPERATION_ID,
                COLLECTION);
        ...
        public Object getMetadata(String name) {
            ...
            throw new DataException(
                    "No such field \"" + name + "\" in the \"source\" field of events from MongoDB connector");
        }
    }
    ...
    public static final class SqlserverRecordParser extends RecordParser {
        ...
        static final Set<String> SQLSERVER_SOURCE_FIELD = Collect.unmodifiableSet(CHANGE_LSN_KEY, COMMIT_LSN_KEY,
                EVENT_SERIAL_NO_KEY);
        ...
        public Object getMetadata(String name) {
            ...
            throw new DataException(
                    "No such field \"" + name + "\" in the \"source\" field of events from SQLServer connector");
        }
    }
}
