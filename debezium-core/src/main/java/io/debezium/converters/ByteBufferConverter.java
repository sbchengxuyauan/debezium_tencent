...
/**
 * A customized value converter to allow avro message to be delivered as it is (byte[]) to kafka, this is used for
 * outbox pattern where payload is serialized by KafkaAvroSerializer, the consumer need to get the deseralized payload.
 *
 * To enabled the converter in a connector, the following value need to be specified "value.converter":
