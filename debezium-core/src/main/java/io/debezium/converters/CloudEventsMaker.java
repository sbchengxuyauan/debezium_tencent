...
/**
 * An abstract class that builds CloudEvents attributes using fields of change records provided by {@link RecordParser
 * RecordParser}. Callers {@link #create(RecordParser, SerializerType, String)} create} a concrete CloudEventsMaker for
...
public abstract class CloudEventsMaker {
    ...
    static final Map<SerializerType, String> CONTENT_TYPE_NAME_MAP = Collect.hashMapOf(SerializerType.JSON,
            "application/json", SerializerType.AVRO, "application/avro");
     ...
     *
     * @param parser
     *            the parser of a change record
     * @param contentType
     *            the data content type of CloudEvents
     * @param dataSchemaUriBase
     *            the URI of the schema in case of Avro; may be null
     * 
    ...
    public static CloudEventsMaker create(RecordParser parser, SerializerType contentType, String dataSchemaUriBase) {
        ...
        default:
            throw new DataException(
                    "No usable CloudEvents converters for connector type \"" + parser.connectorType() + "\"");
        }
    }
    ...
    public String ceEnvelopeSchemaName() {
        return recordParser.getMetadata(AbstractSourceInfo.SERVER_NAME_KEY) + "."
                + recordParser.getMetadata(AbstractSourceInfo.DATABASE_NAME_KEY) + "." + "CloudEvents.Envelope";
    }
    ...
    public static final class MysqlCloudEventsMaker extends CloudEventsMaker {
        ...
        @Override
        public String ceId() {
            return "name:" + recordParser.getMetadata(AbstractSourceInfo.SERVER_NAME_KEY) + ";file:"
                    + recordParser.getMetadata(RecordParser.MysqlRecordParser.BINLOG_FILENAME_OFFSET_KEY) + ";pos:"
                    + recordParser.getMetadata(RecordParser.MysqlRecordParser.BINLOG_POSITION_OFFSET_KEY);
        }
    }
    ...
    public static final class PostgresCloudEventsMaker extends CloudEventsMaker {
        ...
        @Override
        public String ceId() {
            return "name:" + recordParser.getMetadata(AbstractSourceInfo.SERVER_NAME_KEY) + ";lsn:"
                    + recordParser.getMetadata(RecordParser.PostgresRecordParser.LSN_KEY).toString() + ";txId:"
                    + recordParser.getMetadata(RecordParser.PostgresRecordParser.TXID_KEY).toString();
        }
    }
    ...
    public static final class MongodbCloudEventsMaker extends CloudEventsMaker {
        ...
        @Override
        public String ceId() {
            return "name:" + recordParser.getMetadata(AbstractSourceInfo.SERVER_NAME_KEY) + ";h:"
                    + recordParser.getMetadata(RecordParser.MongodbRecordParser.OPERATION_ID);
        }
    }
    ...
    public static final class SqlserverCloudEventsMaker extends CloudEventsMaker {
        ...
        @Override
        public String ceId() {
            return "name:" + recordParser.getMetadata(AbstractSourceInfo.SERVER_NAME_KEY) + ";change_lsn:"
                    + recordParser.getMetadata(RecordParser.SqlserverRecordParser.CHANGE_LSN_KEY) + ";commit_lsn:"
                    + recordParser.getMetadata(RecordParser.SqlserverRecordParser.COMMIT_LSN_KEY);
        }
    }
}
