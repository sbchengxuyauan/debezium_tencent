...
public class TableSchemaBuilderTest {
    ...
    private final Object[] data = new Object[] { "c1value", 3.142d, java.sql.Date.valueOf("2001-10-31"), 4,
            new byte[] { 71, 117, 110, 110, 97, 114 }, null, "c7value", "c8value", "c9value" };
    ...
    public void beforeEach() {
        ...
        table = Table.editor().tableId(id).addColumns(
                Column.editor().name("C1").type("VARCHAR").jdbcType(Types.VARCHAR).length(10).optional(false)
                        .generated(true).create(),
                Column.editor().name("C2").type("NUMBER").jdbcType(Types.NUMERIC).length(5).scale(3).create(),
                Column.editor().name("C3").type("DATE").jdbcType(Types.DATE).length(4).optional(true).create(),
                Column.editor().name("C4").type("COUNTER").jdbcType(Types.INTEGER).autoIncremented(true).optional(true)
                        .create(),
                Column.editor().name("C5").type("BINARY").jdbcType(Types.BINARY).optional(false).length(16).create(),
                Column.editor().name("C6").type("SMALLINT").jdbcType(Types.SMALLINT).optional(false).length(1).create(),
                Column.editor().name("7C7") // test invalid Avro name (starts with digit)
                        .type("VARCHAR").jdbcType(Types.VARCHAR).length(10).optional(false).create(),
                Column.editor().name("C-8") // test invalid Avro name ( contains dash )
                        .type("VARCHAR").jdbcType(Types.VARCHAR).length(10).optional(false).create(),
                Column.editor().name(AVRO_UNSUPPORTED_NAME).type("VARCHAR").jdbcType(Types.VARCHAR).length(10)
                        .optional(false).create())
                .setPrimaryKeyNames("C1", "C2").create();
    ...
    public void shouldFailToBuildTableSchemaFromNullTable() {
        new TableSchemaBuilder(new JdbcValueConverters(), adjuster, customConverterRegistry,
                SchemaBuilder.struct().build(), false).create(prefix, "sometopic", null, null, null, null);
    }
    ...
    public void shouldBuildTableSchemaFromTable() {
        schema = new TableSchemaBuilder(new JdbcValueConverters(), adjuster, customConverterRegistry,
                SchemaBuilder.struct().build(), false).create(prefix, "sometopic", table, null, null, null);
    ...
    public void shouldBuildTableSchemaFromTableWithoutPrimaryKey() {
        ...
        assertThat(values.field("C2").schema())
                .isEqualTo(Decimal.builder(3).parameter("connect.decimal.precision", "5").optional().build()); // scale
                                                                                                               // of 3
        ...
        assertThat(values.field("C4").schema()).isEqualTo(SchemaBuilder.int32().optional().build()); // JDBC INTEGER =
                                                                                                     // 32 bits
    ...
    public void shouldSanitizeFieldNamesAndBuildTableSchemaFromTableWithoutPrimaryKey() {
        table = table.edit().setPrimaryKeyNames().create();
        schema = new TableSchemaBuilder(new JdbcValueConverters(), adjuster, customConverterRegistry,
                SchemaBuilder.struct().build(), true).create(prefix, "sometopic", table, null, null, null);
        assertThat(schema).isNotNull();
        // Check the keys ...
        assertThat(schema.keySchema()).isNull();
        assertThat(schema.keyFromColumnData(data)).isNull();
    ...
    public void shouldSanitizeFieldNamesAndValidateSerialization() {
        ...
        SourceRecord record = new SourceRecord(Collections.emptyMap(), Collections.emptyMap(), "sometopic",
                schema.keySchema(), key, schema.valueSchema(), value);
    ...
    public void shouldBuildTableSchemaFromTableWithCustomKey() {
        table = table.edit().setPrimaryKeyNames().create();
        schema = new TableSchemaBuilder(new JdbcValueConverters(), adjuster, customConverterRegistry,
                SchemaBuilder.struct().build(), false).create(prefix, "sometopic", table, null, null,
                        CustomKeyMapper.getInstance("(.*).table:C2,C3"));
        assertThat(schema).isNotNull();
    ...
    public void customKeyMapperShouldMapMultipleTables() {
        ...
        Table table2 = Table.editor().tableId(id2)
                .addColumns(
                        Column.editor().name("C1").type("VARCHAR").jdbcType(Types.VARCHAR).length(10).optional(false)
                                .create(),
                        Column.editor().name("C2").type("NUMBER").jdbcType(Types.NUMERIC).length(5).scale(3).create(),
        ...
        schema = new TableSchemaBuilder(new JdbcValueConverters(), adjuster, customConverterRegistry,
                SchemaBuilder.struct().build(), false).create(prefix, "sometopic", table, null, null, keyMapper);
        ...
        TableSchema schema2 = new TableSchemaBuilder(new JdbcValueConverters(), adjuster, customConverterRegistry,
                SchemaBuilder.struct().build(), false).create(prefix, "sometopic", table2, null, null, keyMapper);
    ...
    public void defaultKeyMapperShouldOrderKeyColumnsBasedOnPrimaryKeyColumnNamesOrder() {
        ...
        Table table2 = Table.editor().tableId(id2)
                .addColumns(Column.editor().name("t1ID").type("INT").jdbcType(Types.INTEGER).optional(false).create(),
                        Column.editor().name("t2ID").type("INT").jdbcType(Types.INTEGER).optional(false).create())
                .setPrimaryKeyNames("t2ID", "t1ID").create();

        TableSchema schema2 = new TableSchemaBuilder(new JdbcValueConverters(), adjuster, customConverterRegistry,
                SchemaBuilder.struct().build(), false).create(prefix, "sometopic", table2, null, null, null);
