...
public class KafkaServer {
     ...
     *            the supplier of the Zookeeper connection string; may not be null
     */
    ...
    /**
     * Set the initial default configuration properties. This method is called from the constructors and can be
     * overridden to customize these properties.
     *
     * @param props
     ...
     *             if the server is running when this method is called
     */
    public KafkaServer setProperty(String name, String value) {
        ...
        if (!KafkaConfig.ZkConnectProp().equalsIgnoreCase(name) && !KafkaConfig.BrokerIdProp().equalsIgnoreCase(name)
    ...
    /**
     * Set multiple configuration properties. Several key properties that deal with Zookeeper, the host name, and the
     * broker ID, may not be set via this method and are ignored since they are controlled elsewhere in this instance.
     *
     * @param properties
     *            the configuration properties; may be null or empty
     * 
     * @return this instance to allow chaining methods; never null
     * 
     * @throws IllegalStateException
     ...
     *
     * @param port
     *            the desired port, or {@code -1} if a random available port should be found and used
     * 
     * @return this instance to allow chaining methods; never null
     */
    ...
    public Properties config() {
        ...
        runningConfig.setProperty(KafkaConfig.AutoCreateTopicsEnableProp(),
                String.valueOf(config.getOrDefault(KafkaConfig.AutoCreateTopicsEnableProp(), Boolean.TRUE)));
    ...
    /**
     * Get the connection string. If the server is not {@link #startup() running} and the port is to be dynamically
     * discovered upon startup, then this method returns "{@code localhost:-1}".
     *
    ...
    public synchronized KafkaServer startup() {
        ...
        // Start the server ...
        try {
            LOGGER.debug("Starting Kafka broker {} at {} with storage in {}", brokerId, getConnection(),
            ...
            server.startup();
            LOGGER.info("Started Kafka server {} at {} with storage in {}", brokerId, getConnection(),
                    logsDir.getAbsolutePath());
     ...
     *
     * @param deleteLogs
     *            whether or not to remove all the log files after shutting down
     */
    public synchronized void shutdown(boolean deleteLogs) {
        if (server != null) {
            try {
                server.shutdown();
                if (deleteLogs) {
                    // as of 0.10.1.1 if logs are not deleted explicitly, there are open File Handles left on .timeindex
                    // files
    ...
    public void setStateDirectory(File stateDirectory) {
        if (stateDirectory != null && stateDirectory.exists() && !stateDirectory.isDirectory()
                && !stateDirectory.canWrite() && !stateDirectory.canRead()) {
