...
public class ZookeeperServer {
    ...
    /**
     * The basic time unit in milliseconds used by ZooKeeper. It is used to do heartbeats and the minimum session
     ...
     *
     * @param deleteData
     *            true if the data should be removed, or false otherwise
     */
    public synchronized void shutdown(boolean deleteData) {
        if (factory != null) {
            try {
                factory.shutdown();
                try {
                    // Zookeeper 3.4.6 does not close the ZK DB during shutdown, so we must do this here to avoid file
    ...
    /**
     * Set the basic time unit in milliseconds used by ZooKeeper. It is used to do heartbeats and the minimum session
     * timeout will be twice the tickTime.
     *
     * @param tickTime
     *            the desired value, or non-positive if the default of {@link #DEFAULT_TICK_TIME} be used
     * 
     * @return this instance to allow chaining methods; never null
     */
     ...
     * 
     * @return the parent directory for the server's logs and snapshots; may be null if a temporary directory will be
     *         used
     */
