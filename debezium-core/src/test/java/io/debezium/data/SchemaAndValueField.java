...
public class SchemaAndValueField {
    ...
    private void assertValue(Struct content) {
        ...
        if (actualValue instanceof List) {
            ...
            Assertions.assertThat(actualValueList).as("List size don't match for " + fieldName)
            ...
            Assertions.assertThat(actualValue.getClass()).as("Incorrect value type for " + fieldName)
                    .isEqualTo(value.getClass());
        }

        if (actualValue instanceof byte[]) {
            Assertions.assertThat((byte[]) actualValue).as("Values don't match for " + fieldName)
                    .isEqualTo((byte[]) value);
    ...
    private void assertStruct(final Struct expectedStruct, final Struct actualStruct) {
        expectedStruct.schema().fields().stream().forEach(field -> {
            ...
            if (expectedValue == null) {
                Assertions.assertThat(actualStruct.get(field.name()))
                        .as(fieldName + " is present in the actual content").isNull();
                return;
            }
            ...
            if (actualValue instanceof byte[]) {
                Assertions.assertThat(expectedValue).as("Array is not expected for " + fieldName)
