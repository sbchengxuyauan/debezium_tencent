...
public class ConfigurationTest {
    ...
    public void beforeEach() {
        config = Configuration.create().with("A", "a").with("B", "b").with("1", 1).build();
    }
    ...
    public void shouldCallFunctionOnEachMatchingFieldUsingRegex() {
        config = Configuration.create().with("column.truncate.to.-10.chars", "should-not-be-matched")
                .with("column.truncate.to.10.chars", "10-chars").with("column.truncate.to.20.chars", "20-chars")
                .with("column.mask.with.20.chars", "20-mask").with("column.mask.with.0.chars", "0-mask")
                .with("column.mask.with.chars", "should-not-be-matched").build();
    ...
    public void shouldMaskPasswords() {
        ...
        config = Configuration.create().with("column.password", "warning").with("column.Password.this.is.not", "value")
                .with("column.truncate.to.20.chars", "20-chars").with("column.mask.with.20.chars", "20-mask")
                .with("column.mask.with.0.chars", "0-mask").with("column.mask.with.chars", "should-not-be-matched")
                .build();
    ...
    /**
     * On Amazon RDS we'll see INSERT (sic!) statements for a heartbeat table in the DDL even stream, they should be
     * filtered out by default (the reason being that those statements are sent using STATEMENT binlog format, which is
     * just applied for that session emitting those statements).
     */
    @Test
    @FixFor("DBZ-469")
    ...
    public void testMsgKeyColumnsField() {
        ...
        assertThat(config.validate(Field.setOf(MSG_KEY_COLUMNS)).get(MSG_KEY_COLUMNS.name()).errorMessages())
                .isNotEmpty();
        // field: ok
