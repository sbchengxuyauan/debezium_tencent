...
public class ByLogicalTableRouterTest {
    ...
    public void testKeyReplacementWorkingConfiguration() {
        ...
        Schema keySchema = SchemaBuilder.struct().name("mysql-server-1.inventory.customers_shard_1.Key")
                .field("id", SchemaBuilder.int64().build()).build();

        Struct key1 = new Struct(keySchema).put("id", 123L);

        SourceRecord record1 = new SourceRecord(new HashMap<>(), new HashMap<>(),
                "mysql-server-1.inventory.customers_shard_1", keySchema, key1, null, null);
        ...
        SourceRecord record2 = new SourceRecord(new HashMap<>(), new HashMap<>(),
                "mysql-server-1.inventory.customers_shard_2", keySchema, key2, null, null);
        ...
        SourceRecord record3 = new SourceRecord(new HashMap<>(), new HashMap<>(),
                "mysql-server-1.inventory.customers_shard_2", keySchema, key3, null, null);
    ...
    public void testHeartbeatMessageTopicReplacementTopic() {
        ...
        Schema keySchema = SchemaBuilder.struct().name("io.debezium.connector.mysql.ServerNameKey")
                .field(keyFieldName, Schema.STRING_SCHEMA).build();

        Struct key1 = new Struct(keySchema).put(keyFieldName, "test_server_name_db");
        SourceRecord record1 = new SourceRecord(new HashMap<>(), new HashMap<>(),
                "__debezium-heartbeat.test_server_name_db", keySchema, key1, null, null);
        ...
        assertThat(((Struct) transformed1.key()).get(keyOriginalKeyTopic))
                .isEqualTo("__debezium-heartbeat.test_server_name_db");

        assertThat(transformed1.value()).isNull();

        Struct key2 = new Struct(keySchema).put(keyFieldName, "test_server_name_db_2");
        SourceRecord record2 = new SourceRecord(new HashMap<>(), new HashMap<>(),
                "__debezium-heartbeat.test_server_name_db_2", keySchema, key2, null, null);
        ...
        assertThat(((Struct) transformed2.key()).get(keyOriginalKeyTopic))
                .isEqualTo("__debezium-heartbeat.test_server_name_db_2");
    ...
    public void testKeyNullValue() {
        ...
        SourceRecord record1 = new SourceRecord(new HashMap<>(), new HashMap<>(),
                "mysql-server-1.inventory.customers_shard_1", null, null, null, null);
