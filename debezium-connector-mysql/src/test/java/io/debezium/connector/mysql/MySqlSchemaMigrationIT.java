...
public class MySqlSchemaMigrationIT extends AbstractConnectorTest {
    ...
    public void shouldCorrectlyMigrateTable() throws SQLException, InterruptedException {
        ...
        config = DATABASE.defaultConfig()
                ...
                .with(MySqlConnectorConfig.TABLE_WHITELIST,
                        DATABASE.qualifiedTableName("monitored") + "," + DATABASE.qualifiedTableName("_monitored_new"))
                .build();

        final MySQLConnection connection = MySQLConnection.forTestDatabase(DATABASE.getDatabaseName());
        connection
                .execute("create table monitored (id int auto_increment primary key, value1 varchar(100), value2 int)");
    ...
    public void shouldProcessAndWarnOnNonWhitelistedMigrateTable() throws SQLException, InterruptedException {
        ...
        config = DATABASE.defaultConfig()
                .with(MySqlConnectorConfig.SNAPSHOT_MODE, MySqlConnectorConfig.SnapshotMode.INITIAL)
                .with(MySqlConnectorConfig.INCLUDE_SCHEMA_CHANGES, false)
                .with(MySqlConnectorConfig.TABLE_WHITELIST, DATABASE.qualifiedTableName("monitored")).build();
        ...
        final String msg1 = "Renaming whitelisted table " + DATABASE.qualifiedTableName("monitored")
                + " to non-whitelisted table " + DATABASE.qualifiedTableName("_monitored_old")
                + ", this can lead to schema inconsistency";
        final String msg2 = "Renaming non-whitelisted table " + DATABASE.qualifiedTableName("_monitored_new")
                + " to whitelisted table " + DATABASE.qualifiedTableName("monitored")
                + ", this can lead to schema inconsistency";
