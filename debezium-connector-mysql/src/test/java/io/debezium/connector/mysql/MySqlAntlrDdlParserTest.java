...
public class MySqlAntlrDdlParserTest {
    ...
    public void shouldHandleQuotes() {
        String ddl = "CREATE TABLE mytable1 (`i.d` INT PRIMARY KEY)" + "CREATE TABLE `mytable2` (`i.d` INT PRIMARY KEY)"
    ...
    public void shouldSupportBinaryCharset() {
        String ddl = "CREATE TABLE mytable (col BINARY(16) GENERATED ALWAYS AS (CAST('xx' as CHAR(16) CHARSET BINARY)) VIRTUAL)"
    ...
    public void shouldSupportCreateIndexBothAlgoAndLock() {
        parser.parse("CREATE INDEX `idx` ON `db`.`table` (created_at) COMMENT '' ALGORITHM DEFAULT LOCK DEFAULT",
                tables);
    ...
    public void shouldParseInternalColumnId() {
        String ddl = "CREATE TABLE USER (INTERNAL BOOLEAN DEFAULT FALSE) ENGINE=InnoDB DEFAULT CHARSET=latin1"
                + System.lineSeparator();
        parser.parse(ddl, tables);
        assertThat(((MySqlAntlrDdlParser) parser).getParsingExceptionsFromWalker().size()).isEqualTo(0);
        assertThat(tables.size()).isEqualTo(1);
    ...
    public void shouldParseTimeWithNowDefault() {
        String ddl = "CREATE TABLE t1 ( " + "c1 int primary key auto_increment, " + "c2 datetime, "
                + "c3 datetime on update now(), " + "c4 char(4));";
        parser.parse(ddl, tables);
        assertThat(tables.size()).isEqualTo(1);
    ...
    public void shouldParseTestStatements() {
        ...
        assertThat(listener.total()).isEqualTo(59 - numberOfAlteredTablesWhichDoesNotExists
                - numberOfIndexesOnNonExistingTables - numberOfCreatedIndexesWhichNotMakeChangeOnTablesModel
                + numberOfAlterViewStatements + numberOfDroppedViews);
    ...
    public void shouldParseCreateViewStatementStartSelect() {
        String ddl = "CREATE TABLE foo ( " + System.lineSeparator() + " c1 INTEGER NOT NULL AUTO_INCREMENT, "
                + System.lineSeparator() + " c2 VARCHAR(22) " + System.lineSeparator() + "); " + System.lineSeparator();
    ...
    public void shouldParseAlterTableRename() {
        final String ddl = "USE db;" + "CREATE TABLE db.t1 (ID INTEGER PRIMARY KEY);"
                + "ALTER TABLE `t1` RENAME TO `t2`;" + "ALTER TABLE `db`.`t2` RENAME TO `db`.`t3`;";
        parser = new MysqlDdlParserWithSimpleTestListener(listener, true);
        parser.parse(ddl, tables);
        assertThat(tables.size()).isEqualTo(1);
    ...
    public void shouldParseCreateViewStatementColumnAliasInnerSelect() {
        String ddl = "CREATE TABLE foo ( " + System.lineSeparator() + " c1 INTEGER NOT NULL AUTO_INCREMENT, "
                + System.lineSeparator() + " c2 VARCHAR(22) " + System.lineSeparator() + "); " + System.lineSeparator();
        String ddl2 = "CREATE VIEW fooView(w1) AS (SELECT foo2.c2 as w1 FROM (SELECT c1 as c2 FROM foo) AS foo2)"
                + System.lineSeparator();
    ...
    public void shouldUseFiltersForAlterTable() {
        parser = new MysqlDdlParserWithSimpleTestListener(listener,
                TableFilter.fromPredicate(x -> !x.table().contains("ignored")));
    ...
    public void shouldParseCommentWithEngineName() {
        final String ddl = "CREATE TABLE t1 (" + "`id` int(11) NOT NULL AUTO_INCREMENT, "
                + "`field_1` int(11) NOT NULL,  " + "`field_2` int(11) NOT NULL,  " + "`field_3` int(11) NOT NULL,  "
                + "`field_4` int(11) NOT NULL,  " + "`field_5` tinytext COLLATE utf8_unicode_ci NOT NULL, "
                + "`field_6` tinytext COLLATE utf8_unicode_ci NOT NULL, "
                + "`field_7` tinytext COLLATE utf8_unicode_ci NOT NULL COMMENT 'CSV'," + "primary key(id));";
        parser.parse(ddl, tables);
        assertThat(tables.size()).isEqualTo(1);
    ...
    public void parseAddPartition() {
        String ddl = "CREATE TABLE flat_view_request_log (" + "  id INT NOT NULL, myvalue INT DEFAULT -10,"
    ...
    public void shouldParseEnumOptions() {
        ...
        assertParseEnumAndSetOptions("enum('a','b','c')", "a", "b", "c");
        assertParseEnumAndSetOptions("ENUM('a','multi','multi with () paren', 'other')", "a", "multi",
                "multi with () paren", "other");
    ...
    public void shouldParseSetOptions() {
        ...
        assertParseEnumAndSetOptions("set('a','b','c')", "a", "b", "c");
        assertParseEnumAndSetOptions("SET('a','multi','multi with () paren', 'other')", "a", "multi",
                "multi with () paren", "other");
    ...
    public void shouldParseMultipleStatements() {
        String ddl = "CREATE TABLE foo ( " + System.lineSeparator() + " c1 INTEGER NOT NULL, " + System.lineSeparator()
                + " c2 VARCHAR(22) " + System.lineSeparator() + "); " + System.lineSeparator() + "-- This is a comment"
                + System.lineSeparator() + "DROP TABLE foo;" + System.lineSeparator();
        parser.parse(ddl, tables);
    ...
    public void shouldParseCreateTableStatementWithSingleGeneratedColumnAsPrimaryKey() {
        String ddl = "CREATE TABLE my.foo ( " + System.lineSeparator() + " c1 INTEGER NOT NULL AUTO_INCREMENT, "
                + System.lineSeparator() + " c2 VARCHAR(22), " + System.lineSeparator() + " PRIMARY KEY (c1)"
    ...
    public void shouldParseCreateTableStatementWithMultipleColumnsForPrimaryKey() {
        String ddl = "CREATE TABLE shop (" + " id BIGINT(20) NOT NULL AUTO_INCREMENT," + " version BIGINT(20) NOT NULL,"
                + " name VARCHAR(255) NOT NULL," + " owner VARCHAR(255) NOT NULL,"
                + " phone_number VARCHAR(255) NOT NULL," + " primary key (id, name)" + " );";
        parser.parse(ddl, tables);
        assertThat(tables.size()).isEqualTo(1);
    ...
    public void shouldParseThirdPartyStorageEngine() {
        String ddl1 = "CREATE TABLE foo ( " + System.lineSeparator() + " c1 INTEGER NOT NULL, " + System.lineSeparator()
                + " c2 VARCHAR(22) " + System.lineSeparator() + ") engine=TokuDB compression=tokudb_zlib;";
        String ddl2 = "CREATE TABLE bar ( " + System.lineSeparator() + " c1 INTEGER NOT NULL, " + System.lineSeparator()
                + " c2 VARCHAR(22) " + System.lineSeparator() + ") engine=TokuDB compression='tokudb_zlib';";
        String ddl3 = "CREATE TABLE baz ( " + System.lineSeparator() + " c1 INTEGER NOT NULL, " + System.lineSeparator()
                + " c2 VARCHAR(22) " + System.lineSeparator() + ") engine=Aria;";
    ...
    public void shouldParseEngineNameWithApostrophes() {
        ...
        assertThat(tables.tableIds().stream().map(TableId::table).collect(Collectors.toSet())).containsOnly("t1", "t2",
                "t3", "t4");
    }
    ...
    public void shouldParseCreateTableStatementWithSignedTypes() {
        String ddl = "CREATE TABLE foo ( " + System.lineSeparator() + " c1 BIGINT SIGNED NOT NULL, "
                + System.lineSeparator() + " c2 INT UNSIGNED NOT NULL " + System.lineSeparator() + "); "
                + System.lineSeparator();
        parser.parse(ddl, tables);
        assertThat(tables.size()).isEqualTo(1);
        Table foo = tables.forTable(new TableId(null, null, "foo"));
    ...
    public void shouldParseCreateDatabaseAndTableThatUsesDefaultCharacterSets() {
        String ddl = "SET character_set_server=utf8;" + System.lineSeparator()
                + "CREATE DATABASE db1 CHARACTER SET utf8mb4;" + System.lineSeparator() + "USE db1;"
                + System.lineSeparator() + "CREATE TABLE t1 (" + System.lineSeparator()
                + " id int(11) not null auto_increment," + System.lineSeparator() + " c1 varchar(255) default null,"
                + System.lineSeparator() + " c2 varchar(255) not null," + System.lineSeparator()
                + " c3 varchar(255) charset latin2 not null," + System.lineSeparator() + " primary key ('id')"
                + System.lineSeparator() + ") engine=InnoDB auto_increment=1006 default charset=latin1;"
                + System.lineSeparator();
        parser.parse(ddl, tables);
        ...
        ddl = "CREATE TABLE t2 (" + System.lineSeparator() + " id int(11) not null auto_increment,"
                + System.lineSeparator() + " c1 varchar(255) default null," + System.lineSeparator()
                + " c2 varchar(255) not null," + System.lineSeparator() + " c3 varchar(255) charset latin2 not null,"
                + System.lineSeparator() + " primary key ('id')" + System.lineSeparator()
    ...
    public void shouldParseCreateAndAlterWithOnUpdate() {
        String ddl = "CREATE TABLE customers ( " + "id INT PRIMARY KEY NOT NULL, " + "name VARCHAR(30) NOT NULL, "
                + "PRIMARY KEY (id) );" + "" + "CREATE TABLE `CUSTOMERS_HISTORY` LIKE `customers`; " + ""
                + "ALTER TABLE `CUSTOMERS_HISTORY` MODIFY COLUMN `id` varchar(36) NOT NULL," + "DROP PRIMARY KEY,"
                + "ADD action tinyint(3) unsigned NOT NULL FIRST,"
                + "ADD revision int(10) unsigned NOT NULL AFTER action,"
                + "ADD changed_on DATETIME NOT NULL DEFAULT NOW() AFTER revision," + "ADD PRIMARY KEY (id, revision);";
        parser.parse(ddl, tables);
        assertThat(tables.size()).isEqualTo(2);
        assertThat(listener.total()).isEqualTo(3);
    ...
    public void shouldParseFulltextKeyInCreateTable() {
        ...
        assertThat(t).isNotNull();
        assertThat(t.retrieveColumnNames()).containsExactly("id", "name", "context", "organization_id", "client_id",
                "scope_action_ids");
        assertThat(t.primaryKeyColumnNames()).containsExactly("id");
    ...
    public void shouldParseAlterTableWithIndex() {
        final String ddl = "USE db;" + "CREATE TABLE db.t1 (ID INTEGER PRIMARY KEY, val INTEGER, INDEX myidx(val));"
    ...
    public void shouldParseStatementForDbz200() {
        ...
        assertThat(t).isNotNull();
        assertThat(t.retrieveColumnNames()).containsExactly("ENCODEDKEY", "ID", "CREATIONDATE", "LASTMODIFIEDDATE",
                "DATATYPE", "ISDEFAULT", "ISREQUIRED", "NAME", "VALUES", "AMOUNTS", "DESCRIPTION", "TYPE",
                "VALUELENGTH", "INDEXINLIST", "CUSTOMFIELDSET_ENCODEDKEY_OID", "STATE", "VALIDATIONPATTERN",
                "VIEWUSAGERIGHTSKEY", "EDITUSAGERIGHTSKEY", "BUILTINCUSTOMFIELDID", "UNIQUE", "STORAGE");
    ...
    public void shouldParseCreateTableStatementWithColumnNamedColumn() {
        String ddl = "CREATE TABLE `mytable` ( " + System.lineSeparator()
                ...
                + " `column` varchar(255) NOT NULL DEFAULT '', " + System.lineSeparator() + " PRIMARY KEY (`def`) "
                + System.lineSeparator() + " ) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
    ...
    public void shouldParseAlterTableStatementWithColumnNamedColumnWithoutColumnWord() {
        String ddl = "CREATE TABLE `mytable` ( " + System.lineSeparator()
                + " `def` int(11) unsigned NOT NULL AUTO_INCREMENT, " + System.lineSeparator() + " PRIMARY KEY (`def`) "
                + System.lineSeparator() + " ) ENGINE=InnoDB DEFAULT CHARSET=utf8;";

        parser.parse(ddl, tables);

        ddl = "ALTER TABLE `mytable` " + "ADD `column` varchar(255) NOT NULL DEFAULT '', "
                + "ADD `ghi` varchar(255) NOT NULL DEFAULT '', " + "ADD jkl varchar(255) NOT NULL DEFAULT '' ;";
    ...
    public void shouldParseAlterTableStatementWithColumnNamedColumnWithColumnWord() {
        ...
        ddl = "ALTER TABLE `mytable` " + "ADD COLUMN `column` varchar(255) NOT NULL DEFAULT '', "
    ...
    public void parseUserDdlStatements() {
        String ddl = "CREATE USER 'jeffrey'@'localhost' IDENTIFIED BY 'password';"
                + "RENAME USER 'jeffrey'@'localhost' TO 'jeff'@'127.0.0.1';" + "DROP USER 'jeffrey'@'localhost';"
    ...
    public void shouldParseChangeColumnAndKeepName() {
        final String create = "CREATE TABLE test ("
                + "  id INT NOT NULL, myvalue ENUM('Foo','Bar','Baz') NOT NULL DEFAULT 'Foo'," + "  PRIMARY KEY (`id`)"
                + ");";
    ...
    public void parseDefaultValue() {
        String ddl = "CREATE TABLE tmp (id INT NOT NULL, " + "columnA CHAR(60) NOT NULL DEFAULT 'A',"
                + "columnB INT NOT NULL DEFAULT 1," + "columnC VARCHAR(10) NULL DEFAULT 'C',"
                + "columnD VARCHAR(10) NULL DEFAULT NULL," + "columnE VARCHAR(10) NOT NULL,"
        ...
        assertThat(table.columnWithName("my_dateA").defaultValue())
                .isEqualTo(LocalDateTime.of(2018, 4, 27, 13, 28, 43).toEpochSecond(ZoneOffset.UTC) * 1_000);
        assertThat(table.columnWithName("my_dateB").defaultValue())
                .isEqualTo(LocalDateTime.of(9999, 12, 31, 0, 0, 0).toEpochSecond(ZoneOffset.UTC) * 1_000);
    }
    ...
    public void shouldTreatPrimaryKeyColumnsImplicitlyAsNonNull() {
        ...
        ddl = "DROP TABLE IF EXISTS data; " + "CREATE TABLE data(id INT DEFAULT 1, PRIMARY KEY (id))";
        parser.parse(ddl, tables);
    ...
    /**
     * Assert whether the provided {@code typeExpression} string after being parsed results in a list of {@code ENUM} or
     * {@code SET} options that match exactly to the provided list of {@code expecetedValues}.
     * <p/>
     * In this particular implementation, we construct a {@code CREATE} statement and invoke the antlr parser on the
     * statement defining a column named {@code options} that represents the supplied {@code ENUM} or {@code SET}
     * expression.
     *
     * @param typeExpression
     *            The {@code ENUM} or {@code SET} expression to be parsed
     * @param expectedValues
     *            An array of options expected to have been parsed from the expression.
     */
    ...
    /**
     * Reads the lines starting with a given line number from the specified file on the classpath. Any lines preceding
     * the given line number will be included as empty lines, meaning the line numbers will match the input file.
     *
     * @param startingLineNumber
     *            the 1-based number designating the first line to be included
    ...
    private void assertColumn(Table table, String name, String typeName, int jdbcType, int length, String charsetName,
            boolean optional) {
    ...
    class MysqlDdlParserWithSimpleTestListener extends MySqlAntlrDdlParser {
        ...
        private MysqlDdlParserWithSimpleTestListener(DdlChanges changesListener, boolean includeViews,
                TableFilter tableFilter) {
            super(false, includeViews, new MySqlValueConverters(JdbcValueConverters.DecimalMode.DOUBLE,
                    TemporalPrecisionMode.ADAPTIVE_TIME_MICROSECONDS, JdbcValueConverters.BigIntUnsignedMode.PRECISE),
                    tableFilter);
