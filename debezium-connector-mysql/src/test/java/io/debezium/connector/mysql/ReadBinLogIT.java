...
public class ReadBinLogIT implements Testing {
    ...
    protected void startClient(Consumer<BinaryLogClient> preConnect)
        ...
        conn.execute("DROP TABLE IF EXISTS person",
                "CREATE TABLE person (" + "  name VARCHAR(255) primary key," + "  age INTEGER NULL DEFAULT 10,"
    ...
    public void shouldCaptureMultipleWriteUpdateDeletesInSingleEvents() throws Exception {
        ...
        counters.consume(1, XidEventData.class); // COMMIT
        List<WriteRowsEventData> writeRowEvents = recordedEventData(WriteRowsEventData.class, 1);
        assertRows(writeRowEvents.get(0),
                rows().insertedRow("Georgia", 30, any(), any()).insertedRow("Janice", 19, any(), any()));
        counters.reset();

        // update as a single statement/transaction
        conn.execute("UPDATE person SET name = CASE " + "                          WHEN name = 'Georgia' THEN 'Maggie' "
                + "                          WHEN name = 'Janice' THEN 'Jamie' " + "                         END "
        ...
        counters.consume(1, XidEventData.class); // COMMIT
        List<DeleteRowsEventData> deleteRowEvents = recordedEventData(DeleteRowsEventData.class, 1);
        assertRows(deleteRowEvents.get(0),
                rows().removedRow("Maggie", 30, any(), any()).removedRow("Jamie", 19, any(), any()));
    }

    /**
     * Test case that is normally commented out since it is only useful to print out the DDL statements recorded by the
     * binlog during a MySQL server initialization and startup.
     *
     * @throws Exception
    ...
    public void shouldQueryInformationSchema() throws Exception {
        ...
        //
        // conn.query("select TABLE_NAME, ROW_FORMAT, TABLE_ROWS, AVG_ROW_LENGTH, DATA_LENGTH, MAX_DATA_LENGTH,
        // INDEX_LENGTH,
        // DATA_FREE, " +
        // "AUTO_INCREMENT, CREATE_TIME, UPDATE_TIME, CHECK_TIME, TABLE_COLLATION, CHECKSUM, CREATE_OPTIONS,
        // TABLE_COMMENT " +
    ...
    protected <T extends EventData> List<T> recordedEventData(Class<T> eventType, int expectedCount) {
        List<T> results = null;
        synchronized (events) {
            results = events.stream().map(Event::getData).filter(eventType::isInstance).map(eventType::cast)
    ...
    protected static class EventQueue implements EventListener {
        ...
        /**
         * Blocks until the listener has consume the specified number of matching events, blocking at most the default
         * number of milliseconds. If this method has not reached the number of matching events and comes across events
         * that do not satisfy the predicate, those events are consumed and ignored.
         *
         * @param eventCount
        ...
        /**
         * Blocks until the listener has consume the specified number of matching events, blocking at most the specified
         * number of milliseconds. If this method has not reached the number of matching events and comes across events
         * that do not satisfy the predicate, those events are consumed and ignored.
         *
         * @param eventCount
        ...
        public void consume(int eventCount, long timeoutInMillis, Predicate<Event> condition) throws TimeoutException {
            ...
            if (eventsRemaining > 0) {
                throw new TimeoutException("Received " + (eventCount - eventsRemaining) + " of " + eventCount + " in "
                        + timeoutInMillis + "ms");
            }
        }

        /**
         * Blocks until the listener has seen the specified number of events with the given type, or until the default
         * timeout has passed.
         *
         * @param eventCount
         *            the number of events
        ...
        /**
         * Blocks until the listener has seen the specified number of events with the given type, or until the specified
         * time has passed.
         *
         * @param eventCount
         *            the number of events
        ...
        /**
         * Blocks until the listener has seen the specified number of events with event data matching the specified
         * class, or until the specified time has passed.
         *
         * @param eventCount
         *            the number of events
         * @param eventDataClass
         ...
         *             if the waiting timed out before the expected number of events were received
         */
        public void consume(int eventCount, Class<? extends EventData> eventDataClass, long timeoutMillis)
                throws TimeoutException {
            consume(eventCount, defaultTimeoutInMillis, event -> {
