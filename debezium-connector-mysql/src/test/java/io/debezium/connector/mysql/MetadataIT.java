...
public class MetadataIT implements Testing {
    ...
    /**
     * Loads the {@link Tables} definition by reading JDBC metadata. Note that some characteristics, such as whether
     * columns are generated, are not exposed through JDBC (unlike when reading DDL).
     * 
     * @throws SQLException
     *             if there's an error
     */
    ...
    public void shouldLoadMetadataViaJdbc() throws SQLException {
        ...
        try (MySQLConnection conn = MySQLConnection.forTestDatabase(DATABASE.getDatabaseName());) {
            ...
            conn.execute("DROP TABLE IF EXISTS person", "DROP TABLE IF EXISTS product",
                    "DROP TABLE IF EXISTS purchased");

            conn.execute("CREATE TABLE person (" + "  name VARCHAR(255) primary key," + "  birthdate DATE NULL,"
                    + "  age INTEGER NULL DEFAULT 10," + "  salary DECIMAL(5,2)," + "  bitStr BIT(18)" + ")");
            ...
            conn.execute("CREATE TABLE product (" + "  id INT NOT NULL AUTO_INCREMENT,"
            ...
            assertThat(product.columnWithName("createdByDate").isGenerated())
                    .isEqualTo(conn.databaseAsserts().isCurrentDateTimeDefaultGenerated());
            ...
            assertThat(product.columnWithName("modifiedDate").isGenerated())
                    .isEqualTo(conn.databaseAsserts().isCurrentDateTimeDefaultGenerated());
            assertThat(product.columnWithName("modifiedDate").isOptional()).isFalse();

            conn.execute("CREATE TABLE purchased (" + "  purchaser VARCHAR(255) NOT NULL," + "  productId INT NOT NULL,"
            ...
            assertThat(purchased.columnWithName("purchaseDate").isGenerated())
                    .isEqualTo(conn.databaseAsserts().isCurrentDateTimeDefaultGenerated());
