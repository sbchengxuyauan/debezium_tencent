...
/**
 * Run the {@link MySqlConnector} in various configurations and against different MySQL server instances and verify the
...
public class AbstractMySqlConnectorOutputTest extends ConnectorOutputTest {
     ...
     *
     * @param master
     *            the configuration with the {@link MySqlConnectorConfig#HOSTNAME} and {@link MySqlConnectorConfig#PORT}
     ...
     */
    protected static void waitForGtidSetsToMatch(Configuration master, Configuration replica, long timeout,
            TimeUnit unit) {
        ...
        try {
            ...
            Testing.print("Waited a total of " + sw.durations().statistics().getTotalAsString()
                    + " for the replica to catch up to the master.");
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        }
    }
    ...
    /**
     * Read the system variables and master status using the given connector configuration. This can be used when
     * creating {@link io.debezium.embedded.ConnectorOutputTest.TestSpecification} instances
     * {@link io.debezium.embedded.ConnectorOutputTest.TestSpecification#withVariables(VariableSupplier) with
     * variables}.
     * <p>
     * When connected to a replica, the GTID source for the master is added to the variables using the "@{code
     * master_uuid}" variable, which does not correspond to a real MySQL system variable. The GTID source of the server
     * to which the test case connects is given by the "{@code server_uuid}" system variable.
     *
     * @param config
     *            the connector configuration; never null
     * @return the available system variables
     * @throws Exception
     *             if there is a problem connecting to the database and reading the system variables
     */
        ...
        try (MySqlJdbcContext context = new MySqlJdbcContext(new MySqlConnectorConfig(config))) {
            ...
            if (serverUuid != null && !serverUuid.trim().isEmpty()) {
                ...
                if (availableServerGtidStr != null && !availableServerGtidStr.trim().isEmpty()) {
                    ...
                    Collection<String> uuids = gtidSet.getUUIDSets().stream().map(UUIDSet::getUUID)
                    ...
                    if (uuids.size() == 1) {
                        ...
                        logger.warn("More than 2 GTID sources were found, so unable to determine master UUID: {}",
                                gtidSet);
                    }
                }
            }
