...
public class MySqlConnectorRegressionIT extends AbstractConnectorTest {
    ...
    private static final Path DB_HISTORY_PATH = Testing.Files.createTestingPath("file-db-history-regression.txt")
            .toAbsolutePath();
    ...
    public void shouldConsumeAllEventsFromDatabaseUsingBinlogAndNoSnapshot() throws SQLException, InterruptedException {
        ...
        config = DATABASE.defaultConfig().with(MySqlConnectorConfig.INCLUDE_SCHEMA_CHANGES, true)
                ...
                .with("database.serverTimezone", DATABASE.timezone()).build();
        // Start the connector ...
        ...
        SourceRecords records = consumeRecordsByTopic(
                numCreateDatabase + numCreateTables + numDataRecords + numCreateDefiner);
        stopConnector();
        ...
        records.forEach(record -> {
            ...
            if (record.topic().endsWith("dbz_100_enumsettest")) {
                ...
                // c4 TIMESTAMP(2)
                //
                // {"c1" : "16321", "c2" : "17:51:04.777", "c3" : "1410198664780", "c4" :
                // "2014-09-08T17:51:04.78-05:00"}
                ...
                LocalDateTime c3DateTime = LocalDateTime.ofEpochSecond(c3Seconds,
                        (int) TimeUnit.MILLISECONDS.toNanos(c3Millis), ZoneOffset.UTC);
                ...
                // c4 TIMESTAMP(2)
                //
                // INSERT IGNORE INTO dbz_114_zerovaluetest VALUES ('0000-00-00', '00:00:00.000', '0000-00-00
                // 00:00:00.000',
                // '0000-00-00 00:00:00.000');
                // INSERT IGNORE INTO dbz_114_zerovaluetest VALUES ('0001-00-00', '00:01:00.000', '0001-00-00
                // 00:00:00.000',
                ...
                assertThat(c5Time)
                        .isEqualTo(Duration.ofHours(-838).minusMinutes(59).minusSeconds(58).minusNanos(999999000));
            }
        });

        final Struct rec1 = ((Struct) records.recordsForTopic(DATABASE.topicForTable("dbz_1318_zerovaluetest")).get(0)
                .value()).getStruct("after");
        final Struct rec2 = ((Struct) records.recordsForTopic(DATABASE.topicForTable("dbz_1318_zerovaluetest")).get(1)
                .value()).getStruct("after");
    ...
    @Test
    @FixFor("DBZ-61")
    public void shouldConsumeAllEventsFromDatabaseUsingBinlogAndNoSnapshotAndConnectTimesTypes()
        ...
        records.forEach(record -> {
            ...
            if (record.topic().endsWith("dbz_100_enumsettest")) {
                ...
                assertThat(c2.toInstant()).isEqualTo(
                        LocalDateTime.of(1970, 1, 1, 17, 51, 4, 780_000_000).atOffset(ZoneOffset.UTC).toInstant());
    ...
    public void shouldConsumeAllEventsFromDatabaseUsingSnapshot() throws SQLException, InterruptedException {
        ...
        int numTables = 12;
        int numDataRecords = 22;
        int numDdlRecords = numTables * 2 + 3; // for each table (1 drop + 1 create) + for each db (1 create + 1 drop +
                                               // 1 use)
        int numCreateDefiner = 1;
        ...
        records.forEach(record -> {
            ...
            if (record.topic().endsWith("dbz_100_enumsettest")) {
                ...
                OffsetDateTime expected = ZonedDateTime
                        .of(LocalDateTime.of(2014, 9, 8, 17, 51, 4, (int) TimeUnit.MILLISECONDS.toNanos(780)),
        ...
        try (MySQLConnection conn = MySQLConnection.forTestDatabase(DATABASE.getDatabaseName());) {
            final Struct rec1 = ((Struct) records.recordsForTopic(DATABASE.topicForTable("dbz_1318_zerovaluetest"))
                    .get(0).value()).getStruct("after");
            final Struct rec2 = ((Struct) records.recordsForTopic(DATABASE.topicForTable("dbz_1318_zerovaluetest"))
                    .get(1).value()).getStruct("after");
            assertThat(rec1.get("c1")).isNull();
            assertThat(rec1.get("c2")).isEqualTo(0L);
            assertThat(rec1.get("c3")).isNull();
            assertThat(rec1.get("c4"))
                    .isEqualTo(conn.databaseAsserts().currentDateTimeDefaultOptional("1970-01-01T00:00:00Z"));
            assertThat(rec1.get("nnc1")).isEqualTo(0);
            assertThat(rec1.get("nnc2")).isEqualTo(0L);
            assertThat(rec1.get("nnc3")).isEqualTo(0L);
            assertThat(rec2.get("c1")).isNull();
    ...
    @Test
    public void shouldConsumeDatesCorrectlyWhenClientTimezonePrecedesServerTimezoneUsingSnapshot()
            throws SQLException, InterruptedException {
    ...
    public void shouldConsumeAllEventsFromDecimalTableInDatabaseUsingBinlogAndNoSnapshot()
        ...
        config = DATABASE.defaultConfig()
                ...
                .with(MySqlConnectorConfig.DECIMAL_HANDLING_MODE, DecimalHandlingMode.DOUBLE).build();
        // Start the connector ...
        start(MySqlConnector.class, config);
        ...
        assertThat(records).isNotNull();
        assertThat(records.recordsForTopic(DATABASE.getServerName()).size())
                .isEqualTo(numCreateDatabase + numCreateTables);
        assertThat(records.recordsForTopic(DATABASE.topicForTable("dbz_147_decimalvalues")).size()).isEqualTo(1);
    ...
    public void shouldConsumeDecimalAsStringFromBinlog() throws SQLException, InterruptedException {
        ...
        config = DATABASE.defaultConfig()
                ...
                .with(MySqlConnectorConfig.INCLUDE_SCHEMA_CHANGES, true)
                .with(MySqlConnectorConfig.SNAPSHOT_MODE, SnapshotMode.NEVER.toString())
                .with(MySqlConnectorConfig.DECIMAL_HANDLING_MODE, DecimalHandlingMode.STRING).build();
        // Start the connector ...
        start(MySqlConnector.class, config);
