...
/**
 * The test is named to make sure it runs alphabetically last as it can influence execution of for example
 * {@link BinlogReaderIT}
 *
...
public class ZZZGtidSetIT extends AbstractConnectorTest {
    ...
    public void shouldProcessPurgedGtidSet() throws SQLException, InterruptedException {
        ...
        config = ro_database.defaultConfig().with(MySqlConnectorConfig.SNAPSHOT_MODE, SnapshotMode.NEVER)
                ...
                .with(MySqlConnectorConfig.TABLE_WHITELIST, ro_database.qualifiedTableName("customers")).build();
    ...
    public void shouldProcessPurgedLogsWhenDownAndSnapshotNeeded() throws SQLException, InterruptedException {
        ...
        // Use the DB configuration to define the connector's configuration ...
        config = database.defaultConfig().with(MySqlConnectorConfig.SNAPSHOT_MODE, SnapshotMode.WHEN_NEEDED)
                .with(MySqlConnectorConfig.INCLUDE_SCHEMA_CHANGES, true)
                .with(MySqlConnectorConfig.TABLE_WHITELIST, database.qualifiedTableName("customers")).build();
        ...
        // Consume the first records due to startup and initialization of the database ...
        // Testing.Print.enable();
        SourceRecords records = consumeRecordsByTopic(1 + 3 + 2 * 4 + 4); // SET + DROP/CREATE/USE DB + DROP/CREATE 4
                                                                          // tables + 4 data
