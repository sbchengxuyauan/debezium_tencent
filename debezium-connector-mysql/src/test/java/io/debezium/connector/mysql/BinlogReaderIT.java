...
public class BinlogReaderIT {
    ...
    private static final Path DB_HISTORY_PATH = Testing.Files.createTestingPath("file-db-history-binlog.txt")
            .toAbsolutePath();
    ...
    protected long filterAtLeast(final int minNumber, final long timeout, final TimeUnit unit)
        ...
        long startTime = System.currentTimeMillis();
        while (metrics.getNumberOfEventsFiltered() < targetNumber
    ...
    protected Configuration.Builder simpleConfig() {
        return DATABASE.defaultConfig().with(MySqlConnectorConfig.USER, "replicator")
    ...
    public void shouldCreateSnapshotOfSingleDatabaseWithSchemaChanges() throws Exception {
        ...
        int expected = (9 + 9 + 4 + 5 + 1) + expectedSchemaChangeCount; // only the inserts for our 4 tables in this
                                                                        // database, plus
        // schema changes
        int consumed = consumeAtLeast(expected);
        assertThat(consumed).isGreaterThanOrEqualTo(expected);
        ...
        final List<String> expectedAffectedTables = Arrays.asList(null, // CREATE DATABASE
    ...
    /**
     * Setup a DATABASE_WHITELIST filter that filters all events. Verify all events are properly filtered. Verify
     * numberOfFilteredEvents metric is incremented correctly.
     */
    @Test
    @FixFor("DBZ-1206")
    ...
    public void shouldHandleTimestampTimezones() throws Exception {
        ...
        config = simpleConfig().with(MySqlConnectorConfig.INCLUDE_SCHEMA_CHANGES, false)
                ...
                .with(MySqlConnectorConfig.TABLE_WHITELIST, REGRESSION_DATABASE.qualifiedTableName(tableName)).build();
        Filters filters = new Filters.Builder(config).build();
        context = new MySqlTaskContext(config, filters);
        context.start();
        context.source().setBinlogStartPoint("", 0L); // start from beginning
        ...
        ZonedDateTime expectedTimestamp = ZonedDateTime
                .of(LocalDateTime.parse("2014-09-08T17:51:04.780"), UniqueDatabase.TIMEZONE)
    ...
    public void shouldFailOnUnknownTlsProtocol() {
        ...
        config = simpleConfig().with(MySqlConnectorConfig.SSL_MODE, SecureConnectionMode.REQUIRED)
                .with(SET_TLS_PROTOCOLS, "TLSv1.7").build();
        Filters filters = new Filters.Builder(config).build();
        context = new MySqlTaskContext(config, filters);
        context.start();
        context.source().setBinlogStartPoint("", 0L); // start from beginning
    ...
    public void shouldAcceptTls12() {
        ...
        config = simpleConfig().with(MySqlConnectorConfig.SSL_MODE, SecureConnectionMode.REQUIRED)
                .with(SET_TLS_PROTOCOLS, "TLSv1.2").build();
        Filters filters = new Filters.Builder(config).build();
        context = new MySqlTaskContext(config, filters);
        context.start();
        context.source().setBinlogStartPoint("", 0L); // start from beginning
    ...
    private void inconsistentSchema(EventProcessingFailureHandlingMode mode) throws InterruptedException, SQLException {
        ...
        try (final MySQLConnection db = MySQLConnection.forTestDatabase(DATABASE.getDatabaseName());
                final JdbcConnection connection = db.connect(); final Connection jdbc = connection.connection();
