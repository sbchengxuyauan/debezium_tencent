...
public class MySqlSourceTypeInSchemaIT extends AbstractConnectorTest {
    ...
    private static final Path DB_HISTORY_PATH = Testing.Files.createTestingPath("file-db-history-schema-parameter.txt")
            .toAbsolutePath();
    private final UniqueDatabase DATABASE = new UniqueDatabase("schemaparameterit",
            "source_type_as_schema_parameter_test").withDbHistoryPath(DB_HISTORY_PATH);
    ...
    public void shouldPropagateSourceTypeAsSchemaParameter() throws SQLException, InterruptedException {
        ...
        config = DATABASE.defaultConfig()
                ...
                .with("column.propagate.source.type", ".*c1,.*c2,.*c3.*,.*f.").build();
        ...
        List<SourceRecord> dmls = records
                .recordsForTopic(DATABASE.topicForTable("dbz_644_source_type_mapped_as_schema_parameter_test"));
        ...
        assertThat(c3aSchemaParameters).includes(entry(TYPE_NAME_PARAMETER_KEY, "NUMERIC"),
                entry(TYPE_LENGTH_PARAMETER_KEY, "5"), entry(TYPE_SCALE_PARAMETER_KEY, "2"));

        // variable width, name and length info
        Map<String, String> c3bSchemaParameters = before.schema().field("c3b").schema().parameters();

        assertThat(c3bSchemaParameters).includes(entry(TYPE_NAME_PARAMETER_KEY, "VARCHAR"),
                entry(TYPE_LENGTH_PARAMETER_KEY, "128"));

        // float info
        Map<String, String> f1SchemaParameters = before.schema().field("f1").schema().parameters();

        assertThat(f1SchemaParameters).includes(entry(TYPE_NAME_PARAMETER_KEY, "FLOAT"),
                entry(TYPE_LENGTH_PARAMETER_KEY, "10"));

        Map<String, String> f2SchemaParameters = before.schema().field("f2").schema().parameters();

        assertThat(f2SchemaParameters).includes(entry(TYPE_NAME_PARAMETER_KEY, "FLOAT"),
                entry(TYPE_LENGTH_PARAMETER_KEY, "8"), entry(TYPE_SCALE_PARAMETER_KEY, "4"));
    }
    ...
    public void shouldPropagateSourceTypeByDatatype() throws SQLException, InterruptedException {
        ...
        config = DATABASE.defaultConfig()
                .with(MySqlConnectorConfig.SNAPSHOT_MODE, MySqlConnectorConfig.SnapshotMode.NEVER)
                .with("datatype.propagate.source.type", ".+\\.FLOAT,.+\\.VARCHAR").build();
