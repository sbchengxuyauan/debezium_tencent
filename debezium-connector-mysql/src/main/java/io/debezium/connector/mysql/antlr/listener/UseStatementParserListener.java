...
/**
 * Parser listener that is parsing MySQL USE statements that changes current database/schema on which all changes are
 * applied.
 *
...
public class UseStatementParserListener extends MySqlParserBaseListener {
    ...
    public void enterUseStatement(MySqlParser.UseStatementContext ctx) {
        ...
        // Every time MySQL switches to a different database, it sets the "character_set_database" and
        // "collation_database"
        // system variables. We replicate that behavior here (or the variable we care about) so that these variables are
        // always
        // right for the current database.
        String charsetForDb = parser.charsetNameForDatabase().get(dbName);
        parser.systemVariables().setVariable(MySqlSystemVariables.MySqlScope.SESSION,
                MySqlSystemVariables.CHARSET_NAME_DATABASE, charsetForDb);
