...
/**
 * A {@link Reader} implementation that runs one or more other {@link Reader}s in a consistently, completely, and
 * sequentially. This reader ensures that all records generated by one of its contained {@link Reader}s are all passed
 * through to callers via {@link #poll() polling} before the next reader is started. And, when this reader is
 * {@link #stop() stopped}, this class ensures that current reader is stopped and that no additional readers will be
 * started.
 *
...
public final class ChainedReader implements Reader {
    ...
    /**
     * Called when the previously-started reader has returned all of its records via {@link #poll() polling}. Only when
     * this method is called is the now-completed reader removed as the current reader, and this is what guarantees that
     * all records produced by the now-completed reader have been polled.
     */
