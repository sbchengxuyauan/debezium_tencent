...
public class MySqlJdbcContext implements AutoCloseable {
    ...
    public MySqlJdbcContext(MySqlConnectorConfig config) {
        ...
        // to give us better JDBC database metadata behavior, including using UTF-8 for the client-side character
        // encoding
        ...
        Configuration jdbcConfig = this.config
                ...
                .edit().withDefault(MySqlConnectorConfig.PORT, MySqlConnectorConfig.PORT.defaultValue()).build()
                .subset("database.", true);

        Builder jdbcConfigBuilder = jdbcConfig.edit()
    ...
    public GtidSet subtractGtidSet(GtidSet set1, GtidSet set2) {
        try {
            return jdbc.prepareQueryAndMap("SELECT GTID_SUBTRACT(?, ?)", ps -> {
     ...
     *
     * @param grantName
     *            the name of the MySQL privilege; may not be null
    ...
    public boolean userHasPrivileges(String grantName) {
        ...
        try {
            ...
            throw new ConnectException(
                    "Unexpected error while connecting to MySQL and looking at privileges for current user: ", e);
        }
        return result.get();
    }
            ...
            if (existingValue == null) {
                ...
                if (!existingValue.equalsIgnoreCase(value)) {
                    ...
                    String msg = "System or JVM property '" + property
                            + "' is already defined, but the configuration property '" + field.name()
                            + "' defines a different value";
                    if (showValueInError) {
                        msg = "System or JVM property '" + property + "' is already defined as " + existingValue
                                + ", but the configuration property '" + field.name() + "' defines a different value '"
                                + value + "'";
                    }
