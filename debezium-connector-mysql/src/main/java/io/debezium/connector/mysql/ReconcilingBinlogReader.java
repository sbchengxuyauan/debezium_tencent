 ...
 *
 * To do this, at start time we evaluate the (now completed) states of the two binlog readers we want to unify, and
 * create a new {@link BinlogReader} duplicating the lagging reader, but with a halting predicate that will halt it once
 * it has passed the final position of the leading reader.
 */
public class ReconcilingBinlogReader implements Reader {
     ...
     *
     * @param binlogReaderA
     *            the first binlog reader to unify.
     * @param binlogReaderB
     *            the second binlog reader to unify.
     * @param unifiedReader
     *            the final, unified binlog reader that will run once the reconciliation is complete.
     */
    public ReconcilingBinlogReader(BinlogReader binlogReaderA, BinlogReader binlogReaderB, BinlogReader unifiedReader,
            long serverId) {
    ...
    public void start() {
        if (running.compareAndSet(false, true)) {
            ...
            reconcilingReader = new BinlogReader("innerReconcilingReader", laggingReaderContext, offsetLimitPredicate,
                    serverId);
    ...
    private void setupUnifiedReader() {
        ...
        Map<String, ?> keyedOffset = reconcilingReader.getLastOffset() == null ? getLeadingReader().getLastOffset()
                : reconcilingReader.getLastOffset();
        unifiedReader.context.source().setCompletedGtidSet((String) keyedOffset.get(GTID_SET_KEY));
        unifiedReader.context.source().setBinlogStartPoint((String) keyedOffset.get(BINLOG_FILENAME_OFFSET_KEY),
    ...
    private void determineLeadingReader() {
        ...
        if (noOffsets || aNotStopped || bNotStopped) {
            throw new IllegalStateException(
                    "Cannot determine leading reader until both source readers have completed.");
        }
        ...
        if (aOffset == null) {
            ...
            aReaderLeading = SourceInfo.isPositionAtOrBefore(bDocument, aDocument,
    ...
    /* package private */ static class OffsetLimitPredicate implements HaltingPredicate {
        ...
        public boolean accepts(SourceRecord sourceRecord) {
            ...
            return !SourceInfo.isPositionAtOrBefore(leadingReaderFinalOffsetDocument, offsetDocument, gtidFilter);
        }
    }
}
