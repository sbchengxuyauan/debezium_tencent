...
public interface BinlogReaderMetricsMXBean extends StreamingChangeEventSourceMetricsMXBean {
    ...
    /**
     * Tracks the number of events skipped by underlying mysql-binlog-client, generally due to the client being unable
     * to properly deserialize the event.
     */
    ...
    /**
     * Tracks the number of transactions which are not well-formed. Example - The connector sees a commit TX event
     * without a matched begin TX event.
     */
    long getNumberOfNotWellFormedTransactions();

    /**
     * Tracks the number of transaction which contains events that contained more entries than could be contained within
     * the connectors {@see io.debezium.connector.mysql.EventBuffer} instance.
     */
