...
public class AlterTableParserListener extends MySqlParserBaseListener {
    ...
    public void enterAlterTable(MySqlParser.AlterTableContext ctx) {
        ...
        if (tableEditor == null) {
            throw new ParsingException(null,
                    "Trying to alter table " + tableId.toString() + ", which does not exist. Query: " + getText(ctx));
        }
    ...
    public void enterAlterByAddColumn(MySqlParser.AlterByAddColumnContext ctx) {
        parser.runIfNotNull(() -> {
            ...
            columnDefinitionListener = new ColumnDefinitionParserListener(tableEditor, columnEditor,
                    parser.dataTypeResolver(), parser.getConverters());
    ...
    public void enterAlterByAddColumns(MySqlParser.AlterByAddColumnsContext ctx) {
        ...
        parser.runIfNotNull(() -> {
            ...
            columnDefinitionListener = new ColumnDefinitionParserListener(tableEditor, columnEditors.get(0),
                    parser.dataTypeResolver(), parser.getConverters());
            listeners.add(columnDefinitionListener);
        }, tableEditor);
    ...
    public void enterAlterByChangeColumn(MySqlParser.AlterByChangeColumnContext ctx) {
        parser.runIfNotNull(() -> {
            ...
            if (existingColumn != null) {
                // DBZ-771 unset previously set default value, as it's not kept by MySQL; for any column modifications a
                // new
                // default value (which could be the same) has to be provided by the column_definition which we'll parse
                // later
                // on; only in 8.0 (not yet supported by this parser) columns can be renamed without repeating the full
                // column
                // definition; so in fact it's arguably not correct to use edit() on the existing column to begin with,
                // but
                // I'm going to leave this as is for now, to be prepared for the ability of updating column definitions
                // in 8.0
