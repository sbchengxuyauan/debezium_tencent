...
/**
 * Component that records the schema history for databases hosted by a MySQL database server. The schema information
 * includes the {@link Tables table definitions} and the Kafka Connect {@link #schemaFor(TableId) Schema}s for each
 * table, where the {@link Schema} excludes any columns that have been {@link MySqlConnectorConfig#COLUMN_BLACKLIST
 * specified} in the configuration.
 * <p>
 * The history is changed by {@link #applyDdl(SourceInfo, String, String, DatabaseStatementStringConsumer) applying DDL
 * statements}, and every change is {@link DatabaseHistory persisted} as defined in the supplied
 * {@link MySqlConnectorConfig MySQL connector configuration}. This component can be reconstructed (e.g., on connector
 * restart) and the history {@link #loadHistory(SourceInfo) loaded} from persisted storage.
 * <p>
 * Note that when {@link #applyDdl(SourceInfo, String, String, DatabaseStatementStringConsumer) applying DDL
 * statements}, the caller is able to supply a {@link DatabaseStatementStringConsumer consumer function} that will be
 * called with the DDL statements and the database to which they apply, grouped by database names. However, these will
 * only be called based when the databases are included by the database filters defined in the
 * {@link MySqlConnectorConfig MySQL connector configuration}.
 *
...
public class MySqlSchema extends RelationalDatabaseSchema {
     ...
     *
     * @param configuration
     *            the connector configuration, which is presumed to be valid
     * @param gtidFilter
     *            the predicate function that should be applied to GTID sets in database history, and which returns
     *            {@code true} if a GTID source is to be included, or {@code false} if a GTID source is to be excluded;
     *            may be null if not needed
     * @param tableIdCaseInsensitive
     *            true if table lookup ignores letter case
     */
    public MySqlSchema(MySqlConnectorConfig configuration, Predicate<String> gtidFilter, boolean tableIdCaseInsensitive,
            TopicSelector<TableId> topicSelector, Filters tableFilters) {
        super(configuration, topicSelector, TableFilter.fromPredicate(tableFilters.tableFilter()),
                tableFilters.columnFilter(),
                new TableSchemaBuilder(getValueConverters(configuration), SchemaNameAdjuster.create(logger),
                        configuration.customConverterRegistry(), configuration.getSourceInfoStructMaker().schema(),
                        configuration.getSanitizeFieldNames()),
                tableIdCaseInsensitive, configuration.getKeyMapper());
        ...
        Configuration dbHistoryConfig = config.subset(DatabaseHistory.CONFIGURATION_FIELD_PREFIX_STRING, false).edit()
                .withDefault(DatabaseHistory.NAME, connectorName + "-dbhistory").build();
    ...
    private static MySqlValueConverters getValueConverters(MySqlConnectorConfig configuration) {
        ...
        BigIntUnsignedHandlingMode bigIntUnsignedHandlingMode = BigIntUnsignedHandlingMode
                .parse(bigIntUnsignedHandlingModeStr);
        BigIntUnsignedMode bigIntUnsignedMode = bigIntUnsignedHandlingMode.asBigIntUnsignedMode();

        final boolean timeAdjusterEnabled = configuration.getConfig()
     ...
     *
     * @param id
     *            the fully-qualified table identifier; may be null
     ...
     *
     * @param startingPoint
     *            the source information with the current {@link SourceInfo#partition()} and {@link SourceInfo#offset()
    ...
    /**
     * Apply the supplied DDL statements to this database schema and record the history. If a {@code statementConsumer}
     * is supplied, then call it for each sub-sequence of the DDL statements that all apply to the same database.
     * <p>
     * Typically DDL statements are applied using a connection to a single database, and unless the statements use
     * fully-qualified names, the DDL statements apply to this database.
     *
     * @param source
     *            the current {@link SourceInfo#partition()} and {@link SourceInfo#offset() offset} at which these
     *            changes are found; may not be null
     * @param databaseName
     *            the name of the default database under which these statements are applied; may not be null
     * @param ddlStatements
     *            the {@code ;}-separated DDL statements; may be null or empty
     * @param statementConsumer
     *            the consumer that should be called with each sub-sequence of DDL statements that apply to a single
     *            database; may be null if no action is to be performed with the changes
        ...
        if (!storeOnlyMonitoredTablesDdl || ddlChanges.anyMatch(filters.databaseFilter(), filters.tableFilter())) {
            if (statementConsumer != null) {
                ...
                if (!ddlChanges.isEmpty() && ddlChanges.applyToMoreDatabasesThan(databaseName)) {
                    ...
                    // We understood at least some of the DDL statements and can figure out to which database they
                    // apply.
                ...
                } else if (filters.databaseFilter().test(databaseName) || databaseName == null
                        || "".equals(databaseName)) {
            ...
            if (!storeOnlyMonitoredTablesDdl || isGlobalSetVariableStatement(ddlStatements, databaseName)
                    || changes.stream().anyMatch(filters().tableFilter()::test)) {
