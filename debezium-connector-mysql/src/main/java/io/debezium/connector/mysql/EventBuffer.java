...
/**
 * This class represents a look-ahead buffer that allows Debezium to accumulate binlog events and decide if the last
 * event in transaction is either {@code ROLLBACK} or {@code COMMIT}. The incoming events are either supposed to be in
 * transaction or out-of-transaction. When out-of-transaction they are sent directly into the destination handler. When
 * in transaction the event goes through the buffering.
 * <p>
 * The reason for the buffering is that the binlog contains rolled back transactions in some cases. E.g. that's the case
 * when a temporary table is dropped (see DBZ-390). For rolled back transactions we may not propagate any of the
 * contained events, hence the buffering is applied.
 * <p>
 * The transaction start is identified by a {@code BEGIN} event. Transaction is ended either by {@code COMMIT} event or
 * by {@code XID} an event.
 * <p>
...
class EventBuffer {
    ...
    /**
     * Adds an event to the buffer if there is a space available. Records binlog position for the first event that does
     * not fit for later replay.
     *
     * @param event
     */
    ...
    private void switchToBufferFullMode() {
        ...
        LOGGER.info("Buffer full, will need to re-read part of the transaction from binlog from {}",
    ...
    /**
     * Sends all events from the buffer int a final handler. For large transactions it executes rewind of binlog reader
     * back to the first event that was not stored in the buffer.
     *
     * @param wellFormed
     * @param event
     */
