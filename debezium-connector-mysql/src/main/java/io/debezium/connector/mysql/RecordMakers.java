...
public class RecordMakers {
     ...
     *
     * @param schema
     *            the schema information about the MySQL server databases; may not be null
     * @param source
     *            the connector's source information; may not be null
     * @param topicSelector
     *            the selector for topic names; may not be null
     * @param emitTombstoneOnDelete
     *            whether to emit a tombstone message upon DELETE events or not
     * @param restartOffset
     *            the offset to publish with the {@link SourceInfo#RESTART_PREFIX} prefix as additional information in
     *            the offset. If the connector attempts to restart from an offset with information with this prefix it
     *            will create an offset from the prefixed information rather than restarting from the base offset.
    ...
    public RecordMakers(MySqlSchema schema, SourceInfo source, TopicSelector<TableId> topicSelector,
        ...
        this.schemaChangeKeySchema = SchemaBuilder.struct()
                ...
                .field(Fields.DATABASE_NAME, Schema.STRING_SCHEMA).build();
        this.schemaChangeValueSchema = SchemaBuilder.struct()
                ...
                .field(Fields.SOURCE, source.schema()).field(Fields.DATABASE_NAME, Schema.STRING_SCHEMA)
                .field(Fields.DDL_STATEMENTS, Schema.STRING_SCHEMA).build();
    }

    /**
     * Obtain the record maker for the given table, using the specified columns and sending records to the given
     * consumer.
     *
     * @param tableId
     *            the identifier of the table for which records are to be produced
     * @param includedColumns
     *            the set of columns that will be included in each row; may be null if all columns are included
     * @param consumer
     *            the consumer for all produced records; may not be null
     ...
     * @return {@code true} if there is a {@link #forTable(TableId, BitSet, BlockingConsumer) record maker}, or
     ...
     *
     * @param tableNumber
     *            the {@link #assign(long, TableId) assigned table number} for which records are to be produced
     * @param includedColumns
     *            the set of columns that will be included in each row; may be null if all columns are included
     * @param consumer
     *            the consumer for all produced records; may not be null
     ...
     *
     * @param databaseName
     *            the name of the database that is affected by the DDL statements; may not be null
     * @param tables
     *            the list of tables affected by the DDL statements
     * @param ddlStatements
     *            the DDL statements; may not be null
     * @param consumer
     *            the consumer for all produced records; may not be null
     * @return the number of records produced; will be 0 or more
     */
    public int schemaChanges(String databaseName, Set<TableId> tables, String ddlStatements,
        ...
        SourceRecord record = new SourceRecord(source.partition(), source.offset(), topicName, partition,
    ...
    /**
     * Clear all of the cached record makers. This should be done when the logs are rotated, since in that a different
     * table numbering scheme will be used by all subsequent TABLE_MAP binlog events.
     */
    public void clear() {
    ...
    /**
     * Clear all of the cached record makers and generate new ones. This should be done when the schema changes for
     * reasons other than reading DDL from the binlog.
     */
    ...
    private Map<String, ?> getSourceRecordOffset(Map<String, Object> sourceOffset) {
        if (restartOffset == null) {
            ...
            for (Entry<String, ?> restartOffsetEntry : restartOffset.entrySet()) {
                sourceOffset.put(SourceInfo.RESTART_PREFIX + restartOffsetEntry.getKey(),
                        restartOffsetEntry.getValue());
            }
     ...
     *
     * @param tableNumber
     *            the table number found in binlog events
     * @param id
     *            the identifier for the corresponding table
    ...
    public boolean assign(long tableNumber, TableId id) {
        ...
        Converter converter = new Converter() {
            ...
            @Override
            public int read(SourceInfo source, Object[] row, int rowNumber, int numberOfRows, BitSet includedColumns,
                    Instant ts, BlockingConsumer<SourceRecord> consumer) throws InterruptedException {
                ...
                if (value != null || key != null) {
                    ...
                    SourceRecord record = new SourceRecord(partition, getSourceRecordOffset(offset), topicName,
                            partitionNum, keySchema, key, envelope.schema(), envelope.read(value, origin, ts));
                    consumer.accept(record);
                    return 1;
                }
                return 0;
            ...
            @Override
            public int insert(SourceInfo source, Object[] row, int rowNumber, int numberOfRows, BitSet includedColumns,
                    Instant ts, BlockingConsumer<SourceRecord> consumer) throws InterruptedException {
                ...
                if (value != null || key != null) {
                    ...
                    Struct origin = source.struct();
                    SourceRecord record = new SourceRecord(partition, getSourceRecordOffset(offset), topicName,
                            partitionNum, keySchema, key, envelope.schema(), envelope.create(value, origin, ts));
                    consumer.accept(record);
                    return 1;
                }
                return 0;
            ...
            @Override
            public int update(SourceInfo source, Object[] before, Object[] after, int rowNumber, int numberOfRows,
                    BitSet includedColumns, Instant ts, BlockingConsumer<SourceRecord> consumer)
                ...
                if (valueAfter != null || key != null) {
                    ...
                    if (key != null && !Objects.equals(key, oldKey)) {
                        ...
                        // Consumers may push the events into a system that won't allow both records to exist at the
                        // same time,
                        // so we first want to send the delete event for the old key...
                        SourceRecord record = new SourceRecord(partition, getSourceRecordOffset(offset), topicName,
                                partitionNum, keySchema, oldKey, envelope.schema(),
                                envelope.delete(valueBefore, origin, ts));
                        consumer.accept(record);
                        ++count;
                        ...
                        SourceRecord record = new SourceRecord(partition, getSourceRecordOffset(offset), topicName,
                                partitionNum, keySchema, key, envelope.schema(),
            ...
            @Override
            public int delete(SourceInfo source, Object[] row, int rowNumber, int numberOfRows, BitSet includedColumns,
                    Instant ts, BlockingConsumer<SourceRecord> consumer) throws InterruptedException {
                int count = 0;
                validateColumnCount(tableSchema, row);
                Object key = tableSchema.keyFromColumnData(row);
                ...
                if (value != null || key != null) {
                    ...
                    SourceRecord record = new SourceRecord(partition, getSourceRecordOffset(offset), topicName,
                            partitionNum, keySchema, key, envelope.schema(), envelope.delete(value, origin, ts));
                    consumer.accept(record);
                    ++count;
            ...
            private void validateColumnCount(TableSchema tableSchema, Object[] row) {
                ...
                if (expectedColumnsCount != row.length) {
                    logger.error("Invalid number of columns, expected '{}' arrived '{}'", expectedColumnsCount,
                            row.length);
    ...
    public final class RecordsForTable {
         ...
         *
         * @param row
         *            the values of the row, in the same order as the columns in the {@link Table} definition in the
         *            {@link MySqlSchema}.
         * @param ts
         *            the timestamp for this row
         * @return the number of records produced; will be 0 or more
         * @throws InterruptedException
         ...
         *            the timestamp for this row
         * @param rowNumber
         *            the number of this row; must be 0 or more
         * @param numberOfRows
         *            the total number of rows to be read; must be 1 or more
         * @return the number of records produced; will be 0 or more
         * @throws InterruptedException
         *             if this thread is interrupted while waiting to give a source record to the consumer
         */
         ...
         *
         * @param before
         *            the values of the row <i>before</i> the update, in the same order as the columns in the
         *            {@link Table} definition in the {@link MySqlSchema}
         * @param after
         *            the values of the row <i>after</i> the update, in the same order as the columns in the
         *            {@link Table} definition in the {@link MySqlSchema}
         * @param ts
         *            the timestamp for this row
         * @return the number of records produced; will be 0 or more
