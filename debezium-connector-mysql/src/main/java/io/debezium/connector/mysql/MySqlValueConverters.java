 ...
 * {@link Types#TIMESTAMP_WITH_TIMEZONE} type, the converters will need to convert that {@link java.sql.Timestamp} value
 * into an {@link OffsetDateTime} using the default time zone, which always is UTC.
 *
...
public class MySqlValueConverters extends JdbcValueConverters {
    ...
    /**
     * A utility method that adjusts
     * <a href="https://dev.mysql.com/doc/refman/5.7/en/two-digit-years.html">ambiguous</a> 2-digit year values of
     * DATETIME, DATE, and TIMESTAMP types using these MySQL-specific rules:
     * <ul>
     ...
     *
     * @param temporal
     *            the temporal instance to adjust; may not be null
     ...
     */
    public MySqlValueConverters(DecimalMode decimalMode, TemporalPrecisionMode temporalPrecisionMode,
            ZoneOffset defaultOffset, BigIntUnsignedMode bigIntUnsignedMode, TemporalAdjuster adjuster) {
    ...
    public SchemaBuilder schemaBuilder(Column column) {
        ...
        if (matches(typeName, "GEOMETRY") || matches(typeName, "LINESTRING") || matches(typeName, "POLYGON")
                || matches(typeName, "MULTIPOINT") || matches(typeName, "MULTILINESTRING")
                || matches(typeName, "MULTIPOLYGON") || isGeometryCollection(typeName)) {
        ...
        if (matches(typeName, "INT UNSIGNED") || matches(typeName, "INT UNSIGNED ZEROFILL")) {
            // In order to capture unsigned INT 32-bit data source, INT64 will be required to safely capture all valid
            // values
            // Source: https://kafka.apache.org/0102/javadoc/org/apache/kafka/connect/data/Schema.Type.html
        ...
        if (matches(typeName, "BIGINT UNSIGNED") || matches(typeName, "BIGINT UNSIGNED ZEROFILL")) {
            ...
            case LONG:
                return SchemaBuilder.int64();
            case PRECISE:
                // In order to capture unsigned INT 64-bit data source, org.apache.kafka.connect.data.Decimal:Byte will
                // be required to safely capture all valid values with scale of 0
                // Source: https://kafka.apache.org/0102/javadoc/org/apache/kafka/connect/data/Schema.Type.html
     ...
     *
     * @param column
     *            the column in which the character set is used; never null
     ...
     *
     * @param column
     *            the column in which the value appears
     * @param fieldDefn
     *            the field definition for the {@link SourceRecord}'s {@link Schema}; never null
     * @param data
    ...
    protected Object convertJson(Column column, Field fieldDefn, Object data) {
        return convertValue(column, fieldDefn, data, "{}", (r) -> {
            if (data instanceof byte[]) {
                // The BinlogReader sees these JSON values as binary encoded, so we use the binlog client library's
                // utility
                ...
                if (((byte[]) data).length == 0) {
                    ...
                    try {
                        ...
                        throw new ConnectException(
                                "Failed to parse and read a JSON value on " + column + ": " + e.getMessage(), e);
                    }
                }
     ...
     *            the field definition for the {@link SourceRecord}'s {@link Schema}; never null
     * @param columnCharset
     *            the Java character set in which column byte[] values are encoded; may not be null
     * @param data
     *            the data; may be null
     * @return the converted value, or null if the conversion could not be made and the column allows nulls
     * @throws IllegalArgumentException
     ...
     * @param data
     *            the data object to be converted into a year literal integer value; never null
     * @return the converted value, or null if the conversion could not be made and the column allows nulls
     * @throws IllegalArgumentException
     *             if the value could not be converted but the column does not allow nulls
     */
     ...
     * containing the index of the enum option. The MySQL JDBC driver returns a string containing the option, so this
     * method calculates the same.
     *
     * @param options
     *            the characters that appear in the same order as defined in the column; may not be null
     * @param column
     *            the column definition describing the {@code data} value; never null
     * @param fieldDefn
     *            the field definition; never null
     * @param data
     *            the data object to be converted into an {@code ENUM} literal String value
     * @return the converted value, or null if the conversion could not be made and the column allows nulls
     * @throws IllegalArgumentException
     *             if the value could not be converted but the column does not allow nulls
     */
     ...
     *            the field definition; never null
     * @param data
     *            the data object to be converted into an {@code SET} literal String value; never null
     * @return the converted value, or null if the conversion could not be made and the column allows nulls
     * @throws IllegalArgumentException
     *             if the value could not be converted but the column does not allow nulls
     */
    ...
    /**
     * Determine if the uppercase form of a column's type exactly matches or begins with the specified prefix. Note that
     * this logic works when the column's {@link Column#typeName() type} contains the type name followed by parentheses.
     *
     * @param upperCaseTypeName
     *            the upper case form of the column's {@link Column#typeName() type name}
     * @param upperCaseMatch
     *            the upper case form of the expected type or prefix of the type; may not be null
    ...
    protected Object convertPoint(Column column, Field fieldDefn, Object data) {
        ...
        return convertValue(column, fieldDefn, data,
                io.debezium.data.geometry.Geometry.createValue(fieldDefn.schema(), empty.getWkb(), empty.getSrid()),
                (r) -> {
                    if (data instanceof byte[]) {
                        // The binlog utility sends a byte array for any Geometry type, we will use our own binaryParse
                        // to parse the byte to WKB, hence
                ...
                (r) -> {
                    if (data instanceof byte[]) {
                        ...
                        if (data instanceof byte[]) {
                            ...
                            // binaryParse to parse the byte to WKB, hence
                            // to the suitable class
                            MySqlGeometry mySqlGeometry = MySqlGeometry.fromBytes((byte[]) data);
    ...
    protected Object convertUnsignedBigint(Column column, Field fieldDefn, Object data) {
        return convertValue(column, fieldDefn, data, 0L, (r) -> {
            if (data instanceof BigDecimal) {
                ...
                r.deliver(MySqlUnsignedIntegerConverter
                        .convertUnsignedBigint(new BigDecimal(((Number) data).toString())));
            } else if (data instanceof String) {
     ...
     *            the field definition; never null
     * @param data
     *            the data object to be converted into a {@link java.time.Duration} type; never null
     * @return the converted value, or null if the conversion could not be made and the column allows nulls
     * @throws IllegalArgumentException
     *             if the value could not be converted but the column does not allow nulls
     */
