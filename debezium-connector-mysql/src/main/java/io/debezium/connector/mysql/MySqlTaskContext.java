...
public final class MySqlTaskContext extends CdcSourceTaskContext {
    ...
    public MySqlTaskContext(Configuration config, Filters filters, Boolean tableIdCaseInsensitive,
        ...
        this.topicSelector = MySqlTopicSelector.defaultSelector(connectorConfig.getLogicalName(),
                connectorConfig.getHeartbeatTopicsPrefix());
        ...
        if (tableIdCaseInsensitive == null) {
            this.tableIdCaseInsensitive = !"0".equals(
                    connectionContext.readMySqlSystemVariables().get(MySqlSystemVariables.LOWER_CASE_TABLE_NAMES));
        } else {
            this.tableIdCaseInsensitive = tableIdCaseInsensitive;
        }

        // Set up the MySQL schema ...
        this.dbSchema = new MySqlSchema(connectorConfig, this.gtidSourceFilter, this.tableIdCaseInsensitive,
                topicSelector, filters);

        // Set up the record processor ...
        this.recordProcessor = new RecordMakers(dbSchema, source, topicSelector,
                connectorConfig.isEmitTombstoneOnDelete(), restartOffset);
    ...
    /**
     * Initialize the database history with any server-specific information. This should be done only upon connector
     * startup when the connector has no prior history.
     */
    ...
    public void loadHistory(SourceInfo startingPoint) {
        ...
        String systemCharsetNameFromHistory = dbSchema.systemVariables()
                .getVariable(MySqlSystemVariables.CHARSET_NAME_SERVER);
    ...
    public void start() {
        ...
        // Start the MySQL database history, which simply starts up resources but does not recover the history to a
        // specific point
        dbSchema().start();
    }
    ...
    /**
     * Run the supplied function in the temporary connector MDC context, and when complete always return the MDC context
     * to its state before this method was called.
     *
     * @param contextName
     *            the name of the context; may not be null
     * @param operation
     *            the function to run in the new MDC context; may not be null
     ...
     * consume a MySQL binlog that has multi-source replication enabled, if a failover has to occur. In such a case, the
     * server that Debezium is failed over to might have a different set of sources, but still include the sources
     * required for Debezium to continue to function. MySQL does not allow downstream replicas to connect if the GTID
     * set does not contain GTIDs for all channels that the server is replicating from, even if the server does have the
     * data needed by the client. To get around this, we can have Debezium merge its GTID set with whatever is on the
     * server, so that MySQL will allow it to connect. See <a href="https://issues.jboss.org/browse/DBZ-143">DBZ-143</a>
     * for details.
     *
     * This method does not mutate any state in the context.
     *
     * @param availableServerGtidSet
     *            the GTID set currently available in the MySQL server
     * @param purgedServerGtid
     *            the GTID set already purged by the MySQL server
     * @return A GTID set meant for consuming from a MySQL binlog; may return null if the SourceInfo has no GTIDs and
     *         therefore none were filtered
     */
    public GtidSet filterGtidSet(GtidSet availableServerGtidSet, GtidSet purgedServerGtid) {
        ...
        if (connectorConfig.gtidNewChannelPosition() == GtidNewChannelPosition.EARLIEST) {
            ...
            final GtidSet relevantAvailableServerGtidSet = (gtidSourceFilter != null)
                    ? availableServerGtidSet.retainAll(gtidSourceFilter) : availableServerGtidSet;
            LOGGER.info("Relevant GTID set available on server: {}", relevantAvailableServerGtidSet);

            mergedGtidSet = relevantAvailableServerGtidSet.retainAll(uuid -> knownGtidSet.forServerWithId(uuid) != null)
                    .with(purgedServerGtid).with(filteredGtidSet);
        } else {
    ...
    /**
     * Get the predicate function that will return {@code true} if a DDL has to be skipped over and left out of the
     * schema history or {@code false} when it should be processed.
     *
