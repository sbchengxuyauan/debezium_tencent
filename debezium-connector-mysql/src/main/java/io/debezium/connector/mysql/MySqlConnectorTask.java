...
public final class MySqlConnectorTask extends BaseSourceTask {
     ...
     * {@link TopicSelector#defaultSelector(String) default topic selector} of
    ...
    public ChangeEventSourceCoordinator start(Configuration config) {
        ...
        try {
            ...
            if (offsets != null) {
                Filters filters = SourceInfo.offsetsHaveFilterInfo(offsets) ? getOldFilters(offsets, config)
                ...
                if (!taskContext.historyExists()) {
                    if (taskContext.isSchemaOnlyRecoverySnapshot()) {
                        ...
                        if (!isBinlogAvailable()) {
                            String msg = "The connector is trying to read binlog starting at " + source
                ...
                if (taskContext.isSnapshotNeverAllowed()) {
                    ...
                    logger.info(
                            "Found no existing offset and snapshots disallowed, so starting at beginning of binlog");
                    source.setBinlogStartPoint("", 0L); // start from the beginning of the binlog
                    taskContext.initializeHistory();

                    // Look to see what the first available binlog file is called, and whether it looks like binlog
                    // files have
            ...
            if (!startWithSnapshot && source.gtidSet() == null && connectionContext.isGtidModeEnabled()) {
                // The snapshot will properly determine the GTID set, but we're not starting with a snapshot and GTIDs
                // were not
            ...
            if (startWithSnapshot) {
                ...
                if (!taskContext.getConnectorConfig().getSnapshotDelay().isZero()) {
                    // Adding a timed blocking reader to delay the snapshot, can help to avoid initial rebalancing
                    // interruptions
                    chainedReaderBuilder.addReader(new TimedBlockingReader("timed-blocker",
                            taskContext.getConnectorConfig().getSnapshotDelay()));
                }
                ...
                if (!source.hasFilterInfo()) {
                    ...
                    // 1. the snapshot was taken in a version of debezium before the filter info was stored in the
                    // offsets, or
                    // 2. this connector previously had no filter information.
                    // either way, we have to assume that the filter information currently in the config accurately
                    // reflects
                ...
                if (newTablesInConfig()) {
                    ...
                    if (taskContext.getConnectorConfig()
                            .getSnapshotNewTables() == MySqlConnectorConfig.SnapshotNewTables.PARALLEL) {
                        ServerIdGenerator serverIdGenerator = new ServerIdGenerator(
                                config.getLong(MySqlConnectorConfig.SERVER_ID),
                                config.getLong(MySqlConnectorConfig.SERVER_ID_OFFSET));
                        ParallelSnapshotReader parallelSnapshotReader = new ParallelSnapshotReader(config, taskContext,
                                getNewFilters(offsets, config), serverIdGenerator);

                        MySqlTaskContext unifiedTaskContext = createAndStartTaskContext(config, getAllFilters(config));
                        // we aren't completing a snapshot, but we need to make sure the "snapshot" flag is false for
                        // this new context.
            ...
            try {
                ...
                logger.error(
                        "Failed to start the connector (see other exception), but got this error while cleaning up", s);
            }
     ...
     *
     * However, if we were doing a parallel load with new tables, it's possible that the last committed offset is from
     * reading the new tables, which could be beyond where we want to restart from (and restarting there could cause
     * skipped events). To fix this, the new tables binlog reader records extra information in its offset to tell the
     * connector where to restart from. If this extra information is present in the stored offset, that is the offset
     * that is returned.
     * 
     * @param storedOffset
     *            the stored offset.
    ...
    private boolean newTablesInConfig() {
        ...
        // take in two stringified lists, and return true if the first list contains elements that are not in the second
        // list
        BiFunction<String, String, Boolean> hasExclusiveElements = (String a, String b) -> {
            ...
            return !Stream.of(a.split(elementSep)).filter((x) -> !bSet.contains(x)).collect(Collectors.toSet())
                    .isEmpty();
        };
        ...
        if (hasExclusiveElements.apply(config.getString(MySqlConnectorConfig.DATABASE_WHITELIST),
                sourceInfo.getDatabaseWhitelist())) {
            return true;
        }
        if (hasExclusiveElements.apply(config.getString(MySqlConnectorConfig.TABLE_WHITELIST),
                sourceInfo.getTableWhitelist())) {
            return true;
        }
        // if either blacklist has been removed from, then we may have new tables
        if (hasExclusiveElements.apply(sourceInfo.getDatabaseBlacklist(),
                config.getString(MySqlConnectorConfig.DATABASE_BLACKLIST))) {
            return true;
        }
        if (hasExclusiveElements.apply(sourceInfo.getTableBlacklist(),
    ...
    /**
     * Get the filters representing the tables that have been newly added to the config, but not those that previously
     * existed in the config.
     * 
     * @return {@link Filters}
     */
    ...
    /**
     * Get the filters representing those tables that previously existed in the config, but not those newly added to the
     * config.
     * 
     * @return {@link Filters}
     */
     ...
     * stop and before all their records have been consumed via the {@link #poll()} method. This method signals that all
     * of this has completed.
     */
    ...
    /**
     * Determine whether the binlog position as set on the {@link MySqlTaskContext#source() SourceInfo} is available in
     * the server.
     *
    ...
    protected boolean isBinlogAvailable() {
        ...
        if (gtidStr != null) {
            ...
            // GTIDs are enabled, and we used them previously, but retain only those GTID ranges for the allowed source
            // UUIDs ...
            ...
            if (gtidSet.isContainedWithin(availableGtidSet)) {
                logger.info("MySQL current GTID set {} does contain the GTID set required by the connector {}",
                ...
                final GtidSet nonPurgedGtidSetToReplicate = connectionContext.subtractGtidSet(gtidSetToReplicate,
                        purgedGtidSet);
                logger.info("GTIDs known by the server but not processed yet {}, for replication are available only {}",
                        gtidSetToReplicate, nonPurgedGtidSetToReplicate);
        ...
        if (!found) {
            if (logger.isInfoEnabled()) {
                logger.info("Connector requires binlog file '{}', but MySQL only has {}", binlogFilename,
    ...
    protected boolean isRowBinlogEnabled() {
        ...
        try {
            ...
            throw new ConnectException(
                    "Unexpected error while connecting to MySQL and looking at BINLOG row image mode: ", e);
        }
