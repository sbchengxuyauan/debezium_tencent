...
public class BinlogReader extends AbstractReader {
     ...
     *            see {@link AbstractReader#AbstractReader(String, MySqlTaskContext, Predicate)}
     */
     ...
     *            see {@link AbstractReader#AbstractReader(String, MySqlTaskContext, Predicate)}
     * @param serverId
     *            the server id to use for the {@link BinaryLogClient}
     */
    public BinlogReader(String name, MySqlTaskContext context, HaltingPredicate acceptAndContinue, long serverId) {
        ...
        // Use exponential delay to log the progress frequently at first, but the quickly tapering off to once an
        // hour...
        pollOutputDelay = ElapsedTimeStrategy.exponential(clock, INITIAL_POLL_PERIOD_IN_MILLIS,
                MAX_POLL_PERIOD_IN_MILLIS);

        // Set up the log reader ...
        client = new BinaryLogClient(connectionContext.hostname(), connectionContext.port(),
                connectionContext.username(), connectionContext.password());
        // BinaryLogClient will overwrite thread names later
        client.setThreadFactory(Threads.threadFactory(MySqlConnector.class,
                context.getConnectorConfig().getLogicalName(), "binlog-client", false));
        ...
        client.registerEventListener(context.bufferSizeForBinlogReader() == 0 ? this::handleEvent
        ...
        boolean filterDmlEventsByGtidSource = context.config()
                .getBoolean(MySqlConnectorConfig.GTID_SOURCE_FILTER_DML_EVENTS);
        ...
        eventDeserializer.setEventDataDeserializer(EventType.EXT_WRITE_ROWS,
                new RowDeserializers.WriteRowsDeserializer(tableMapEventByTableId).setMayContainExtraInformation(true));
    ...
    protected void doStart() {
        ...
        if (isRunning()) {
            ...
            try {
                ...
                // If the client thread is interrupted *before* the client could connect, the client throws a timeout
                // exception
                // The only way we can distinguish this is if we get the timeout exception before the specified timeout
                // has
                ...
                if (duration > (0.9 * timeout)) {
                    ...
                    throw new ConnectException("Timed out after " + actualSeconds
                            + " seconds while waiting to connect to MySQL at " + connectionContext.hostname() + ":"
                ...
                throw new ConnectException(
                        "Failed to authenticate to the MySQL database at " + connectionContext.hostname() + ":"
                                + connectionContext.port() + " with user '" + connectionContext.username() + "'",
                        e);
            } catch (Throwable e) {
                throw new ConnectException("Unable to connect to the MySQL database at " + connectionContext.hostname()
                        + ":" + connectionContext.port() + " with user '" + connectionContext.username() + "': "
                        + e.getMessage(), e);
            }
        }
    ...
    protected void handleEvent(Event event) {
        ...
        source.setBinlogTimestampSeconds(eventHeader.getTimestamp() / 1000L); // client returns milliseconds, but only
                                                                              // second
                                                                              // precision
        ...
        try {
            ...
            heartbeat.heartbeat(source.partition(), source.offset(),
                    (BlockingConsumer<SourceRecord>) this::enqueueRecord);
    ...
    /**
     * Handle the supplied event that is sent by a master to a slave to let the slave know that the master is still
    ...
    /**
     * Handle the supplied event that signals that an out of the ordinary event that occurred on the master. It notifies
     * the slave that something happened on the master that might cause data to be in an inconsistent state.
     *
     * @param event
     *            the server stopped event to be processed; may not be null
     */
        ...
        if (event.getData() instanceof EventDataDeserializationExceptionData) {
            ...
            if (eventDeserializationFailureHandlingMode == EventProcessingFailureHandlingMode.FAIL) {
                logger.error("Error while deserializing binlog event at offset {}.{}"
                        ...
                        source.offset(), System.lineSeparator(), eventHeader.getPosition(),
                ...
                logger.warn("Error while deserializing binlog event at offset {}.{}"
                        + "This exception will be ignored and the event be skipped.{}"
                        + "Use the mysqlbinlog tool to view the problematic event: mysqlbinlog --start-position={} --stop-position={} --verbose {}",
                        source.offset(), System.lineSeparator(), System.lineSeparator(), eventHeader.getPosition(),
                        eventHeader.getNextPosition(), source.binlogFilename(), data.getCause());
            }
        } else {
    ...
    /**
     * Handle the supplied event with a {@link RotateEventData} that signals the logs are being rotated. This means that
     * either the server was restarted, or the binlog has transitioned to a new file. In either case, subsequent table
     * numbers will be different than those seen to this point, so we need to {@link RecordMakers#clear() discard the
     * cache of record makers}.
     *
     * @param event
     ...
     * yet know whether this transaction contains any events we're interested in, but we have to record it so that we
     * know the position of this event and know we've processed the binlog to this point.
     * <p>
     * Note that this captures the current GTID and complete GTID set, regardless of whether the connector is
     * {@link MySqlTaskContext#gtidSourceFilter() filtering} the GTID set upon connection. We do this because we
     * actually want to capture all GTID set values found in the binlog, whether or not we process them. However, only
     * when we connect do we actually want to pass to MySQL only those GTID ranges that are applicable per the
     * configuration.
     *
     * @param event
    ...
    /**
     * Handle the supplied event with an {@link RowsQueryEventData} by recording the original SQL query that generated
     * the event.
     *
     * @param event
     *            the database change data event to be processed; may not be null
    ...
    /**
     * Handle the supplied event with an {@link QueryEventData} by possibly recording the DDL statements as changes in
     * the MySQL schemas.
     *
     * @param event
     *            the database change data event to be processed; may not be null
    ...
    protected void handleQueryEvent(Event event) throws InterruptedException {
        ...
        if (upperCasedStatementBegin.equals("INSERT ") || upperCasedStatementBegin.equals("UPDATE ")
            ...
            throw new ConnectException("Received DML '" + sql
        ...
        context.dbSchema().applyDdl(context.source(), command.getDatabase(), command.getSql(),
                (dbName, tables, statements) -> {
                    if (recordSchemaChangesInSourceRecords
                            && recordMakers.schemaChanges(dbName, tables, statements, super::enqueueRecord) > 0) {
     ...
     * monotonically-increasing numeric identifier, and this identifier is used within subsequent events within the same
     * transaction. This table identifier can change when:
     * <ol>
    ...
    /**
     * If we receive an event for a table that is monitored but whose metadata we don't know, either ignore that event
     * or raise a warning or error as per the {@link MySqlConnectorConfig#INCONSISTENT_SCHEMA_HANDLING_MODE}
     * configuration.
     */
        ...
        if (tableId != null && context.dbSchema().isTableMonitored(tableId)) {
            ...
            if (inconsistentSchemaHandlingMode == EventProcessingFailureHandlingMode.FAIL) {
                logger.error(
                        ...
                        eventHeader.getNextPosition(), source.binlogFilename());
                throw new ConnectException("Encountered change event for table " + tableId
            ...
            if (startingRowNumber < numRows) {
                ...
                if (logger.isDebugEnabled()) {
                    if (startingRowNumber != 0) {
                        logger.debug("Recorded {} insert record(s) for last {} row(s) in event: {}", count,
                                numRows - startingRowNumber, event);
                    } else {
    ...
    protected void handleUpdate(Event event) throws InterruptedException {
        ...
        if (recordMaker != null) {
            ...
            if (startingRowNumber < numRows) {
                ...
                if (logger.isDebugEnabled()) {
                    if (startingRowNumber != 0) {
                        logger.debug("Recorded {} update record(s) for last {} row(s) in event: {}", count,
                                numRows - startingRowNumber, event);
                    } else {
    ...
    protected void handleDelete(Event event) throws InterruptedException {
        ...
        if (recordMaker != null) {
            ...
            if (startingRowNumber < numRows) {
                ...
                if (logger.isDebugEnabled()) {
                    if (startingRowNumber != 0) {
                        logger.debug("Recorded {} delete record(s) for last {} row(s) in event: {}", count,
                                numRows - startingRowNumber, event);
                    } else {
    ...
    protected final class ReaderThreadLifecycleListener implements LifecycleListener {
        ...
        public void onConnect(BinaryLogClient client) {
            ...
            logger.info("Connected to MySQL binlog at {}:{}, starting at {}", connectionContext.hostname(),
                    connectionContext.port(), source);
        }
