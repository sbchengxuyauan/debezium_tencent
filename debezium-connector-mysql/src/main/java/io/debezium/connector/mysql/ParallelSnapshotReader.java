...
/**
 * A reader that runs a {@link ChainedReader} consisting of a {@link SnapshotReader} and a {@link BinlogReader} for all
 * tables newly added to the config in parallel with a {@link BinlogReader} for all the tables previously in the config.
 */
public class ParallelSnapshotReader implements Reader {
     ...
     * @param noSnapshotContext
     *            The context for those tables not undergoing a snapshot.
     * @param snapshotFilters
     *            {@link Filters} matching the tables that should be snapshotted.
     * @param serverIdGenerator
     *            a generator for creating unconflicting serverIds.
     */
    public ParallelSnapshotReader(Configuration config, MySqlTaskContext noSnapshotContext, Filters snapshotFilters,
        ...
        ParallelHaltingPredicate oldTablesReaderHaltingPredicate = new ParallelHaltingPredicate(oldTablesReaderNearEnd,
                newTablesReaderNearEnd);
        ParallelHaltingPredicate newTablesReaderHaltingPredicate = new ParallelHaltingPredicate(newTablesReaderNearEnd,
                oldTablesReaderNearEnd);
        ...
        this.newTablesBinlogReader = new BinlogReader("newBinlog", newTablesContext, newTablesReaderHaltingPredicate,
                serverIdGenerator.getNextServerId());
        this.newTablesReader = new ChainedReader.Builder().addReader(newTablesSnapshotReader)
                .addReader(newTablesBinlogReader).build();
    }

    // for testing purposes
    /* package private */ ParallelSnapshotReader(BinlogReader oldTablesBinlogReader,
            SnapshotReader newTablesSnapshotReader, BinlogReader newTablesBinlogReader) {
    ...
    public List<SourceRecord> poll() throws InterruptedException {
        // the old tables reader is a raw BinlogReader and will throw an exception of poll is called when it is not
        // running.
    ...
    /**
     * A Halting Predicate for the parallel snapshot reader. Usage for this predicate assumes two readers using two
     * ParallelHalting Predicates. The booleans are owned by the two predicates, and keep track of whether they or the
     * other reader has reached the end of the binlog.
     *
     * Test returns false if both both readers have been determined to be near the end of the binlog. Being near the end
     * of the binlog is determined to be this predicate having seen a record with a timestamp within
     * {@link ParallelHaltingPredicate#DEFAULT_MIN_HALTING_DURATION} of the current time. Once a single record near the
     * end of the binlog has been seen, we we assume the reader will stay near the end of the binlog.
     */
    /* package local */ static class ParallelHaltingPredicate implements HaltingPredicate {
        ...
        /* package local */ ParallelHaltingPredicate(AtomicBoolean thisReaderNearEndRef,
                AtomicBoolean otherReaderNearEndRef, Duration minHaltingDuration) {
