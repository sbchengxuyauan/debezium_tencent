 ...
 * <p>
 * A few of the {@link AbstractRowsEventDataDeserializer MySQL Binlog Client row deserializers} convert MySQL raw row
 * data into {@link java.sql.Date}, {@link java.sql.Time}, and {@link java.sql.Timestamp} values using {@link Calendar}
 * instances (and thus implicitly use the local timezone to calculate the milliseconds past epoch. Rather than perform
 * this conversion, we prefer to convert the raw MySQL row values directly into {@link LocalDate}, {@link LocalTime},
 * {@link LocalDateTime}, and {@link OffsetDateTime}.
 * <p>
 ...
 * {@link UpdateRowsEventDataDeserializer}, and {@link WriteRowsEventDataDeserializer} subclasses. Since we can't
 * provide a new base class, the simplest way to override these methods is to subclass each of these 3 subclasses and
 * override the methods on all 3 classes. It's ugly, but it works.
 * <p>
...
class RowDeserializers {
     ...
     *
     * @param length
     *            the number of bytes used to store the length of the string
     * @param inputStream
     *            the binary stream containing the raw binlog event data for the value
     ...
     *
     * @param meta
     *            the {@code meta} value containing the number of bytes in the length field
     * @param inputStream
     *            the binary stream containing the raw binlog event data for the value
     * @return the {@code byte[]} object
     * @throws IOException
     ...
     *
     * @param meta
     *            the {@code meta} value containing the fractional second precision, or {@code fsp}
     * @param inputStream
     *            the binary stream containing the raw binlog event data for the value
    ...
    protected static Serializable deserializeTimeV2(int meta, ByteArrayInputStream inputStream) throws IOException {
         ...
         * (in big endian)
         *
         * 1 bit sign (1= non-negative, 0= negative) 1 bit unused (reserved for future extensions) 10 bits hour (0-838)
         * 6 bits minute (0-59) 6 bits second (0-59)
         *
         * (3 bytes in total)
         *
     ...
     * <p>
     * This method treats all <a href="http://dev.mysql.com/doc/refman/5.7/en/date-and-time-types.html">zero values</a>
     * for {@code DATETIME} columns as NULL, since they cannot be accurately represented as valid {@link LocalDateTime}
     * objects.
     *
     * @param inputStream
     *            the binary stream containing the raw binlog event data for the value
    ...
    protected static Serializable deserializeDatetimeV2(int meta, ByteArrayInputStream inputStream) throws IOException {
         ...
         * (in big endian)
         *
         * 1 bit sign (1= non-negative, 0= negative) 17 bits year*13+month (year 0-9999, month 0-12) 5 bits day (0-31) 5
    ...
    /**
     * Converts a MySQL {@code TIMESTAMP} value <em>without fractional seconds</em> to a {@link OffsetDateTime}. MySQL
     * stores the {@code TIMESTAMP} values as seconds past epoch in UTC, but the resulting {@link OffsetDateTime} will
    ...
    /**
     * Converts a MySQL {@code TIMESTAMP} value <em>with fractional seconds</em> to a {@link OffsetDateTime}. MySQL
     * stores the {@code TIMESTAMP} values as seconds + fractional seconds past epoch in UTC, but the resulting
     ...
     * <p>
     * We can't use/access the private {@code split} method in the {@link AbstractRowsEventDataDeserializer} class, so
     * we replicate it here. Note the original is licensed under the same Apache Software License 2.0 as Debezium.
     *
     * @param value
     *            the long value
     * @param divider
     *            the value used to separate the individual values (e.g., 10 to separate each digit into a separate
     *            value, 100 to separate each pair of digits into a separate value, 1000 to separate each 3 digits into
     *            a separate value, etc.)
     * @param length
     *            the expected length of the integer array
     ...
     * <p>
     * We can't use/access the private {@code bigEndianLong} method in the {@link AbstractRowsEventDataDeserializer}
     * class, so we replicate it here. Note the original is licensed under the same Apache Software License 2.0 as
     * Debezium.
     *
     * @param bytes
     *            the bytes containing the big-endian representation of the value
     * @param offset
     *            the offset within the {@code bytes} byte array where the value starts
     * @param length
     *            the length of the byte representation within the {@code bytes} byte array
     ...
     * <p>
     * We can't use/access the private {@code bitSlice} method in the {@link AbstractRowsEventDataDeserializer} class,
     * so we replicate it here. Note the original is licensed under the same Apache Software License 2.0 as Debezium.
     *
     * @param value
     *            the long containing the integer encoded within it
     * @param bitOffset
     *            the number of bits where the integer value starts
     * @param numberOfBits
     *            the number of bits in the integer value
     * @param payloadSize
     *            the total number of bits used in the {@code value}
    ...
    /**
     * Read the binary input stream to obtain the number of nanoseconds given the <em>fractional seconds precision</em>,
     * or <em>fsp</em>.
     * <p>
     * We can't use/access the {@code deserializeFractionalSeconds} method in the
     * {@link AbstractRowsEventDataDeserializer} class, so we replicate it here with modifications to support
     * nanoseconds rather than microseconds. Note the original is licensed under the same Apache Software License 2.0 as
     * Debezium.
     *
     * @param fsp
     *            the fractional seconds precision describing the number of digits precision used to store the
     *            fractional seconds (e.g., 1 for storing tenths of a second, 2 for storing hundredths, 3 for storing
     *            milliseconds, etc.)
     * @param inputStream
     ...
     *             if there is an error reading from the binlog event data
     */
    protected static int deserializeFractionalSecondsInNanos(int fsp, ByteArrayInputStream inputStream)
            throws IOException {
     ...
     * We can't use/access the {@code deserializeFractionalSeconds} method in the
     * {@link AbstractRowsEventDataDeserializer} class, so we replicate it here with modifications to support
     * nanoseconds rather than microseconds and negative values. Note the original is licensed under the same Apache
     * Software License 2.0 as Debezium.
     *
     * @param fsp
     *            the fractional seconds precision describing the number of digits precision used to store the
     *            fractional seconds (e.g., 1 for storing tenths of a second, 2 for storing hundredths, 3 for storing
     ...
     *             if there is an error reading from the binlog event data
     */
    protected static int deserializeFractionalSecondsInNanosNegative(int fsp, ByteArrayInputStream inputStream)
            throws IOException {
        // Calculate the number of bytes to read, which is
        // '1' when fsp=(1,2)
        // '2' when fsp=(3,4) and
