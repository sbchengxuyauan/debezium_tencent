...
public class SqlServerConnectorConfig extends HistorizedRelationalDatabaseConnectorConfig {
    ...
    public static enum SnapshotIsolationMode implements EnumeratedValue {
        ...
        /**
         * This mode uses SNAPSHOT isolation level. This way reads and writes are not blocked for the entire duration of
         * the snapshot. Snapshot consistency is guaranteed as long as DDL statements are not executed at the time.
         */
        SNAPSHOT("snapshot"),

        /**
         * This mode uses REPEATABLE READ isolation level. This mode will avoid taking any table locks during the
         * snapshot process, except schema snapshot phase where exclusive table locks are acquired for a short period.
         * Since phantom reads can occur, it does not fully guarantee consistency.
         */
        REPEATABLE_READ("repeatable_read"),

        /**
         * This mode uses READ COMMITTED isolation level. This mode does not take any table locks during the snapshot
         * process. In addition, it does not take any long-lasting row-level locks, like in repeatable read isolation
         * level. Snapshot consistency is not guaranteed.
         */
    ...
    public static final Field SERVER_TIMEZONE = Field
            .create(DATABASE_CONFIG_PREFIX + SqlServerConnection.SERVER_TIMEZONE_PROP_NAME)
            .withDisplayName("Server timezone").withType(Type.STRING).withImportance(Importance.LOW)
            .withValidation((config, field, problems) -> {
                ...
                return 0;
                    ...
                    "The timezone of the server used to correctly shift the commit transaction timestamp on the client side"
    ...
    public static final Field SNAPSHOT_MODE = Field.create("snapshot.mode").withDisplayName("Snapshot mode")
            ...
            .withValidation(SqlServerConnectorConfig::validateSnapshotMode).withWidth(Width.SHORT)
    ...
    public static final Field SNAPSHOT_ISOLATION_MODE = Field.create("snapshot.isolation.mode")
            ...
            .withEnum(SnapshotIsolationMode.class, SnapshotIsolationMode.REPEATABLE_READ).withWidth(Width.SHORT)
            .withImportance(Importance.LOW).withDescription(
                    "Controls which transaction isolation level is used and how long the connector locks the monitored tables. "
     ...
     */
    public static Field.Set ALL_FIELDS = Field.setOf(SERVER_NAME, DATABASE_NAME, HOSTNAME, PORT, USER, PASSWORD,
            SNAPSHOT_MODE, SERVER_TIMEZONE, RelationalDatabaseConnectorConfig.SNAPSHOT_LOCK_TIMEOUT_MS,
            ...
            RelationalDatabaseConnectorConfig.TABLE_IGNORE_BUILTIN, RelationalDatabaseConnectorConfig.COLUMN_BLACKLIST,
            RelationalDatabaseConnectorConfig.MSG_KEY_COLUMNS, RelationalDatabaseConnectorConfig.DECIMAL_HANDLING_MODE,
            RelationalDatabaseConnectorConfig.TIME_PRECISION_MODE, CommonConnectorConfig.POLL_INTERVAL_MS,
            CommonConnectorConfig.MAX_BATCH_SIZE, CommonConnectorConfig.MAX_QUEUE_SIZE,
    ...
    public static ConfigDef configDef() {
        ...
        Field.group(config, "SQL Server", SERVER_NAME, DATABASE_NAME, HOSTNAME, PORT, USER, PASSWORD, SNAPSHOT_MODE,
                SERVER_TIMEZONE);
        ...
        Field.group(config, "Events", RelationalDatabaseConnectorConfig.TABLE_WHITELIST,
                RelationalDatabaseConnectorConfig.TABLE_BLACKLIST, RelationalDatabaseConnectorConfig.COLUMN_BLACKLIST,
