...
/**
 * <p>
 * A {@link StreamingChangeEventSource} based on SQL Server change data capture functionality. A main loop polls
 * database DDL change and change data tables and turns them into change events.
 * </p>
 *
 * <p>
 * The connector uses CDC functionality of SQL Server that is implemented as as a process that monitors source table and
 * write changes from the table into the change table.
 * </p>
 *
 * <p>
 * The main loop keeps a pointer to the LSN of changes that were already processed. It queries all change tables and get
 * result set of changes. It always finds the smallest LSN across all tables and the change is converted into the event
 * message and sent downstream. The process repeats until all result sets are empty. The LSN is marked and the procedure
 * repeats.
 * </p>
 *
 * <p>
 * The schema changes detection follows the procedure recommended by SQL Server CDC documentation. The database operator
 * should create one more capture process (and table) when a table schema is updated. The code detects presence of two
 * change tables for a single source table. It decides which table is the new one depending on LSNs stored in them. The
 * loop streams changes from the older table till there are events in new table with the LSN larger than in the old one.
 * Then the change table is switched and streaming is executed from the new one.
 * </p>
 *
...
public class SqlServerStreamingChangeEventSource implements StreamingChangeEventSource {
    ...
    private static final Pattern MISSING_CDC_FUNCTION_CHANGES_ERROR = Pattern
            .compile("Invalid object name 'cdc.fn_cdc_get_all_changes_(.*)'\\.");
    ...
    public SqlServerStreamingChangeEventSource(SqlServerConnectorConfig connectorConfig,
            SqlServerOffsetContext offsetContext, SqlServerConnection dataConnection,
            SqlServerConnection metadataConnection, EventDispatcher<TableId> dispatcher, ErrorHandler errorHandler,
    ...
    public void execute(ChangeEventSourceContext context) throws InterruptedException {
        ...
        final Queue<ChangeTable> schemaChangeCheckpoints = new PriorityQueue<>(
                    ...
                    dataConnection.getChangesForTables(tablesSlot.get(), fromLsn, currentMaxLsn, resultSets -> {
                        ...
                        for (;;) {
                            ...
                            if (!(tableWithSmallestLsn.getChangePosition().isAvailable()
                                ...
                                LOGGER.error("Skipping change {} as its LSN is NULL which is not expected",
                            ...
                            if (tableWithSmallestLsn.getChangePosition().compareTo(lastProcessedPositionOnStart) < 0) {
                                LOGGER.info(
                                        ...
                                        tableWithSmallestLsn, lastProcessedPositionOnStart);
                                tableWithSmallestLsn.next();
                                continue;
                            }
                            // After restart for change that was the last committed and operations in it before the last
                            // committed offset
                            if (tableWithSmallestLsn.getChangePosition().compareTo(lastProcessedPositionOnStart) == 0
                                ...
                                LOGGER.info(
                                        ...
                                        tableWithSmallestLsn, eventSerialNoInInitialTx, lastProcessedPositionOnStart,
                                        lastProcessedEventSerialNoOnStart);
                            ...
                            if (tableWithSmallestLsn.getChangeTable().getStopLsn().isAvailable()
                                    && tableWithSmallestLsn.getChangeTable().getStopLsn()
                                            .compareTo(tableWithSmallestLsn.getChangePosition().getCommitLsn()) <= 0) {
                                LOGGER.debug(
                                        "Skipping table change {} as its stop LSN is smaller than the last recorded LSN {}",
                                        tableWithSmallestLsn, tableWithSmallestLsn.getChangePosition());
                                tableWithSmallestLsn.next();
                                continue;
                            }
                            ...
                            if (!schemaChangeCheckpoints.isEmpty()) {
                                if (tableWithSmallestLsn.getChangePosition().getCommitLsn()
                                        .compareTo(schemaChangeCheckpoints.peek().getStartLsn()) >= 0) {
                                    migrateTable(schemaChangeCheckpoints);
                                }
                            }
                            ...
                            if (operation == SqlServerChangeRecordEmitter.OP_UPDATE_BEFORE) {
                                if (!tableWithSmallestLsn.next() || tableWithSmallestLsn
                                        .getOperation() != SqlServerChangeRecordEmitter.OP_UPDATE_AFTER) {
                                    throw new IllegalStateException("The update before event at "
                                            + tableWithSmallestLsn.getChangePosition() + " for table " + tableId
                                            + " was not followed by after event.\n Please report this as a bug together with a events around given LSN.");
                                }
                                eventCount = 2;
                            }
                            final Object[] dataNext = (operation == SqlServerChangeRecordEmitter.OP_UPDATE_BEFORE)
                                    ? tableWithSmallestLsn.getData() : null;
                            ...
                            offsetContext.event(tableWithSmallestLsn.getChangeTable().getSourceTableId(),
                                    metadataConnection
                                            .timestampOfLsn(tableWithSmallestLsn.getChangePosition().getCommitLsn()));

                            dispatcher.dispatchDataChangeEvent(tableId,
                                    new SqlServerChangeRecordEmitter(offsetContext, operation, data, dataNext, clock));
                            tableWithSmallestLsn.next();
                        }
                    });
    ...
    private void migrateTable(final Queue<ChangeTable> schemaChangeCheckpoints)
        ...
        dispatcher.dispatchSchemaChangeEvent(newTable.getSourceTableId(),
                new SqlServerSchemaChangeEventEmitter(offsetContext, newTable,
                        metadataConnection.getTableSchemaFromTable(newTable), SchemaChangeEventType.ALTER));
    }

    private ChangeTable[] processErrorFromChangeTableQuery(SQLException exception, ChangeTable[] currentChangeTables)
            throws Exception {
        ...
        if (m.matches()) {
            ...
            return Arrays.asList(currentChangeTables).stream().filter(x -> !x.getCaptureInstance().equals(captureName))
    ...
    private ChangeTable[] getCdcTablesToQuery() throws SQLException, InterruptedException {
        ...
        final Map<TableId, List<ChangeTable>> whitelistedCdcEnabledTables = cdcEnabledTables.stream()
                .filter(changeTable -> {
                    if (connectorConfig.getTableFilters().dataCollectionFilter()
        ...
        for (List<ChangeTable> captures : whitelistedCdcEnabledTables.values()) {
            ...
            if (captures.size() > 1) {
                ...
                LOGGER.info("Multiple capture instances present for the same table: {} and {}", currentTable,
                        futureTable);
            }
            if (schema.tableFor(currentTable.getSourceTableId()) == null) {
                LOGGER.info("Table {} is new to be monitored by capture instance {}", currentTable.getSourceTableId(),
                        currentTable.getCaptureInstance());
                // We need to read the source table schema - nullability information cannot be obtained from change
                // table
                dispatcher.dispatchSchemaChangeEvent(currentTable.getSourceTableId(),
                        new SqlServerSchemaChangeEventEmitter(offsetContext, currentTable,
                                dataConnection.getTableSchemaFromTable(currentTable), SchemaChangeEventType.CREATE));
            }
    ...
    /**
     * The logical representation of a position for the change in the transaction log. During each sourcing cycle it is
     * necessary to query all change tables and then make a total order of changes across all tables.<br>
     * This class represents an open database cursor over the change table that is able to move the cursor forward and
     * report the LSN for the change to which the cursor now points.
     *
     * @author Jiri Pechanec
     *
     */
    private static class ChangeTablePointer {
        ...
        public boolean next() throws SQLException {
            ...
            currentChangePosition = completed ? TxLogPosition.NULL : TxLogPosition.valueOf(
                    Lsn.valueOf(resultSet.getBytes(COL_COMMIT_LSN)), Lsn.valueOf(resultSet.getBytes(COL_ROW_LSN)));
            if (completed) {
