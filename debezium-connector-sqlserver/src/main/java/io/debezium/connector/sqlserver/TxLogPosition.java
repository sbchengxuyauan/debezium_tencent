...
/**
 * Defines a position of change in the transaction log. The position is defined as a combination of commit LSN and
 * sequence number of the change in the given transaction. The sequence number is monotonically increasing in
 * transaction but it is not guaranteed across multiple transactions so the combination is necessary to get total order.
 *
...
public class TxLogPosition implements Nullable, Comparable<TxLogPosition> {
    ...
    public static TxLogPosition valueOf(Lsn commitLsn, Lsn inTxLsn) {
        return commitLsn == null && inTxLsn == null ? NULL
                : new TxLogPosition(commitLsn == null ? Lsn.NULL : commitLsn, inTxLsn == null ? Lsn.NULL : inTxLsn);
    }
