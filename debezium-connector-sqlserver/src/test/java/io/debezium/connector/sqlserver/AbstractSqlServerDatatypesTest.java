...
public abstract class AbstractSqlServerDatatypesTest extends AbstractConnectorTest {
    ...
    private static final String DDL_STRING = "create table type_string (" + "  id int not null, "
            + "  val_char char(3), " + "  val_varchar varchar(1000), " + "  val_text text, " + "  val_nchar nchar(3), "
            + "  val_nvarchar nvarchar(1000), " + "  val_ntext ntext, " + "  primary key (id)" + ")";

    private static final String DDL_FP = "create table type_fp (" + "  id int not null, "
            + "  val_decimal decimal(6,3), " + "  val_numeric numeric, " + "  val_float float, " + "  val_real real, "
            + "  val_smallmoney smallmoney, " + "  val_money money " + "  primary key (id)" + ")";

    private static final String DDL_INT = "create table type_int (" + "  id int not null, " + "  val_bit bit, "
            + "  val_tinyint tinyint, " + "  val_smallint smallint, " + "  val_int int, " + "  val_bigint bigint, "
            + "  primary key (id)" + ")";

    private static final String DDL_TIME = "create table type_time (" + "  id int not null, " + "  val_date date, "
            + "  val_time_p2 time(2), " + "  val_time time(4), " + "  val_datetime2 datetime2, "
            + "  val_datetimeoffset datetimeoffset, " + "  val_datetime datetime, "
            + "  val_smalldatetime smalldatetime, " + "  primary key (id)" + ")";

    private static final String DDL_XML = "create table type_xml (" + "  id int not null, " + "  val_xml xml, "
            + "  primary key (id)" + ")";
    ...
    private static final List<SchemaAndValueField> EXPECTED_FP = Arrays.asList(new SchemaAndValueField("val_decimal",
            Decimal.builder(3).parameter(PRECISION_PARAMETER_KEY, "6").optional().build(), new BigDecimal("1.123")),
            new SchemaAndValueField("val_numeric",
                    Decimal.builder(0).parameter(PRECISION_PARAMETER_KEY, "18").optional().build(),
                    new BigDecimal("2")),
            ...
            new SchemaAndValueField("val_smallmoney",
                    Decimal.builder(4).parameter(PRECISION_PARAMETER_KEY, "10").optional().build(),
                    new BigDecimal("5.3230")),
            new SchemaAndValueField("val_money",
                    Decimal.builder(4).parameter(PRECISION_PARAMETER_KEY, "19").optional().build(),
                    new BigDecimal("6.3230")));
    ...
    private static final List<SchemaAndValueField> EXPECTED_DATE_TIME = Arrays.asList(
            ...
            new SchemaAndValueField("val_datetime2", NanoTimestamp.builder().optional().build(),
                    1_531_481_025_340_000_000L),
            new SchemaAndValueField("val_datetimeoffset", ZonedTimestamp.builder().optional().build(),
                    "2018-07-13T12:23:45.456+11:00"),
    ...
    private static final List<SchemaAndValueField> EXPECTED_DATE_TIME_AS_CONNECT = Arrays.asList(
            new SchemaAndValueField("val_date", org.apache.kafka.connect.data.Date.builder().optional().build(),
                    java.util.Date.from(LocalDate.of(2018, 7, 13).atStartOfDay().atOffset(ZoneOffset.UTC).toInstant())),
            ...
            new SchemaAndValueField("val_time", org.apache.kafka.connect.data.Time.builder().optional().build(),
                    java.util.Date.from(LocalTime.of(10, 23, 45, 678_900_000).atDate(LocalDate.ofEpochDay(0))
                            .atOffset(ZoneOffset.UTC).toInstant())),
            new SchemaAndValueField("val_datetime2",
                    org.apache.kafka.connect.data.Timestamp.builder().optional().build(),
                    java.util.Date.from(LocalDateTime.of(2018, 7, 13, 11, 23, 45, 340_000_000).atOffset(ZoneOffset.UTC)
                            .toInstant())),
            new SchemaAndValueField("val_datetimeoffset", ZonedTimestamp.builder().optional().build(),
                    "2018-07-13T12:23:45.456+11:00"),
            new SchemaAndValueField("val_datetime",
                    org.apache.kafka.connect.data.Timestamp.builder().optional().build(),
                    java.util.Date.from(LocalDateTime.of(2018, 7, 13, 13, 23, 45, 780_000_000).atOffset(ZoneOffset.UTC)
                            .toInstant())),
            new SchemaAndValueField("val_smalldatetime",
                    org.apache.kafka.connect.data.Timestamp.builder().optional().build(), java.util.Date
                            .from(LocalDateTime.of(2018, 7, 13, 14, 24, 00).atOffset(ZoneOffset.UTC).toInstant())));

    private static final List<SchemaAndValueField> EXPECTED_XML = Arrays
            .asList(new SchemaAndValueField("val_xml", Schema.OPTIONAL_STRING_SCHEMA, "<a>b</a>"));
    ...
    public void init(TemporalPrecisionMode temporalPrecisionMode) throws Exception {
        ...
        Configuration config = TestHelper.defaultConfig()
                ...
                .with(RelationalDatabaseConnectorConfig.TIME_PRECISION_MODE, temporalPrecisionMode).build();
