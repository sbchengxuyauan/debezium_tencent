...
public class TransactionMetadataIT extends AbstractConnectorTest {
    ...
    public void transactionMetadata() throws Exception {
        ...
        final Configuration config = TestHelper.defaultConfig()
                ...
                .with(SqlServerConnectorConfig.PROVIDE_TRANSACTION_METADATA, true).build();
    ...
    private void restartInTheMiddleOfTx(boolean restartJustAfterSnapshot, boolean afterStreaming) throws Exception {
        ...
        if (afterStreaming) {
            ...
            assertRecord(((Struct) records.allRecordsInOrder().get(1).value()).getStruct(Envelope.FieldName.AFTER),
                    expectedRow);
        ...
        for (int i = 0; i < RECORDS_PER_TABLE / 2; i++) {
            ...
            assertRecordTransactionMetadata(recordA, batchTxId, RECORDS_PER_TABLE + 2 * i + 1,
                    RECORDS_PER_TABLE / 2 + i + 1);
            assertRecordTransactionMetadata(recordB, batchTxId, RECORDS_PER_TABLE + 2 * i + 2,
                    RECORDS_PER_TABLE / 2 + i + 1);
        }
        ...
        // END of previous TX, data records, BEGIN of TX for every pair of record, END of TX for every pair of record
        // but last
        ...
        for (int i = 0; i < RECORDS_PER_TABLE; i++) {
            ...
            if (i < RECORDS_PER_TABLE - 1) {
                assertEndTransaction(txMetadata.get(2 * i + 2), txId, 2,
                        Collect.hashMapOf("testDB.dbo.tablea", 1, "testDB.dbo.tableb", 1));
            }
        }
    }
